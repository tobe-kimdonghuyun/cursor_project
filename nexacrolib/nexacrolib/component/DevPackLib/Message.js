...
/**
 * @class 메세지팝업오픈
 * @param {String} sMsgId - 메세지ID 또는 메시지로 사용할 문자열
...
pForm.gfnAlert = function (sMsgId, arrArg, sPopId, sCallback, arrButton, arrRtn)
{
	...
	} else
	{
		...
		} else
		{
			...
			{
				sMsg = objApp.gdsMessage.lookup("MSGE_CD", sMsgId, "KORN_MSGE");
				sMsgType = objApp.gdsMessage.lookup("MSGE_CD", sMsgId, "MSGE_FLAG_CD");
			}
			// 입력한 문자열을 메시지로 사용
			else
			{
				sMsg = sMsgId;
				sMsgType = "INF"; // 등록하지 않은 메시지는 "알림"으로 처리
			}
