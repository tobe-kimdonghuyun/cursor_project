...
if (!nexacro.Tab)
{
    ...
    _pTab.removeTabpage = function (nIndex)
    {
		...
		if (tabbuttonitems_len > 0)
        {
            ...
            if ((begin_index + this._visibleTabbuttonCount) > tabbuttonitems_len)
    ...
    _pTab._rearrangeContents = function (beginTabindex)
    {
        ...
        ******/
        ...
        if (beginTabindex == undefined || beginTabindex == null)
        {
            ...
            var temp_begin = this._getFirstTabbuttonIndex();           
            if (temp_begin > -1)
            {
                var cnt = 0;
                for (i = temp_begin; i < tabbuttonitems_len; i++)
                {
                    tabbuttonitem = tabbuttonitems[i];
                    tabbuttonitem_pos = tabbuttonitem._position;
                    if (tabposition == "top" || tabposition == "bottom")
                    ...
                    if (!is_within_maxsize)
                        break;
                    cnt++;
                }
                if (is_within_maxsize)
                {
                    for (i = 0; i < temp_begin; i++)
                    {
                        tabbuttonitem = tabbuttonitems[i];
                        tabbuttonitem_pos = tabbuttonitem._position;
                        if (tabposition == "top" || tabposition == "bottom")
                ...
                temp_width = 0;
                temp_height = 0;
                if (cnt == this._visibleTabbuttonCount && temp_begin <= tabindex && tabindex < (temp_begin + cnt))
                    beginTabindex = temp_begin;
            }
            ...
            if (beginTabindex == undefined || beginTabindex == null)
            {
                for (i = tabindex; i >= 0; i--)
                {
                    ...
                    {
                        is_within_maxsize = temp_width <= tab_max_width;
                        is_visible = nexacro._enableaccessibility ? (tabbuttonitem._p_left > -1) : true;
                    }
                    else
                    {
                        is_within_maxsize = temp_height <= tab_max_height;
                        is_visible = nexacro._enableaccessibility ? (tabbuttonitem._p_top > -1) : true;
                    }
        ...
        this._visibleTabbuttonCount = tabbuttonitems_len;
        for (i = 0; i < tabbuttonitems_len; i++)
        {
            tabbuttonitem = tabbuttonitems[i];
            tabbuttonitem_pos = tabbuttonitem._position;
...
*/                
    ...
    _pTab._getComponentsByTaborder = function (p, filter_type)
    {
		...
		{
			if (p && p == this && filter_type == 15 && !this._focusobj)
				return null;
		}
