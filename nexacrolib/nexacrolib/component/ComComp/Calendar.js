...
if (!nexacro.Calendar)
{
	...
	_pCalendar._p_restoreonblur = true;
	_pCalendar._real_restoreonblur = undefined;
	...
	_pCalendar._apply_bind_process = false; // bindapply로 value 변경 체크
	...
	_pCalendar.on_change_bindSource = function (propid, ds, row, col)
	{
		if (propid == "value")
		{
			...
			this._on_apply_text();
			this._apply_bind_process = true; // bind된 Dataset에서 실제 변경이 수신됬는지 확인
		}
	};
	...
	//===============================================================
	_pCalendar.set_restoreonblur = function(v)
	{
		v= nexacro._toBoolean(v);
		
		if(this._real_restoreonblur != v || this._p_restoreonblur != v)
		{
			this._p_restoreonblur = v;
			this._real_restoreonblur = v;
		}
	};
	...
	_pCalendar.set_useautocalibration = function(v)
	{
		...
		if(this._useautocalibration != v || this._p_useautocalibration != v)
		{
	...
	_pCalendar._properties = [{ name: "text" }, { name: "value" }, { name: "displaynulltext" }, { name: "displayinvalidtext" }, { name: "readonly" }, { name: "autoselect" },
		...
		{ name: "usetrailingday" }, { name: "showmonthspin" }, { name: "showyearspin" }, { name: "visible" }, { name: "calendaredit", readonly: true }, { name: "calendarspindownbutton", readonly: true }, { name: "calendarspinupbutton", readonly: true }, { name: "datepicker", readonly: true }, { name: "dropbutton", readonly: true }, { name: "useautocalibration"}, { name: "restoreonblur"}];
	...
	_pCalendar.applyto_bindSource = function (propid, Val)
	{
		this._apply_bind_process = false;
		return nexacro.Component.prototype.applyto_bindSource.call(this, propid, Val);
	};
	...
	_pCalendar._on_value_change = function (prevalue, postvalue)
	{
		...
		else if (this._bind_event)
		{
			...
			else if (this._apply_bind_process)
			{
				// on_change_bindSource가 실제 실행되어 _p_value가 이미 확정됨
				this._apply_bind_process = false;
				postvalue = this._p_value;
				if (postvalue === prevalue)
				{
					// bind가 같은 값으로 복원한 경우 변경이 없기 때문에 이벤트 발생 하지 않도록 함
					bclose = true;
					ret = false;
				}
				else
				{
					posttext = this._getEventInfoText(postvalue);
				}
			}
		}
	...
	_pCalendar.on_killfocus_basic_action = function (/*new_focus, new_refer_focus*/)
	{
		...
		else
		{
			...
			var restore = this._real_restoreonblur !== undefined ? this.restoreonblur : nexacro._calendarrestoreonblur;
