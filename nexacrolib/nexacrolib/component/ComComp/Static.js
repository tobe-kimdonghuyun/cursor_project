...
if (!nexacro._IconText)
{
    ...
    _pIconText._on_getFitSize = function ()
    {
        ...
        if (control_elem)
        {
            ...
            if (text)
            {
				...
				if (this._p_fittocontents == "height")
				{
					...
					if (wordwrap && wordwrap != "none")
					{
						...
						if (cell_elem)
						{
							cell_elem.setElementWordWrap(wordwrap);
						}
						multiline = true;
            ...
            total_w = (this._minwidth && this._minwidth > total_w) ? this._minwidth : total_w;
            total_h = (this._minheight && this._minheight > total_h) ? this._minheight : total_h;
...
if (!nexacro.Static)
{
    ...
    _pStatic._curLinkIndex = -1;
    _pStatic._linkCount = -1;
    ...
    _pStatic.on_attach_contents_handle = function (win)
    {
        ...
        {
            cellElem.attachHandle(win);
            var links = cellElem._getLinkTagElements();
            if (links)
            {
                this._linkCount = links.length;
                if (this._linkCount > 0)
                {
                    this._p_tabstop = true;
                    this._is_focus_accept = true;
                }
            }   
        }
    ...
    //tabkey 제어
    _pStatic._getDlgCode = function (keycode, altKey, ctrlKey, shiftKey)
    {
        var _want_tab = this._want_tab;
        var _want_arrow = this._want_arrow;

        if (keycode == 9 && this._p_usedecorate)
        {
            _want_tab = this._isTabkeyFocusable(shiftKey);
        }
        return { want_tab: _want_tab, want_return: false, want_escape: false, want_chars: false, want_arrows: _want_arrow };
    };

    _pStatic.on_killfocus_basic_action = function (new_focus, new_refer_focus)
    {
        nexacro.Component.prototype.on_killfocus_basic_action.call(this);
        this._curLinkIndex = -1;
    };

    //테스트코드 (확인 필요)
    _pStatic._apply_setfocus = function (evt_name) 
    {
        if (evt_name == "shifttabkey" || evt_name == "tabkey")
        {            
            if (this._p_usedecorate)
            {
                if (this._linkCount > -1)
                {
                    //link로 포커싱     
                    if (evt_name == "shifttabkey")
                    {
                        this._curLinkIndex = this._linkCount - 1;
                    }
                    else if (this._curLinkIndex == -1)    
                    {
                        this._curLinkIndex++;
                    }
                    const cell_elem = this._cell_elem;
                    if (cell_elem)
                        cell_elem.setElementTagFocus(this._curLinkIndex);
                }
            }
        }
        else
        {
             nexacro.Component.prototype._apply_setfocus.call(this);
        }
    };

    _pStatic.on_keydown_default_action = function (keycode, alt_key, ctrl_key, shift_key, refer_comp, meta_key)
    {
        if (this._p_usedecorate && this._linkCount > 0)
        {
            if (keycode == nexacro.Event.KEY_TAB)
            {
                ...
                {
                    if (this._curLinkIndex < this._linkCount - 1)
                    {
                        this._curLinkIndex++
                    }
                }      
    ...
    //Mouse로 링크 클릭후 Tabkey로 tab이동시 스펙대로 움직이기위해 확인 동작(prototype)
    _pStatic.on_lbuttonup_default_action = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, refer_comp, meta_key)
    {
        if (this._linkCount > 0)
        {
            var cell_elem = this._cell_elem;            
    ...
    //===============================================================
    _pStatic._isTabkeyFocusable = function (shiftKey)
    {
        var ret = false;
        if ((shiftKey && this._curLinkIndex > 0)
            || (!shiftKey && this._curLinkIndex < (this._linkCount - 1)))
        {
            ret = true;
        }
        return ret;
    ...
    _pStatic._isFocusInLinkedComponent = function (shiftKey)
    {
        var ret = false;
        if ((shiftKey && (this._curLinkIndex > 0 && this._curLinkIndex < (this._linkCount - 1)))
            || (!shiftKey && this._curLinkIndex > 0 && this._curLinkIndex < (this._linkCount - 1)))
        {
            ret = true;
        }
        return ret;
    ...
    {		
		...
		{            
            ...
            if (cell_elem)
            {
                ...
                var links = cell_elem._getLinkTagElements();
                if (links)
                    this._linkCount = links.length;
                if (this._isAccessibilityRoleHeading() || this._linkCount > 0)
                {
                    if (nexacro._accessibilitytype > 3)
                    {
                        ...
                        if (control_elem)
                        {
                            ...
                            control_elem._refreshAccessibilityTree();
                        }
                    }
                    this._p_tabstop = true;
                    this._is_focus_accept = true;
        ...
        if (this._cell_elem && this._p_fittocontents != "none")
        {
            this._update_position();
        }
	};
...
if (!nexacro.FitStatic)
{
    //==============================================================================
    // nexacro.FitStatic
    //==============================================================================
    nexacro.FitStatic = function (id, left, top, width, height, right, bottom, minwidth, maxwidth, minheight, maxheight, parent)
    {
        nexacro.Static.call(this, id, left, top, width, height, right, bottom, minwidth, maxwidth, minheight, maxheight, parent);
    };

    var _pFitStatic = nexacro._createPrototype(nexacro.Static, nexacro.FitStatic);
    nexacro.FitStatic.prototype = _pFitStatic;
    _pFitStatic._type_name = "FitStatic";


    //===============================================================
    // nexacro.FitStatic : Create & Destroy & Update 
    //===============================================================
    _pFitStatic.on_create_contents = function ()
    {
        var control_elem = this.getElement();
        if (control_elem)
        {
            var cellElem = this._cell_elem = new nexacro.TextFitBoxElement(control_elem, "fittext");
            cellElem.setElementPosition(this._getClientLeft(), this._getClientTop());
            cellElem.setElementSize(control_elem.client_width, control_elem.client_height);
