...
if (nexacro.Div)
{
	...
	_pDiv.on_apply_prop_accessibilityenable = function ()
	{
		...
		if (!this._p_form || !this._is_created || !nexacro._enableaccessibility)
			return;
    ...
    _pDiv._updateAccessibilityControl = function ()
    {
        ...
        if (control_elem)
        {
            ...
            // innerform에 accessibilityenable이 true인 component가 존재하면 내부 초점 이동
            // Div가 visible 상태일때만 체크한다. 가려지면 스크린리더의 초점도 이동안함
			let _inner_focus = (inner_form && inner_form._p_components && inner_form._p_components.length > 0);
			if (_inner_focus && this._isVisible && this._isVisible() && this._p_visible)
            {   
...
if (nexacro._GridCellControl)
{
    ...
    {   
        ...
        if (!role)
			role = this._p_accessibilityrole;
...
{    
    ...
    {
        var cellobj = this._cellobj;
        if (this._displaymode && cellobj && !cellobj._isSummaryBand())
            return false;
        else
...
if (nexacro.Static)
{
    ...
    _pStatic._on_getAccessibilityAdditionalLabel = function ()
    {
        if (nexacro._accessibilitytype < 4 || this._p_usedecorate)
        {
            ...
            if (this._isAccessibilityRoleHeading() || this._p_usedecorate)
            {
                return "";
            }
        }
