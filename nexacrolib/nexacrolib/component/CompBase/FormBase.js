...
if (!nexacro.Form)
{
    ...
    _pTriggerManager._getTriggerView = function (obj, child_view)
    {
        ...
        var view_name = triggerview.split(".");
        if (child_view)
        {
    ...
    _pTriggerManager._notifyTrigger = function (triggertype, triggerobj, triggerview, evt)
    {
        ...
        var targettrigger = this._findTrigger(triggertype, triggerobj, triggerview);// 객체로 넘겨서 확인
            ...
            { // CYBIND_TYPE_SIMPLE
                if (comp._p_enable && comp._p_parent._isEnable())
                {
                    ...
                    else
                    {
                        ...
                        // enable 변경(false -> true) 시 focuspath에 존재하면 focus 처리
                        let _win = comp._getWindow();
                        if (_win && _win._indexOfCurrentFocusPaths(comp) > -1)
                        {
    ...
    _pFormBase._onHttpTransactionError = function (obj, bfireevent, errorobj, errortype, url, returncode, requesturi, locationuri, extramsg, svcid, responsebody)
    {
        var ret = false;
    ...
    _pFormBase._getComponentsByTaborder = function (p, filter_type, include_not_created, bAccessibility)//, arEdit)
    {
        ...
        if (comps)
        {
            ...
            {
                var comp = comps[i];
                if (!comp || (!include_not_created && !comp._is_created) || (comp._isVisible && !comp._isVisible() || !comp._p_visible) ||
                ...
                if (!(filter_type & 1))
                {
                    if (bAccessibility)
                    {
                        // container는 accessibilityenable이 false여도 내부 초점 진입 해야하므로 포함
						if (!comp._hasContainer())
						{
    ...
    _pFormBase._searchNextTabFocus = function (current, b_search_from_first, opt_force_cycle, filter_type)
    {
        ...
        else
        {
            ...
            if (!next)
            {
                // 텅빈 div인 경우 (container, innerform 구조)
                if (!nexacro._isNull(parent) && parent._hasContainer && parent._hasContainer() && !parent._is_form)
                {
                    ret = parent._searchNextTabFocus(this, false, opt_cycle, filter_type);
                }
            ...
            if (!ret)
            {
                ...
                if (!next)
                {
                    ...
                    else
                    {
                        // if (this._checkContainerTabFocus() == true)
                        //    ret = [this];
                        if (tabkeycirculation == 2 || tabkeycirculation == 3)
                        {
                            //처음일 경우
                            while (!next)
                            {
                                ret = parent._getPrevFrame(this);
                                if (ret && ret.form)
                                {       
                                    next = ret.form._getTabOrderLast(filter_type);
                                    parent =  ret.form.parent;
                                }
                                if (!ret)
                                    break;       
    ...
    _pForm.on_fire_user_onkeydown = function (keycode, alt_key, ctrl_key, shift_key, fire_comp, refer_comp, meta_key)
    {
            ...
            if (!dlgc.want_tab && this._isComponentKeydownAction()) // tab을 직접 처리하는 컴포넌트는 제외
            {
				...
				if (newfocus_comp)
				{
					if (newfocus_comp[0])
					{
                        ...
                        //스태틱 내부 다중리크 tabkey 브라우져 내부 처리 위해
                        if (newfocus_comp[0]._isFocusInLinkedComponent(shift_key))
                            keydown_elem._event_stop = false;
					}
        ...
        if (!this.onkeydown || (this.onkeydown && !this.onkeydown.defaultprevented))
        {
            ...
            if (this._isComponentKeydownAction() && (keycode == nexacro.Event.KEY_LEFT || keycode == nexacro.Event.KEY_RIGHT))
            {
                ...
                if (hscrollbar && hscrollbar._p_visible && (ctrl_key || (nexacro._enableaccessibility && !nexacro._accessibilitycontentsearchkey)))
                {
                    if (!dlgc.want_arrows) // arrowkey를 직접 처리하는 컴포넌트는 제외
                    {
                        ...
                        var hscroll_pos = hscrollbar.pos;
                        hscrollbar.set_pos(hscroll_pos + line);
                        return hscroll_pos == hscrollbar.pos ? false : true;
                    }
                }
            }
            else if (this._isComponentKeydownAction() && (keycode == nexacro.Event.KEY_UP || keycode == nexacro.Event.KEY_DOWN))
            {
                ...
                if (vscrollbar && vscrollbar._p_visible && (ctrl_key || (nexacro._enableaccessibility && !nexacro._accessibilitycontentsearchkey)))
                {
                    if (!dlgc.want_arrows) // arrowkey를 직접 처리하는 컴포넌트는 제외
                    {
                        ...
                        var vscroll_pos = vscrollbar.pos;
                        vscrollbar.set_pos(vscroll_pos + line);
                        return vscroll_pos == vscrollbar.pos ? false : true;
                    }
                }
            }
        }
        ...
        if (id != undefined)
        {
            ...
            if (datalistid.length > 0)
            {
                ...
                for (j = 0; datalistfilter.length > j; j++)
                {
                    ...
                    // Rp 37696 작업 이후 14부터 항상 이런 패턴으로 동작하여 가독성 오류 방지를 위해 수정 (104691)
    ...
    _pForm._setSize = function (width, height)
    {
        if (this._adjust_width != width || this._adjust_height != height)
        {
            ...
            if (layout_manger)
            {
                ...
                else if (this._adjust_width > 0 && this._adjust_height > 0)
                {
                    this._selectLayout();
                }
            }
    ...
    _pForm._getAutolayoutInfo = function (componentids)
    {
        ...
        {
            var comp = form_comps[ids[i]];
            if (!comp) return null;

            items.push({
                id: ids[i],
                rect: comp._getLayoutRect()
            });
        }

        if (items.length === 0) return null;

        return this._buildAutoLayoutInfoFromItems(items, true);
    };

    _pForm._sortLayoutItems = function (items, dir)
    {
        const ROW_THRESHOLD = 30; // 같은 row 판별 기준(px)
        const COL_THRESHOLD = 30; // 같은 column 판별 기준(px)

        // 가장 왼쪽 위에 있는 컴포넌트 찾기
        const topLeftItem = items.reduce((prev, curr) =>
            (curr.rect.left + curr.rect.top) < (prev.rect.left + prev.rect.top) ? curr : prev
        );

        const baseLeft = topLeftItem.rect.left;
        const baseTop = topLeftItem.rect.top;
        ...
        return items.slice().sort(function (a, b)
        {
            ...
            if (dir === "horizontal")
            {
                // 1. baseTop 기준으로 둘 다 같은 row면 left → right
                const aInBaseRow = Math.abs(ra.top - baseTop) <= ROW_THRESHOLD;
                const bInBaseRow = Math.abs(rb.top - baseTop) <= ROW_THRESHOLD;

                if (aInBaseRow && bInBaseRow)
                {
                    return ra.left - rb.left;
                }

                // 2. 같은 row끼리 비교 (baseTop 아닌 경우)
                if (Math.abs(ra.top - rb.top) <= ROW_THRESHOLD)
                {
                    return ra.left - rb.left;
                }

                // 3. row 다르면 top → bottom
                return ra.top - rb.top;
            }
            else // vertical
            {
                // 1. baseLeft 기준으로 둘 다 같은 column면 top → bottom
                const aInBaseCol = Math.abs(ra.left - baseLeft) <= COL_THRESHOLD;
                const bInBaseCol = Math.abs(rb.left - baseLeft) <= COL_THRESHOLD;

                if (aInBaseCol && bInBaseCol)
                {
                    return ra.top - rb.top;
                }

                // 2. 같은 column끼리 비교 (baseLeft 아닌 경우)
                if (Math.abs(ra.left - rb.left) <= COL_THRESHOLD)
                {
                    return ra.top - rb.top;
                }

                // 3. column 다르면 left + top 합산으로 정렬
                return (ra.left + ra.top) - (rb.left + rb.top);
            }
        });
    };
    /*
    _pForm._sortLayoutItems = function (items)
    {
        ...
        items.sort(function (a, b)
        {
            ...
            var bBottom = bTop + b.rect.height;

            var topDiff = aTop - bTop;

            //top 차이가 크면 무조건 top 우선
            var rowThreshold = Math.min(a.rect.height, b.rect.height) * rowRatio;
            if (Math.abs(topDiff) > rowThreshold)
            {
                return topDiff;
            }

            //top이 비슷할 때만 overlap 판단
            var isOverlap = !(aBottom <= bTop || bBottom <= aTop);
    ...
    */
    ...
    _pForm._getContainerAutoLayoutInfo = function ()
    {
        var form_comps = this._p_components;
        var items = [];

        for (var i = 0, len = form_comps.length; i < len; i++)
        {
            var comp = form_comps[i];
            if (!comp || comp._is_group) continue;
        ...
        return this._buildAutoLayoutInfoFromItems(items, false);
    };

    _pForm._buildAutoLayoutInfoFromItems = function (items, posInclude)
    {
        const rects = [];
        const ids = [];
        ...
        // 2. 방향 기준으로 정렬
        const sortedItems = this._sortLayoutItems(items, dir);
        ...
        {
            sortedRects.push(sortedItems[i].rect);
            sortedIds.push(sortedItems[i].id);
        }

        // 4. 실제 AutoLayout 계산
        return this._buildAutoLayoutInfo(sortedRects, sortedIds, posInclude);
    };

    _pForm._buildAutoLayoutInfo = function (rects, compids, posInclude)
    {
        if (!rects || rects.length === 0) return null;

        const pos = this._calculateContainerRect(rects, posInclude);
        const dir = this._getAutolayoutDirection(rects);
        ...
        const spacing = this._calculateContainerSpacing(rects, pos, dir, horizontalGap, verticalGap);
    ...
    _pForm._calculateContainerSpacing = function (rects, containerRect)
    {
        if (!rects || rects.length === 0 || !containerRect)
        ...
        // 컨테이너 기준 spacing
        const left = Math.max(0, firstRect.left - containerRect.left);
        const top = Math.max(0, firstRect.top - containerRect.top);

        // bottom = top, right = left
        return top + "px " + 0 + "px " + 0 + "px " + left + "px";
    };

    _pForm._getComponentInfo = function (idList, rects, rows, cols)
    {
        const result = [];
    ...
    _pForm._getGridAreaByPosition = function (rect,  rowLines, colLines)
    {
        // rect 중심점
    ...
    _pForm._getTableLayoutTemplate = function (rows, cols) 
    {
        const rowCount = rows.length;
        const colCount = cols.length;

        const rowTemplate = Array(rowCount).fill("1*").join(" ");
        const colTemplate = Array(colCount).fill("1*").join(" ");

        return `${rowTemplate} / ${colTemplate}`;
    };

    _pForm._calculateContainerRect = function (rects, posInclude) 
    {
        ...
        if (!posInclude) return { left: 0, top: 0, width: this._adjust_width, height: this._adjust_height };
        ...
        var _layouttype = this.getLayoutInfo("default", "type");
        if (_layouttype != "default")
            left = 0;
    ...
    _pForm._groupByLines = function (rects, axis, threshold)
    {
    ...
    _pForm._detectLayoutType = function (rects, mainAxis)
    {
    ...
    _pForm._getAutolayoutDirection = function (rects)
    {
    ...
    _pForm._calculateHorizontalGapFromRows = function (rows)
    {
    ...
    _pForm._calculateVerticalGapFromRows = function (rows)
    {
    ...
    *
    ...
    _pForm._calculateVerticalGapFromRows = function (rows)
    {
        ...
        for (let i = 1; i < rows.length; i++)
        {
            ...
            const prevBottom = Math.max(...prevRow.map(r => r.top + r.height));
            const currTop = Math.min(...currRow.map(r => r.top));
        ...
        _pInnerForm.addChild = function (id, obj)
        {
            ...
            if (obj._is_component)
            {
                ...
                if (nexacro._enableaccessibility && nexacro._accessibilitytype == 2)
                {
                    ...
                    if (this._p_parent)
                    {
                        this._p_parent._setAccessibilityStatHidden(false);
                    }
                } 
