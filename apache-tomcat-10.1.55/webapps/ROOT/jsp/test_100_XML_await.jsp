...
<%
    ...
    HttpPlatformResponse platformResponse = new HttpPlatformResponse(response, PlatformType.CONTENT_TYPE_XML);   
...
*/
	...
	//	outDataSet00.addConstantColumn("ConstColumn1", DataTypes.STRING, 255, "BCD");
	   ...
	   for(int i = 0; i < Integer.parseInt(ag1); i++)
	   {
			...
			{
				outDataSet00.set(newRow, "Column0", "XML");
				outDataSet00.set(newRow, "Column1", "test");
			}
			outDataSet00.set(newRow, "Column2", "^"+i+"^");
			outDataSet00.set(newRow, "useclientlayout_XML", "X"+i+"X");
		...
		*/
        ...
        outVariableList.add("ErrorMsg",  "조회 성공 await aync통신");
	...
	// 쿠키 지정 name, value
	Cookie cookie = new Cookie("Cookie001", "XML서버쿠키테스트!!!");
	// 쿠키의 유효시간 설정
	cookie.setMaxAge(10);
	//생성된 쿠기에 path설정
