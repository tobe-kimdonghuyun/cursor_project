...
if (nexacro._Browser == "Runtime")
{
    if (!nexacro._Init_systembase_runtime)
    {
        ...
        */
	    ...
	    {
            return function (status, data, fireerrorcode, returncode, locationurl, extramsg, responsebody)
            {
			    ...
			    if (status < 0)
			    {
                    ...
                    pthis.on_error(status, fireerrorcode, returncode, locationurl, extramsg, responsebody);
				    pthis = null;
			    }
			    else if (status == 0)
			    {
        ...
        nexacro.__bindLoadDataHandler = function (handlewrap, pthis)
		{
            ...
            return function (status, data, cookie, fireerrorcode, returncode, locationurl, extramsg, responsebody)
			{
				var _recieved_data;
			    if (status < 0)
			    {
...
// 	        }
        ...
        nexacro._convertWindowXYToCanvasXY = function(handle, windowX, windowY)
        {
            ...
            if (nexacro.__getElementHandleApplyScale)
            {
                var applyScale = nexacro.__getElementHandleApplyScale(handle);
                if (applyScale != 100)
                {
                    var scale = applyScale / 100;
                    return [(windowX - elem_pos[0]) / scale, (windowY - elem_pos[1]) / scale];
                }
            }
