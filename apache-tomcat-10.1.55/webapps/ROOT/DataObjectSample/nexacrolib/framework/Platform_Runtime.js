...
if (nexacro._Browser == "Runtime")
{
    if (!nexacro._init_platform_runtime)
    {
	    ...
	    nexacro._syshandler_onmousewheel_forward = function (_window, elem, wheelDelta, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY, metaKey)
		{
            ...
            if (nexacro._nCountAlertConfirm <= 0)
			{
                ...
                if (ret != false && ctrlKey && wheelDelta != 0)
                {
					...
					var zoomframeElement = _window.frame._is_popup_frame != true ? (_window.frame.frame ? _window.frame.frame.getElement() : null) : _window.frame.getElement();
					if (!zoomframeElement) return ret;    // _window.frame : mainframe
        ...
        */
        ...
        nexacro._createOpenWindowHandle = function (parent_win, name, formurl, left, top, width, height, resizable, layered, taskbar, is_main, parentframe, frameopener, arr_arg, ext_options, close_callback)
        {
        	...
        	if (popupframeoption)
        	{
				...
				popupframeoption._close_callback = close_callback;    		
        	}
