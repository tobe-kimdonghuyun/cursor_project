...
if (!nexacro.MaskEdit)
{
    ...
    _pMaskEdit._apply_setfocus = function (evt_name)
	{
        ...
        if (input_elem)
        {
            ...
            if (!this._onlydisplay)
            {
                ...
                if (nexacro._enableaccessibility)
                {
                    if (!this._p_autoselect && nexacro._Browser !== "Runtime")
                    {
    ...
    // 변경 기준값(_default_value, _default_text) 유지용 내부 함수. 속성 setter에서 표시 갱신 시 호출
    _pMaskEdit._on_apply_value = function ()
    {
        var saved_dv = this._default_value;
        var saved_dt = this._default_text;
        this.on_apply_value(this._p_value);
        this._default_value = saved_dv;
        this._default_text = saved_dt;
    };
