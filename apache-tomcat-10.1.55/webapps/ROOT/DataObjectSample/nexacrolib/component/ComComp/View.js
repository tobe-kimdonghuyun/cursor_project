...
if (!nexacro.View)
{
    ...
    _pView.on_notifytrigger = function (obj, info)
    {
        ...
        var manager;
        // Div url로 로드된 Form 내 View인 경우 refform의 trigger_manager 사용
        var refform = this._refform;
        if (refform && refform._trigger_manager)
        {
            manager = refform._trigger_manager;
        }
        else
        {
            // Div/Tabpage 내 View인 경우 (RP 104429) 상위 메인폼의 trigger_manager 사용
            var mainform = this._getMainForm();
            if (mainform)
                manager = mainform._trigger_manager;
        }

        if (manager)
        {
            manager._notifyTrigger(info.eventid, obj, this, info);
        }
    };

    _pView.on_apply_fittocontents = function ()
    {
