...
if (!nexacro.CellInfo)
{
    ...
    _pCellInfo._getDisplayText_localenumber = function (rowidx)
    {
        ...
        else if (typeof v === "string" && v !== "")
        {
            ...
            {
                return (new nexacro.Number(stripped)).toLocaleString(locale);
            }
        }
...
if (!nexacro._CellControl)
{
    ...
    _pCellControl._createDisplayer = function (displayType)
    {
        ...
        // summary 는 특정 타입을 editcontrol로 remapping
        var isSummary = this._isSummaryBand();
        var createType = displayType;
        if (isSummary)
        {
            switch (displayType)
            {
        ...
        switch (createType)
        {
            ...
            case "treeitemcontrol":
                ...
                break;
            case "combocontrol":
                this._createComboDisplayer(isSummary);
                break;
            case "multicombocontrol":
                this._createMultiComboDisplayer();
                break;
            case "calendarcontrol":
                this._createCalendarDisplayer(isSummary);
                break;
            case "maskeditcontrol":
                this._createMaskEditDisplayer();
                break;
            case "textareacontrol":
                this._createTextAreaDisplayer(isSummary);
                break;
            case "progressbarcontrol":
                this._createBarDisplayer();
                break;
            case "editcontrol":
                this._createEditDisplayer(isSummary);
                break;
        ...
        // listview 예외처리
        if (!isSummary && view && view._applyCellSelection)
    ...
    _pCellControl._updateDisplayer = function ()
    {
        ...
        var _is_displayer_changed = this._isChangeDisplayer(displayType);
        if (_is_displayer_changed)
        {
        ...
        if (fake && view)
        {
            if (view.fillareatype == "datarow")
            {
                if (!this._isDisplayDataType(displayType))
                {
                    ...
                    {
                        show = false;
                        if (fake != this._fakecell || view._resetfillarea || _is_displayer_changed)
                        {
    ...
    _pCellControl._createComboDisplayer = function (isSummary)
    {
        var control_elem = this._control_element;
        if (control_elem)
        {
            var rect = this._getControlRect();
            this._changeClientmode("normal");
            var controlComp = new nexacro[this._cellComboObj]("cellcombo", rect.left, rect.top, rect.width, rect.height, this, isSummary ? undefined : true);

            if (isSummary)
                this._prepareSummaryControl(controlComp);
    ...
    _pCellControl._createCalendarDisplayer = function (isSummary)
    {
        var control_elem = this._control_element;
        if (control_elem)
        {
            var rect = this._getControlRect();
            this._changeClientmode("normal");
            var controlComp = new nexacro[this._cellCalendarObj]("cellcalendar", rect.left, rect.top, rect.width, rect.height, this, isSummary ? undefined : true);
    ...
    _pCellControl._createEditDisplayer = function (isSummary)
    {
        var control_elem = this._control_element;
        if (control_elem)
        {
            var rect = this._getControlRect();
            this._changeClientmode("normal");
            var controlComp = new nexacro[this._cellEditObj]("celledit", rect.left, rect.top, rect.width, rect.height, this, isSummary ? undefined : true);
    ...
    _pCellControl._prepareSummaryControl = function (controlComp)
    {
        controlComp._displaymode = true;
		controlComp._apply_client_padding = true;
		controlComp._summarymode = true;
	};

    // 사용위치가 늘어 API 처리
	_pCellControl._isSummaryBand = function ()
	{
		return (this._band && this._band.id == "summary");
	};
    ...
    _pCellControl._createTextAreaDisplayer = function (isSummary)
    {
        var control_elem = this._control_element;
        if (control_elem)
        {
            var rect = this._getControlRect();
            this._changeClientmode("normal");
            var controlComp = new nexacro[this._cellTextAreaObj]("celltextarea", rect.left, rect.top, rect.width, rect.height, this, isSummary ? undefined : true);
    ...
    _pCellEdit.on_apply_readonly = function (breadonly)
    {
        nexacro.Edit.prototype.on_apply_readonly.call(this, breadonly);
    ...
    _pCellTextArea.on_apply_readonly = function (breadonly)
    {
        nexacro.TextArea.prototype.on_apply_readonly.call(this, breadonly);
        if (this._summarymode)
        {
            var input_elem = this._input_element;
            if (input_elem)
    ...
    {
        return this._summarymode || this._view._p_readonly;
    };
    ...
    _pCellCombo.on_apply_readonly = function ()
    {
        nexacro.Combo.prototype.on_apply_readonly.call(this);
        if (this._summarymode)
            this._changeStatus("readonly", false);
    };
    ...
    _pCellCheckboxBase._updateAll = function (onlycontents, breadonly)
    {
        ...
        // RP_105633 : cell 재활용 시 값/selected 상태(아이콘)를 먼저 적용한 뒤 크기를 측정해야 한다.
        this._setProperty();
        this._adjustAlign();
    };
    ...
    _pCellRadioItem._updateAll = function (onlycontents, breadonly)
    {
        ...
        // RP_105633 : checkbox와 동일하게 값/selected 상태를 먼저 적용한 뒤 아이콘 크기를 측정한다.
        this._setProperty();
        this._adjustAlign();
    };
