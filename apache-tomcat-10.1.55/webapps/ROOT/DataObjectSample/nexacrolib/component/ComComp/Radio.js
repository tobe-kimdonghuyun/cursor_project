...
if (!nexacro.Radio)
{
    ...
    _pRadio._getDlgCode = function (keycode, altKey, ctrlKey, shiftKey)
    {
        ...
        {
            // radio가 내부의 방향키를 제어하지 않는 케이스(accessibility only)
            // 1. itemenableaccessibility 속성이 false
            // 2. environment의 accessibilitycontentsearchkey 속성이 true            
            if (nexacro._enableaccessibility && (!this._isItemAccessibilityEnable() || nexacro._accessibilitycontentsearchkey))
            {
                _want_arrows = false;
            }
            else
	...
	_pRadio._on_focus = function (self_flag, evt_name, lose_focus, refer_lose_focus, new_focus, refer_new_focus)
	{
		...
		if (self_flag == false)
        {
            ...
            {
                // tab 키 입력시 아래와 같이 동작
                // index > -1 인 경우, index에 해당하는 라디오 아이템에 포커스                
                var index = this._p_index;                
                if (index == -1)
                {
                    // index == -1 인 경우, 첫번째 라디오 아이템에 포커스
                    index = 0;
                }
                ...
                if (item)
                {
                    if (focusdir == 0)
                    {
                        this._is_first_focus = true; // 정방향 진입만 체크
                    }
                    this._accessibility_index = index;
                    item._changeUserStatus("focused", true);
                    item._on_focus(true, evt_name); // 가상커서만 이동

                }
			}
			else if (focusdir == 2 || focusdir == 3) // down,up key (accessibility)
            {
                // 가상커서가 개입하면 focusin 에서 index 처리, radio에 초점 가지 않고 radioitem에 이동
                if (this._isComponentKeydownAction() && this._isItemAccessibilityEnable())
				{
					...
					{						
                        accIdx = 0;
                        this._is_first_focus = true; // 정방향 진입시에만 체크
					}
					else
					{
    ...
    _pRadio._on_getFitSize = function ()
    {
        ...
        if (control_elem)
        {
            ...
            {                
                ...
                {                    
                    var avail_width = (dir == "horizontal" && this._p_fittocontents == "height") ? this._getClientWidth() : -1;
                    var x = 0, y = 0, lineH = 0, maxW = 0;
                    ...
                    for (i = 0; i < item_len; i++)
                    {
                        ...
                        if (dir == "horizontal")
                        {
                            if (avail_width > 0 && x > 0 && x + item_size[0] > avail_width)
                            {
                                y += lineH;
                                x = lineH = 0;
                            }
                            x += item_size[0];
                            lineH = Math.max(lineH, item_size[1]);
                        }
                        else
                        {
                            y += item_size[1];
    ...
    _pRadio.on_keydown_default_action = function (keycode, alt_key, ctrl_key, shift_key, refer_comp)
    {
        ...
        {
            // focus 위치한 radioitem index 이동
            if (!this._p_readonly)
            {
                let acc_index = this._accessibility_index;
                if (acc_index > -1)
                {                    
                    this.set_index(acc_index);
                }
            }
        }
        else if (this._isComponentKeydownAction() && (keycode >= E.KEY_LEFT && keycode <= E.KEY_DOWN))
        {
            // radioitem 이동
            var ds = this._innerdataset;
            ...
            // -1일 때, accessibility_index에 값을 사용 (Tab 키 진입시 설정되도록 함)
            var idx = (this._p_index == -1) ? this._accessibility_index + op : this._p_index + op;
            ...
            if (ds && item_len)
            {
                ...
                if (radio_columncount == -1 && radio_rowcount == -1)
                {
                    ...
                    for (i = 0; i < item_len; i++)
                    {
                        ...
                        if (apply_colcnt > 0 && sum_width + item_size[0] > client_width)
                        {
                            row_layout.push(current_row_items);
                            row_heights.push(current_row_max_h);
                            apply_rowcnt++;
                            sum_width = item_size[0];
                            apply_colcnt = 1;
                            current_row_items = 1;
                            current_row_max_h = item_size[1];
                        }
                        else
                        {
                            sum_width += item_size[0];
                            apply_colcnt++;
                            current_row_items++;
                            if (current_row_max_h < item_size[1]) current_row_max_h = item_size[1];
                    ...
                    {
                        item_left = 0;
                        var col_count = row_layout[i];
                        // itemheight의 계산은 fittotocontents가 height일때 필요
                        item_height = (fittocontents == "height") ? row_heights[i] : client_height / apply_rowcnt;
                        for (j = 0;j < col_count;j++)
                        {
                            ...
                            var place_width = Math.min(item_size[0], client_width - item_left);
                            item.move(item_left, item_top, place_width, item_height);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            for (var fi = 0; fi < max_rowsize.length; fi++) total_placed_height += max_rowsize[fi];
                        }
                    }
                }
            }

            if ((fittocontents == "height" || fittocontents == "both") && total_placed_height != client_height)
            {
                this._update_position();
            }
        }
