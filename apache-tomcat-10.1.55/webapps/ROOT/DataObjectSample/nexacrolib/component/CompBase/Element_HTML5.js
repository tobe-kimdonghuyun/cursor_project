...
if (nexacro._Browser != "Runtime" && !nexacro.Element)
{
    ...
    */
    ...
    _pInputElement._go_next_focus = function ()
    {
        ...
        if (input_handle && comp)
        {
            ...
            if (newfocus_comp && newfocus_comp[0])
            {
                ...
                //if (!(nexacro._OS == "iOS" && nexacro._enableaccessibility) && newfocus_comp[0]._isEditableComponent())
                //{
    ...
    _pInputElement.on_sys_touchstart_process = function (evt)
    {
        ...
        // RP 105774 : userzoom 미지원 상태에서만 멀티터치 차단
        ...
        if (input_handle)
        {
            ...
            if (pThis)
            {
                ...
                if (bResetPos)
                {
                    ...
                    var dragscrolltype = comp.dragscrolltype;

                    if (hscroll_pos != input_handle.scrollLeft)
                    {
                        if (scrolltype != "none" && scrolltype != "vertical" && !comp._is_tracking && (bWheel || dragscrolltype != "none"))
                        {
                            if (pThis.rtl)
                            {
                    ...
                    {
                        if (scrolltype != "none" && scrolltype != "horizontal" && !comp._is_tracking && (bWheel || dragscrolltype != "none"))
                        {
    ...
    _pControlElement.createCommandStart = function ()
    {
        if (!this._is_popup)
        {
			...
			if (owner_elem && !this.handle)
			{
                ...
                {
                    str += " title = '" + nexacro._encodeXml(this.tooltiptext) + "' ";
                }
    ...
    _pFrameControlElement.create = function (win)
    {
        ...
        if (!this.handle)
        {
            if (this.parent_elem == null)
            {
                ...
                // Android는 브라우저가 scroll시 touch 개입을 막음 (pinch-zoom 은 허용)
                if (nexacro._OS == "Android")
                {
                    nexacro.__setDOMStyle_TouchAction(handle_style, "pinch-zoom");
                }
...
//	        if (this.left || this.top)
    ...
    __pWebBrowserPluginElement._setElementLeaveMessage = function (message)
    {
        ...
        if (next_outfocus_elem)
        {
            ...
            var is_hidden = (typeof message === "string" && message.length > 0) ? false : true;
    ...
    */
    ...
    __pWebViewPluginElement._on_message_handler = null;
	...
	__pWebViewPluginElement._on_frame_load = function (_win)
	{
		...
		{
            // 등록 시점 변경
            if (!this._on_message_handler)
            {
                this._on_message_handler = function (evt)
                {
                    if (!evt) return;
                    var handle = this.handle;
                    if (handle && handle.contentWindow && evt.source === handle.contentWindow)
                    {
                        var c = this.parent_elem && this.parent_elem.linkedcontrol
                        if (c && c._on_webview_message)
                        {
                            c._on_webview_message(evt);
                        }
                    }
                }.bind(this); // 함수 내부에 scope가 this로 유지됨
                try
                {
                    nexacro._observeSysEvent(window, "message", "onmessage", this._on_message_handler);
                }
                catch (e)
                {
                    nexacro._settracemsg(e);
	...
	__pWebViewPluginElement.destroy = function ()
	{
        if (this._on_message_handler)
        {
            try
            {
                nexacro._stopSysObserving(window, "message", "onmessage", this._on_message_handler);
            }
            catch (e)
            {
                nexacro._settracemsg(e);
            }
            this._on_message_handler = null;
        }
	...
	*/
    ...
    {
        var handle = this.handle;
        if (handle && handle.contentWindow && handle.contentWindow.postMessage)
        {
            try
            {
                handle.contentWindow.postMessage(message, targetOrigin || "*");
                return true;
            }
            catch (e)
            {
    ...
    * position 이동  : 터치시 화면반응속도 중간, 스크롤속도 중간, 화면출렁거림 거의없음 (노드량이 많으면 느려질 수 있음)
    ...
    nexacro.GridRowControlElement = function (parent_elem, left_gap)
    {
        ...
        if (nexacro._enableaccessibility && nexacro._accessibilitytype > 3)
            ...
            use_inner_move = (nexacro._Browser == "Safari" || nexacro._Browser == "MobileSafari");
