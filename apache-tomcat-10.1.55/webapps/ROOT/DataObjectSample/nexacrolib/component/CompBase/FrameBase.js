...
if (!nexacro.Frame)
{
    ...
    {
        if (nexacro._isHybrid && nexacro._isHybrid() && nexacro._OS == "Android" && titlebarheight == "")  // titleBarHeight를 지정하지 않은 경우 Default값으로 설정.
        {
            this._p_titlebarheight = this._defaulttitleheight;
            this._titlebarheight = this._getAppliedTitleHeight(this._defaulttitleheight);
    ...
    {
        if (nexacro._isHybrid && nexacro._isHybrid() && nexacro._OS == "Android" && statusbarheight == "")  // StatusBarHeight를 지정하지 않은 경우 Default값으로 설정.
        {
    ...
    _pFrame.set_titlebarbuttongap = function (titlebarbuttongap) // check
    {
    ...
    _pFrame.setFocus = function (bResetScroll, bInnerFocus)
    {
        ...
        else
        {
            if (this._active_frame)
                this._active_frame.setFocus(bResetScroll, bInnerFocus);
            else if (this._p_form)
                this._p_form.setFocus(bResetScroll, bInnerFocus);
        }
    };

    _pFrame._canInnerFocus = function ()
    {
        var frame = this;
        while (frame._active_frame)
            frame = frame._active_frame;

        var form = frame._p_form;
        if (!form || !form.getElement())
            return false;

        var win = this._getRootWindow();
        if (!win || !win._isActiveLayerComponent(form))
            return false;
    ...
    _pFrame._properties = [{ name: "left" }, { name: "top" }, { name: "colcount" , readonly : true},
            ...
            { name: "height" }, { name: "openstatus" }, { name: "progressbardirection"}, { name: "progressbargap"}, { name: "progressbarsize"}, { name: "progressbarsmooth"},
            { name: "showcascadestatustext"}, { name: "showcascadetitletext" }, { name: "showstatusbar" }, { name: "showtitlebar"},
    ...
    _pFrame.removeChild = function (id)
    {
        ...
        if (obj)
        {
            ...
            if (nextframe && nextframe._canInnerFocus())
                nextframe._on_focus(true);
    ...
    _pFrame._on_titlebar_movetrack = function (x, y, dragdata, windowX, windowY)
    {
        ...
        if (!this._is_window && (windowX != undefined && windowY != undefined))
        {
            ...
            mainframe = mainframe ? mainframe : nexacro.getApplication()._p_mainframe;
            var mainframe_left = nexacro._toInt(mainframe._p_left);
            var mainframe_top = nexacro._toInt(mainframe._p_left);
    ...
    _pFrame._changeOpenStatus = function (cur)
    {
        ...
        if (pre != cur)
        {
            ...
            if (cur == 3 && !this._is_window)
            {
                ...
                if (owner_frame && owner_frame._is_frameset == true && this._canInnerFocus())
                    this._on_focus(true);
            }
    ...
    _pMainFrame.createWindow = function ()
    {
        ...
        _win.create(null, this._p_name, this._width, this._height, this._left, this._top, this._p_resizable);
    ...
    _pMainFrame._getNextFrame = function (cur) 
    {
        var frames = this._getFrames();
        var cur_idx = nexacro._indexOf(frames, cur);
    ...
    _pMainFrame._getPrevFrame = function (cur) 
    {
        var frames = this._getFrames();
        var cur_idx = nexacro._indexOf(frames, cur);
        if (cur_idx < 0) return null;
    ...
    _pChildFrame._properties = [{ name: "overlaycolor" }, { name: "autosize" }, { name: "dragmovetype"},
        { name: "openalign" }, { name: "formurl" }, { name: "resizable" }, { name: "layered" }, { name: "showontaskbar" }, { name: "form", readonly: true }      
    ];
    ...
    _pChildFrame.showModal = function (str_id, _parent_frame, arr_arg, opener, callback, is_async)
    {
        ...
        if (parent_frame == null)
        {
            var application = nexacro.getApplication();
            if (application)
                parent_frame = application._p_mainframe;
        }
        if (parent_frame)
        {
        ...
        if (!this.opener || (this.opener && !this.opener._is_form && !this.opener._is_application))
        {
            ...
            if (_focus_obj)
            {
                if (_focus_obj._p_parent._is_form)
                    this.opener = _focus_obj._p_parent;
                else
                    this.opener = parent_frame._p_form;
            }
            else
            {
        ...
        var recalculated_pos = this._recalcModalPosition((this._p_left == null) ? null : this._p_left, (this._p_top == null) ? null : this._p_top, this._p_width, this._p_height);
    ...
    _pChildFrame._showModalWindow = function (str_id, _parent_frame, arr_arg, opener, _lockmode)
    {
        ...
        */
        ...
        if (this._p_form && this._p_form._load_manager.status >= 7)
        {
            this._loadedForm();
        }
        ...
        var rv = this._window.createModal(parent_window, this.id, width, height, left, top, this._p_resizable, this._p_layered, lockmode);
    ...
    _pChildFrame._loadTheme = function (themeid)
    {
        ...
        //for info
        cssurl = nexacro._getMapFileName(nexacro._theme_uri + "/theme");
        cssurl = nexacro._getServiceLocation(cssurl, base_url);
        ...
        if (this._p_autosize)
        {
            ...
            if (this._is_window && this._window_type == 2) // 이제 Modeless 밖에 해당사항이 없음.
            {
                if (this._delayed_create_window)
                {
                    ...
                    _window.create(parent_window, this.id, width, height, left, top, this._p_resizable, this._p_layered, this._p_showontaskbar);
    ...
    _pChildFrame._setModalLock = function (modalWindowOverlayColor)
    {
        ...
        var target_win = win; // overlay
        if (parent)
        {
            target_win = parent._getWindow();
        }

        // calc zindex
        var zindex = nexacro._zindex_firstmodal;
        var modal_stack = target_win._modal_frame_stack;
        ...
        */
        ...
        if(parent)
        {
            if (this._window_type == 5)
            {
                ...
                if (overlaycolor)
                    modal_overlay_elem.setElementBackground(overlaycolor);
                modal_overlay_elem.create(target_win);

                parent._accessibilityModalLock(modal_stack);
                target_win._modal_frame_stack.push([this, zindex, cur_focus_path]);
            }
            else
            {
                modal_overlay_elem = this._modal_overlay_elem = new nexacro.ModalOverlayElement(parent._control_element);
    ...
    */
    ...
    _pFrameSetBase._properties = [{ name: "ctrltaborder", readonly : true }, { name: "ctrltabtype" ,  readonly : true}, { name: "tabkeycirculation",  readonly : true},
    { name: "separatesize"}, { name: "keepseparatesizewhenswap" }, { name: "minimizedchildposition" }]; 
    ...
    _pFrameSetBase._getNextFrame = function (id, cycle)
    {
        ...
        if (frames)
        {
            // 인자로 받는 아이디부터 시작하도록 수정 처음부터 돌면 넘어가지 않음
            var idx = frames._idxMap[id] > -1 && frames._idxMap[id] != n-1 ? frames._idxMap[id] : 0;
            for (idx; idx < n; idx++)
            {
                if (frames[idx]._is_frameset)
                {
                    if (id != frames[idx].id)
                    {
                        if (frames[idx]._isFocusAcceptable())
                        {
                            frame = frames[idx]._getNextChildFrame(frames[idx]._p_frames);
                            break;
                        }
                    }
                }
                else if (frames[idx] instanceof nexacro.ChildFrame)
                {
                    if (frames[idx]._isFocusAcceptable() && frames[idx]._p_formurl)
                    {
                        frame = this._getFocusChildFrame(frames[idx]);
                        break;
                    }
                    else
                    {
        ...
        if (this._p_parent._is_frameset)
            return this._p_parent._getNextFrame(this.id, cycle);
        else
        {
    ...
    _pFrameSetBase._getPrevFrame_ = function (id, cycle)
    {
        ...
        var i = n -1;
         for (var j = 1; j < n; j++)
            {
                if (frames[j].id == id)// && !this._p_parent._is_main)
                {
                    i = j;
                    //if (i==0)
                    //    frame = this._p_parent._getPrevFrame_(this._p_parent.id,cycle);
                    break;
                }   
        ...
        {
            if (this._p_parent._is_frameset)
                return this._p_parent._getPrevFrame_(this.id, cycle);
            else
            {
                // mainframe에 달린 focus가능한 frame으로 이동
                if (cycle)
    ...
    */
    ...
    _pFrameSet._properties = [{ name: "tabkeycirculation"}, { name: "minimizewidth"}, { name: "minimizeheight"},
