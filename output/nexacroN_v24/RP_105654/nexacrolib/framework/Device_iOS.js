...
//Only iOS
//==================================================================
if ((nexacro._OS == "iOS" || nexacro._linux_app)&& (window.location.pathname.split("/").pop() == "Run.html" || window.location.pathname.split("/").pop() == "nexacro.html" || window.location.pathname.split("/").pop() == "popup.html"))
{
    ...
    nexacro.DeviceI.prototype.exec = function (method, sync)
    {
		...
		{
			// Linux인 경우 webviewId를 받아와서 json에 추가한다.
			var json = {
				id: "0000",
				div: "NexacroWindow",
				method: "_getWebViewId",
				params: {}
			};

			var webviewId = prompt("nexacro", JSON.stringify(json));
		
			var jsonObj;
			try 
			{
				jsonObj = JSON.parse(method);
			} 
			catch (e) 
			{
				// If jsonstr is not valid JSON
				return;
			}
			jsonObj.webviewId = webviewId;
			var modifiedJsonStr = JSON.stringify(jsonObj);

			if (!sync)
				window.webkit.messageHandlers.nexacro.postMessage(modifiedJsonStr);
			else
				return prompt("nexacro", modifiedJsonStr);
    ...
    nexacro._execBrowser = function(strUrl)
    {
        ...
        var deviceId = nexacro._linux_app ? "0000" : nexacro.Device.makeID();
