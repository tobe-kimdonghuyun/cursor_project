...
if (!nexacro.Component)
{
    ...
    _pComponent._use_translate_scroll = true; // 전역 nexacro를 사용하지 않고 component default로 처리 (IE 빠지면서 전역으로 필요없음)
    ...
    _pComponent._initUniqueID = function ()
    {
        if (this._unique_id.length <= 0)
        {
            ...
            this._unique_id = _p && _p._unique_id ? (_p._unique_id + "." + this.id) : (this.id ? this.id : "");
        }
    };
    ...
    {
        //fluid
        //if (this._isFluidContainer())//for MLM
        //{
        var control_elem = this.getElement();
        if (control_elem)
        {
    ...
    _pComponent._properties = [{ name: "expr" }, { name: "cssclass" }, { name: "visible" }, { name: "enable" }, { name: "taborder" }, { name: "tabstop" }, { name: "enableredraw" }, { name: "enableevent" }, { name: "tooltiptype" }, { name: "tooltiptext" }, { name: "fittocontents" }, { name: "hotkey" }, { name: "rtl" }, { name: "positionstep" }, { name: "scrollbartype" }, { name: "scrolltype" }, { name: "scrollbarsize" }, { name: "scrollindicatorsize" }, { name: "scrollbarbarminsize" }, { name: "scrollbarbaroutsize" }, { name: "scrollbardecbuttonsize" }, { name: "scrollbarincbuttonsize" }, { name: "scrollbartrackbarsize" }, { name: "left" }, { name: "top" }, { name: "right" }, { name: "bottom" }, { name: "width" }, { name: "height" }, { name: "minwidth" }, { name: "maxwidth" }, { name: "minheight" }, { name: "maxheight" }, { name: "color" }, { name: "font" }, { name: "wordSpacing" }, { name: "letterSpacing" }, { name: "textDecoration" }, { name: "wordWrap" }, { name: "borderRadius" },
    ...
    _pComponent.beginTransitionEffect = function (effectObj, callback)
    {
        var control_elem = this._control_element;
        if (control_elem)
        {
            control_elem.beginTransitionEffect(effectObj, callback);
        }
    };
    ...
    _pComponent._getSizeInfo = function ()
    {
        ...
        {
            width = this._clampToNonNegative(fit_size[0]);

            // fittocontents 결과가 0이고, 원래 % width가 있으면 fallback
            if (width <= 0 && this._p_width
                && typeof this._p_width === "string"
                && this._p_width.indexOf("%") > -1)
            {
                ...
                if (containerComp)
                {
                    ...
                    if (ref > 0)
                    {
                        width = this._convToPixel(this._p_width, ref);
                    }
                }
            }
        ...
        if (fit_type === "height" || fit_type === "both")
        {
            ...
            if (height <= 0 && this._p_height
                && typeof this._p_height === "string"
                && this._p_height.indexOf("%") > -1)
            {
                var containerComp = this._getParentComponent();
                if (containerComp)
                {
                    ...
                    if (ref > 0)
                    {
                        height = this._convToPixel(this._p_height, ref);
                    }
                }
            }
        }
    ...
    _pComponent._isFocusInLinkedComponent = function ()
    {
        return false;
    }
    ...
    _pComponent._isAccessibilityRoleHeading = function () {return false;};
    ...
    */
    ...
    _pComponent._setControlElementCssSelector = function ()
    {
        ...
        if (control_elem)
        {
            ...
            if (this._is_subcontrol)
            {
                ...
                var _p = this._p_parent;
                if (_p && !_p._is_subcontrol && (_p._is_containerset || _p._is_form || _p._is_frame))
                {
    ...
    _pComponent._getReferenceAbsoluteFont = function (target_font)
    {
        ...
        if (!font)
        {
            if (this._is_main || this._is_popup_frame)
            {
    ...
    _pComponent._parseArrangeVal = function (val)
    {
        ...
        if (val && typeof val == "string")
        {
            ...
            else if (is_relative_coordinate)
            {
                if (!isNaN(parseFloat(val)))
                    info = { compid: null, distance: val };
            }
    ...
    _pComponent._calcArrangePosition = function ()
    {
        ...
        else
        {
            ...
            else
            {
                if (this._p_width != null)
                {
                    if (this._p_width && typeof this._p_width == "string" && this._p_width.indexOf("%") > -1)
                    {
                        this._width = isNaN(parsePosition = this._convToPixel(this._p_width, this._basis_width)) ? this._width : parsePosition;
                    }
                    else
                    {
        ...
        else
        {
            ...
            else
            {
                if (this._p_height != null)
                {
                    if (this._p_height && typeof this._p_height == "string" && this._p_height.indexOf("%") > -1)
                    {
                        this._height = isNaN(parsePosition = this._convToPixel(this._p_height, this._basis_height)) ? this._height : parsePosition;
                    }
                    else
                    {
    ...
    */
    ...
    _pComponent._onResetScrollBar = function ()
    {
        ...
        if (control_elem)
        {
            ...
            //if (hscrollbartype == "fixed" || (hscrollbartype != "none" && control_elem.hscroll_limit > 0))
            if(hscrollbartype != "none")
                bcreatehscroll = true;
            //if (vscrollbartype == "fixed" || (vscrollbartype != "none" && control_elem.vscroll_limit > 0))
    ...
    _pComponent._scrollTo = function (hpos, vpos, bapplyscrollbar, bautohidetimer, evttype, evtkind)
    {
        ...
        if (bchange_hscroll_pos || bchange_vscroll_pos)
        {
            ...
            if (focused_comp && focused_comp._isEditableComponent())
            {
    ...
    */
    ...
    _pComponent._isVisible = function ()
    {
        ...
        while (form != null)
        {
            if (form._p_visible == false && !form._isNotProcessedWheelZoom)
                return false;
            if (form._is_window)
                break;
    ...
    _pComponent._isEditableComponent = function (edittype)
    {
        ...
        // 이전 코드에서 taborder 검색시 하위 component의 type 별 검색을 위한 코드로 현재 N에서는 사용하지 않아 주석 처리
    ...
    */
	...
	{
		v = v !== undefined ? this._p_usesoftkeyboard : nexacro._usesoftkeyboard;
		this._use_softkeyboard = nexacro._enabletouchsoftkeyboard ? (v ? 2 : 0) : 1;
		return v;
	};
    ...
    _pComponent._clearMouseoverStatus = function ()
    {
        this._changeStatus("mouseover", false);
        if (this._is_subcontrol)
        {
            var p = this.parent;
            if (p)
                p._clearMouseoverStatus();
        }
    };

    _pComponent._getLayoutRect = function ()
    {
        var rect = {};
        rect.left = this._adjust_left;//comp._left;
        rect.top = this._adjust_top; //comp._top;
        rect.width = this._adjust_width; //comp._width;
        rect.height = this._adjust_height; ////comp._height;
        return rect;
    };
