...
if (!nexacro.ListBox)
{
    ...
    _pListBox._itemheight = undefined;
    ...
    _pListBox.on_attach_contents_handle = function (win, enablea11y)
    {
        var items = this._getContentsItem();
        var item_len = items.length;
        for (var i = 0; i < item_len; i++)
    ...
    {
        // setElementScrollMaxSize의 동기 리사이즈 이벤트로 재진입 시 콜스택 오버플로우 방지
    ...
    _pListBox._on_item_onlbuttondown = function (obj, e)
    {
        ...
        if (!(env._p_enabletouchevent && this._selectscrollmode == "scroll"))
        {
    ...
    {
        // _createListBoxContents의 DOM 이벤트로 재진입 시 콜스택 오버플로우 방지
        if (this._in_redraw_contents) return;
        this._in_redraw_contents = true;
        ...
        this._in_redraw_contents = false;
    };
    ...
    {
        var lastidx = this._select_multi.lastselected == null ? this._getEnvironment()?._p_enableaccessibility ? this._accessibility_index : this._select_multi.lastselected : this._select_multi.lastselected;
