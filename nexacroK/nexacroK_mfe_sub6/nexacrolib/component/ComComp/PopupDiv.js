...
if (!nexacro.PopupDiv)
{
    ...
    _pPopupDiv._trackPromise = null;
    _pPopupDiv._trackResolve = null;
    ...
    _pPopupDiv._is_navtarget_component = false;
    ...
    _pPopupDiv.on_attach_contents_handle = function (win, enablea11y)
    {
        // 기본작업
    ...
    {        
        if (this._p_visible || !this._isAllowDefaultPinchZoom())
        {
            var control_elem = this._control_element;
            if (control_elem)
            {
    ...
    _pPopupDiv._clearEventListener = function (evt_id)
    {
        ...
        const root = this._getRootObject(); //chk
        ...
        if (this._is_trackpopup) 
        {
         ...
         this._trackPromise = new Promise((resolve, reject) => {
            try {
                ...
                try 
                {
                    var layoutmanager = env._getLayoutManager();
                    ...
                    if (win)
                    {
                        ...
                        frame_h = nexacro._getWindowHandleClientHeight(win);
                    }
    ...
    //PopupDiv 가 popup 상태일때는 parent 가 scroll 되지 않음
    _pPopupDiv.on_fire_sys_onmousewheel = function (wheelDeltaX, wheelDeltaY, button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, meta_key)
    {
