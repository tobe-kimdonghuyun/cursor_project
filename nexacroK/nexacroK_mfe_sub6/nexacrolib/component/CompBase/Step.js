...
if (!nexacro.StepControl)
{
    ...
    _pEventStepChangeEventInfo = null;
    ...
    _pEventStepMouseEventInfo = null;
    ...
    /* default properties */
    _pStepControl._p_stepcount = 0;
    _pStepControl._p_stepindex = 0;
    _pStepControl._p_stepitemsize = undefined;
    _pStepControl._p_stepitemgap = undefined;
    _pStepControl._stepitemsize = undefined;
    _pStepControl._stepitemgap = undefined;
    ...
    _pStepControl.on_create_contents = function ()
    {
        ...
        if (control_elem)
        {
            ...
            var stepButton;
            var stepcount = this._p_stepcount;
    ...
    _pStepControl.on_created_contents = function (/*win*/)
    {
        ...
        if (img)
        {
            ...
            var iconurl = nexacro._getURIValue(img.url);
            iconurl = this._getEnvironment()?._getImageLocation(iconurl, this._getRefFormBaseUrl());
            var size = nexacro._getImageSize_extended(iconurl, this._on_icon_onload, this);
            if (size)
            {
    ...
    {
        return this._p_stepindex + " " + this._p_stepcount;
    };
    ...
    {
        this._stepitemsize = nexacro._isEmRemUnit(this._p_stepitemsize) ? this._resolveEmPx(this._p_stepitemsize) : undefined;
        var control_elem = this.getElement();
        if (control_elem)
        {
            this._redrawStepButton();
    ...
    {
        this._stepitemgap = nexacro._isEmRemUnit(this._p_stepitemgap) ? this._resolveEmPx(this._p_stepitemgap) : undefined;
        var control_elem = this.getElement();
        if (control_elem)
        {
            this._redrawStepButton();
    ...
    _pStepControl.set_stepcount = function (v)
    {
        ...
        this._prestepindex = this._p_stepindex;
        this._prestepcount = this._p_stepcount;

        this._poststepcount = v;

        if (this._p_stepcount != v)
        {
            this._p_stepcount = v;

            if (this._p_stepindex >= v)
                this._p_stepindex = v - 1;
    ...
    _pStepControl.set_stepindex = function (v)
    {
        ...
        if (this._p_stepindex != v)
        {
            var check_count = this._p_stepcount <= v || v < 0;
            if (!check_count)
            {
                this._prestepindex = this._p_stepindex;
                ...
                if (form)
                {
                    ...
                    if (form_control_elem)
                    {
                        ...
                        for (var i = 0; i < comp_len; i++)
                        {
                            ...
                            {
                                before_comp = comp;
                                before_elem = form_control_elem.getContainerElement(before_comp._p_positionstep);
                            }

                            if (before_comp && before_comp != comp)
                            {
                                cur_elem = form_control_elem.getContainerElement(comp._p_positionstep);
    ...
    _pStepControl._properties = [{ name: "stepitemsize" }, { name: "stepitemgap" },
    { name: "stepcount" }, { name: "stepindex" }
    ];
    nexacro._defineProperties(_pStepControl, _pStepControl._properties);
    ...
    {
        return this._p_stepcount;
    };

    _pStepControl.stepIt = function (forward, rotate)
    {
        var stepindex = this._p_stepindex;
        var max_step = this._p_stepcount;
    ...
    {
        var stepitemsize = (this._stepitemsize !== undefined) ? this._stepitemsize : this._p_stepitemsize;
    ...
    {
        var v = (this._stepitemgap !== undefined) ? this._stepitemgap : this._p_stepitemgap;
        return v ? v : 3;
    };
