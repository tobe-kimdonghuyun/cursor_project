...
if (!nexacro.DeviceI)
{
	...
	*/
	...
	{
		const jsonStr = nexacro.Device.DatasetToJSONString(dataset);
		return nexacro._parseJsonLiteral(jsonStr);
	};
	...
	_pDeviceI.JSONStringToDataset = function (jsonString, dataset)
	{
		...
		return nexacro.Device.JSONObjectToDataset(jsonObj);
	};
	...
	{
		const jsonStr = nexacro.Device.DatasetToJSONString2(dataset);
		return nexacro._parseJsonLiteral(jsonStr);
	};
	...
	_pDeviceI.JSONStringToDataset2 = function (jsonString, dataset)
	{
		...
		const jsonObj = nexacro._parseJsonLiteral(jsonString);
		return nexacro.Device.JSONObjectToDataset2(jsonObj, dataset);
	};
...
if (nexacro.System)
{
	...
	nexacro.System.getPackageVersion = function ()
	{
		...
		if (nexacro._OS == "Android" && nexacro._isHybrid && nexacro._isHybrid())
		{
			...
			var jsonstr = JSON.stringify(json);
			const response = nexacro.Device.exec(jsonstr);
			const result = nexacro._parseJsonLiteral(response);
...
if (nexacro.ProtocolAdp)
{
	nexacro.ProtocolAdp.isRuntimeProtocol = function (protocol)
	{
		...
		{
			result = nexacro._parseJsonLiteral(result);
		}
	...
	nexacro.ProtocolAdp.setProtocolVar = function (name, key, val, env)
	{
		...
		if (!protocoladp)
		{
			/** Environment._getProtocol()에서 처리
