...
if (!nexacro.Dataset) 
{
        ...
        if (this._p_dataobjectbindmode == "twoway")
        {
            ...
            if (infoarray)
            {
                for (var i = 0, len = infoarray.length; i < len; i++)
                {
                    ...
                    if (index == -1 && infoarray[i].type != "deleteAll")
                    {
                        bReload = true;
                        break;
                    }
    ...
    */
    ...
    _pDataset._append = function (svcurl, async, datatype)
    {
        ...
        {
            var env = this._p_parent._getEnvironment();
            var url = env?._getServiceLocation(serviceurl);
            if (url && url.length)
            {
        ...
        if (reason == 0 || reason == 3)
        {
            ...
            if (view && view._is_view)
            {
                if (this._p_name == view._p_viewdataset)
                {
                    ...
                    var evt = new nexacro.EventInfo(this, "onmodelloadsuccess");
                    manager._notifyTrigger("onmodelloadsuccess", triggerobj, triggerview, evt);
                }
            }
        }
    };
