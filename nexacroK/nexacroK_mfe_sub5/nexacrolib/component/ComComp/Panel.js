...
if (!nexacro.Panel)
{
    ...
    _pPanel.on_attach_contents_handle = function (win, enablea11y)
    {
        if (!this._p_visible)
            this._applyElementVisible(this._p_visible);
    };
    ...
    _pPanel.on_apply_font = function (font)
    {
        nexacro.Component.prototype.on_apply_font.call(this, font);

        var v;
        v = this._p_spacing;
        if (v && this._spacing && this._spacing._hasRelative)
            this.on_apply_spacing();

        v = this._p_horizontalgap;
        if (v !== undefined && nexacro._isEmRemUnit(v))
            this.on_apply_horizontalgap();

        v = this._p_verticalgap;
        if (v !== undefined && nexacro._isEmRemUnit(v))
            this.on_apply_verticalgap();
    };

    _pPanel._reapplyRemValues = function ()
    {
        ...
        this._reapplyItemRemValues();
    };

    _pPanel._reapplyItemRemValues = function ()
    {
        var items = this._p_items;
        if (!items) return;
        for (var i = 0, n = items.length; i < n; i++)
        {
            if (items[i] && items[i]._component)
                items[i]._component._reapplyRemValues();
        }
    };
    ...
    */
    ...
    _pPanel.removeItem = function (itemid)
    {
        var items = this._p_items;
        if (items)
        {
    ...
    {
        var obj = new nexacro.Layout("default", "", 0, 0, this, function (p) { });
        obj.set_type(this._p_type);
        if (this._p_tabletemplatearea)
            obj.set_tabletemplatearea(this._p_tabletemplatearea);
    ...
    _pPanel._updateMinSize = function (current_min, value)
    {
        if (current_min == null) return current_min;
        return value > current_min ? value : current_min;
    };

    _pPanel._adjustWidthAxis = function (width, minwidth, parent_fittocontents)
    {
    ...
    _pPanel._adjustHeightAxis = function (height, minheight, parent_fittocontents)
    {
...
if (!nexacro.PanelItem)
{
    ...
    _pPanelItem._attachPanel = function (idx)
    {
        ...
        if (control_elem)
        {
            const env = _form._getEnvironment();
            var manager = env._getLayoutManager();
