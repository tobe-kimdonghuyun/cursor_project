...
if (!nexacro.ProgressBar)
{
    ...
    _pProgressBar.on_created_contents = function (_window)
    {
        ...
        if (this._getEnvironment()?._p_enableaccessibility && this._isAccessibilityEnable())
        {
    ...
    _pProgressBar.on_attach_contents_handle = function (win, enablea11y)
    {
        if (this._p_progressstartcap)
            this._p_progressstartcap.attachHandle(win, enablea11y);

        var itemlist = this._progressbaritems;
        for (var i = 0; i < itemlist.length; i++)
            itemlist[i].attachHandle(win, enablea11y);

        if (this._p_progressendcap)
            this._p_progressendcap.attachHandle(win, enablea11y);

        if (this._progressbartext)
            this._progressbartext.attachHandle(win, enablea11y);
    ...
    _pProgressBar._createProgressStartCap = function (iscreateonly)
    {
        ...
        if (control_elem)
        {
            ...
            if (this._startcap_iconwidth === undefined)
            {
                ...
                var startcap_iconfullurl = nexacro._getURIValue(startcap_icon);    
                startcap_iconfullurl = this._getEnvironment()?._getImageLocation(startcap_iconfullurl, this._getRefFormBaseUrl());
                var imagesize = nexacro._getImageSize_extended(startcap_iconfullurl, this._on_loadimage_start_icon, this);
                if (imagesize)
                {
    ...
    _pProgressBar._createProgressEndCap = function (iscreateonly)
    {
        ...
        if (control_elem)
        {
            ...
            if (this._endcap_iconwidth === undefined)
            {
                ...
                var endcap_iconfullurl = nexacro._getURIValue(endcap_icon);    
                endcap_iconfullurl = this._getEnvironment()?._getImageLocation(endcap_iconfullurl, this._getRefFormBaseUrl());
                var imagesize = nexacro._getImageSize_extended(endcap_iconfullurl, this._on_loadimage_end_icon, this);
                if (imagesize)
                {
    ...
    _pProgressBar._createProgressBarItemTemp = function (/*iscreateonly*/)
    {
        ...
        if (control_elem)
        {
            ...
            if (this._item_iconwidth === undefined)
            {
                ...
                var item_iconfullurl = nexacro._getURIValue(item_icon);    
                item_iconfullurl = this._getEnvironment()?._getImageLocation(item_iconfullurl, this._getRefFormBaseUrl());
                var imagesize = nexacro._getImageSize_extended(item_iconfullurl, this._on_loadimage_item_icon, this);
                if (imagesize)
                {
