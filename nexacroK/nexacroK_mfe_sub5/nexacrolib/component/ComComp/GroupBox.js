...
if (!nexacro.GroupBox)
{
    ...
    /* default properties */
    _pGroupBox._p_titlealign = "topleft";
    _pGroupBox._p_titlegap = undefined;
    _pGroupBox._titlegap = undefined;
    ...
    _pGroupBox._p_accessibilityrole = "groupbox";
    ...
    _pGroupBox.on_attach_contents_handle = function (win, enablea11y)
    {
        var boxcontrol = this._boxcontrol;
        var titlecontrol = this._titlecontrol;

        if (boxcontrol)
            boxcontrol.attachHandle(win, enablea11y);

        if (titlecontrol)
            titlecontrol.attachHandle(win, enablea11y);
    };
    ...
    _pGroupBox.set_titlealign = function (v)
    {
        ...
        if (this._p_titlealign != v)
        {
            this._p_titlealign = v;
    ...
    _pGroupBox._getTitlegap = function () { return (this._titlegap !== undefined) ? this._titlegap : this._p_titlegap; };

    _pGroupBox.on_apply_titlegap = function (/*titlegap*/)
    {
        this._titlegap = nexacro._isEmRemUnit(this._p_titlegap) ? this._resolveEmPx(this._p_titlegap) : undefined;
        this._recalcLayout();
    };
    ...
    _pGroupBox.on_apply_font = function (font)
    {
        ...
        if (nexacro._isEmRemUnit(this._p_titlegap))
            this._reapplyRemValues();
        else
            this._recalcLayout();
    };

    _pGroupBox._reapplyRemValues = function ()
    {
        if (!this._isEnableRedraw()) return;
        var v = this._p_titlegap;
    ...
    _pGroupBox._properties = [
        { name: "titlealign" }, { name: "titlegap" }
    ];

    nexacro._defineProperties(_pGroupBox, _pGroupBox._properties);
    ...
    _pGroupBox._recalcLayout = function ()
    {
        ...
        if (control_elem && this._is_created_contents)
        {
            ...
            var titlealign = this._p_titlealign;
            var titlegap = this._getTitlegap();
            if (!titlegap)
                titlegap = 10;
