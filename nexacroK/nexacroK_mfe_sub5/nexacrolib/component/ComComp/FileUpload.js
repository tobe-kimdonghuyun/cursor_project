...
if (!nexacro.FileUpload)
{
    ...
    _pFileUpload._itemheight = undefined;
    ...
    _pFileUpload._use_native_scroll = true;
    _pFileUpload._use_native_scroll_size = false;
    /* accessibility */
    ...
    _pFileUpload.on_attach_contents_handle = function (win, enablea11y)
    {
        nexacro._create_hidden_frame(this._unique_id, this._on_manager_onload, this, "fileupload");
        ...
        {
            item = items[i];
            item.attachHandle(win, enablea11y);
            item._setEventHandler("onfindclick", this._on_item_onfindclick, this);
            item._setEventHandler("onitemclick", this._on_item_onitemclick, this);
    ...
    _pFileUpload._getHScrollBarType = function ()
    {
        return "none";
    };

    _pFileUpload._getScrollType = function ()
    {
        return (this._p_scrolltype == "none" || this._p_scrolltype == "horizontal") ? "none" : "vertical";
    };
    //===============================================================
    ...
    _pFileUpload._properties = [
        ...
        { name: "filefilter" }, { name: "filefilterindex" }, { name: "scrollpropagation" }
    ];
    ...
    _pFileUpload.upload = function (v)
    {
        ...
        if (v == undefined)
        {
            ...
            {
                uploadurl = env._getServiceLocation(this._p_uploadurl, null, null, false, true);
            }
        }
        else
        {
            uploadurl = env._getServiceLocation(v, null, null, false, true);
        }

        var url = nexacro._getParsedUri(uploadurl);
        if (url.host != undefined && url.protocol != undefined)
        {
            if (!nexacro._isSameOrigin(uploadurl, env.getProjectPath()))
                this._iscors = true;
        }
        ...
        if (uploadurl)
        {
            ...
            if (nexacro._OS == "iOS" && nexacro._isHybrid && nexacro._isHybrid() || nexacro._macOSWebView)
            {
                        ...
                        try
                        {
                            ...
                            else
                            {
                                if (data)
                                {
                                    ...
                                    {
                                        result = nexacro._Deserializer[data_type](data, evttarget);
                                    }
                                    ...
                                    {
                                        evttarget.on_fire_onsuccess(result[1], errorcode, errormsg, targetUrl, result);
                                    }
                                }
                            }
                        }
                ...
                {
                    nexacro._uploadTransferXHR(fileinfo, "", env._getHttpVariableAll(), env._p_networksecurelevel, uploadurl, _on_manager_onload());
                    ret = true;
                }
            }
            else
    ...
    _pFileUpload._bindUploadDataHandler = function (e)
    {
        ...
        if (this.readyState == 4)
        {
            ...
            if (this.status == 200)
            {
                ...
                if (nexacro._Deserializer[data_type])
                {
                    result = nexacro._Deserializer[data_type](data, pthis);
                }
    ...
    _pFileUpload._updateFileList = function ()
    {
        ...
        for (var i = 0, item_len = this._items.length, files; i < item_len; i++)
        {
            ...
            if (files)
            {
                ...
                {
                    vfile = new nexacro.VirtualFile("upfile" + i + "-" + j, "");
                    
                    vfile._setHandle(files[j]);
    ...
    _pFileUpload.on_apply_font = function (font)
    {
    ...
    _pFileUploadItemControl.on_attach_contents_handle = function (win, enablea11y)
    {
        var itemedit = this.fileitemedit;
        var itembutton = this.fileitembutton;

        itemedit.attachHandle(win, enablea11y);
        itemedit._setEventHandler("oneditclick", this._on_itemedit_oneditclick, this);
        itemedit._setEventHandler("onlbuttondown", this._on_itemedit_oneditlbuttondown, this);

        itembutton.attachHandle(win, enablea11y);
        itembutton._setEventHandler("onclick", this._on_itembutton_onfindclick, this);
        itembutton._setEventHandler("onlbuttondown", this._on_itembutton_onfindlbuttondown, this);
