...
if (!nexacro.Menu)
{
	...
	_pMenu._p_navtargetcolumn = "";
	_pMenu._p_navurlcolumn = "";
	_pMenu._p_navtargetpropertiescolumn = "";
	...
	_pMenu._navigationbuttonsize = undefined;
	_pMenu._popupnavigationbuttonsize = undefined;
	...
	_pMenu.on_attach_contents_handle = function (win, enablea11y)
	{
		var i, len, items = this._items;
		if (items)
		{
	...
	_pMenu._apply_setfocus = function (evt_name)
	{
		...
		if (this._getEnvironment()?._p_enabletouchevent)
		{
	...
	_pMenu._getDlgCode = function (keycode/*, altKey, ctrlKey, shiftKey*/)
	{
		...
		var want_arrow = !(this._getEnvironment()?._p_enableaccessibility && (keycode == E.KEY_DOWN || keycode == E.KEY_UP));
	...
	_pMenu._getNavigationButtonSize = function ()
	{
		return (this._navigationbuttonsize !== undefined) ? this._navigationbuttonsize : this._p_navigationbuttonsize;
	};

	_pMenu.on_apply_navigationbuttonsize = function (/*val*/)
	{
		this._navigationbuttonsize = nexacro._isEmRemUnit(this._p_navigationbuttonsize) ? this._resolveEmPx(this._p_navigationbuttonsize) : undefined;
	...
	_pMenu._getPopupNavigationButtonSize = function ()
	{
		return (this._popupnavigationbuttonsize !== undefined) ? this._popupnavigationbuttonsize : this._p_popupnavigationbuttonsize;
	};

	_pMenu.on_apply_popupnavigationbuttonsize = function (/*val*/)
	{
		this._popupnavigationbuttonsize = nexacro._isEmRemUnit(this._p_popupnavigationbuttonsize) ? this._resolveEmPx(this._p_popupnavigationbuttonsize) : undefined;
		if (this._is_created_contents)
			this.on_change_containerRect();
	};

	_pMenu.on_apply_font = function (font)
	{
		nexacro.Component.prototype.on_apply_font.call(this, font);
		this._reapplyRemValues();
	};

	_pMenu._reapplyRemValues = function ()
	{
		if (!this._isEnableRedraw()) return;
		var v = this._p_navigationbuttonsize;
		if (nexacro._isEmRemUnit(v))
			this.on_apply_navigationbuttonsize(v);
		v = this._p_popupnavigationbuttonsize;
		if (nexacro._isEmRemUnit(v))
			this.on_apply_popupnavigationbuttonsize(v);
		v = this._p_popupitemheight;
		if (nexacro._isEmRemUnit(v))
			this.on_apply_popupitemheight(v);
	};
	...
	_pMenu.set_navtargetcolumn = function (v)
	{		
	...
	_pMenu.on_apply_navtargetcolumn = function ()
	{
		var popupmenu = this._popupmenu;
		if (popupmenu)
		{
	...
	_pMenu.set_navurlcolumn = function (v)
	{
	...
	_pMenu.on_apply_navurlcolumn = function ()
	{
		var popupmenu = this._popupmenu;
		if (popupmenu)
		{
	...
	_pMenu.set_navtargetpropertiescolumn = function (v)
	{
	...
	_pMenu.on_apply_navtargetpropertiescolumn = function ()
	{
		var popupmenu = this._popupmenu;
		if (popupmenu)
		{
	...
	_pMenu._properties = [
		...
		{ name: "navtargetcolumn" }, { name: "navurlcolumn" }, { name: "navtargetpropertiescolumn" }
	];
	...
	{
		var is_accessibility = this._getEnvironment()?._p_enableaccessibility;
	...
	_pMenu.on_fire_onitemclick = function (obj, itemid, itemuserdata, index, level, refobj)
	{
		if (this.onmenuclick && this.onmenuclick._has_handlers)
		{
			...
			if (this.onmenuclick.defaultprevented) return;
		}
	...
	_pMenu._createMenu = function ()
	{
		...
		if (control)
		{
			...
			if (ds && this._p_captioncolumn && this._p_captioncolumn && this._p_idcolumn)
			{
				...
				var text, enable, hotkey, id, level, userdata; //, navtarget, navurl, navtargetproperties;
				var item;
				...
				for (var i = 0; i < len; i++)
				{
					...
					if (level == 0)
					{
						...
						//navtarget = ds.getColumn(i, this._p_navtargetcolumn);
						//navurl = ds.getColumn(i, this._p_navurlcolumn);
						//navtargetproperties = ds.getColumn(i, this._p_navtargetpropertiescolumn);

						item = new nexacro._MenuItemControl("item" + i, 0, 0, 0, 0, null, null, null, null, null, null, this);
						this._items.push(item);

						item.userdata = userdata;
						//item.navtarget = navtarget;
						//item.navurl = navurl;
						//item.navtargetproperties = navtargetproperties ? JSON.parse(navtargetproperties) : undefined;
						item._bindindex = i;
						item.index = index++;
						item.datarow = i;
						item._id = id ? id : "";
	...
	_pMenu._showNavigationButton = function (navigation_visible)
	{
		...
		if (navigation_visible)
		{
			...
			var buttonsize = this._getNavigationButtonSize();
	...
	_pMenu._rearrangeMenuItems = function (need_calc, dir)
	{
		...
		var left = this._is_navigation_visible ? (this._getNavigationButtonSize() || this._prev_width || 0) : 0;
		var navigationnext_width = this._is_navigation_visible ? (this._getNavigationButtonSize() || this._next_width || 0) : 0;
	...
	_pMenu._showPopup = function (obj, x, y, bcapture)
	{
		...
		if (this._innerdataset && this._p_levelcolumn && this._p_captioncolumn && this._p_idcolumn && obj._canExpand === true)
		{
			...
			if (popupmenu == null)
			{
				...
				popupmenu.set_navigationbuttonsize(this._getPopupNavigationButtonSize());
	...
	_pMenu._getItemIndex = function (inc)
	{
		...
		if (!enablea11y && item && item._p_enable == false && inc < 5 && inc > -5)
