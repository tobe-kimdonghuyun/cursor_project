...
if (!nexacro.DataObject)
{
    ...
    _pDataObject.on_created = function () 
    {
        ...
        if (!nexacro.isDesignMode && this._p_preload && !this._is_preloaded)
        {
            if (this._p_url && this._p_parent)
            {
                ...
                var env = this._p_parent._getEnvironment();
                var url = env?._getServiceLocation(this._p_url);
    ...
    */
    ...
    _pDataObject.load = function (async)
    {
        ...
        {
            var env = this._p_parent._getEnvironment();
            _url = env?._getServiceLocation(_url, baseurl);
            ...
            if (_url.length)
            {
                ...
                {
                    //outds
                    var service = env?._getServiceObject(this._p_url, true);
    ...
    _pDataObject.request = function (svcid, method, url, jsonparam)
    {
        ...
        {
            var env = this._p_parent._getEnvironment();
            let _url = env?._getServiceLocation(url, baseurl);
    ...
    //deleteAllDataInPath
    _pDataObject.deleteAllDataInPath = function (path, srcdsid)
    {
        var rootobj = this.getObjectByPath(path);
        if (rootobj)
        {
            var rowobjs = this._deleteAllRowData(path);
            this._updatelist.push({ "uid": "deleteAll" + "_" + parseInt(nexacro._random() * 100000000), "type": "deleteAll", "index": -1, "data": rowobjs, "dataobjectpath": path, "sourcedatasetid": srcdsid });
            if (this.enableevent && rowobjs)
            {
                this._before_fire_event = true;
                this.on_fire_ondatachanged(0, "SUCCESS", 1, this._updatelist); //test code
                this._before_fire_event = false;
    ...
    */
    ...
    _pDataObject._onDataObjectRequestError = function (obj, bfireevent, errorobj, errortype, url, returncode, requesturi, locationuri, extramsg, svcid, response, response_headers)
    {
        ...
        if (bfireevent)
        {
            const environment = this._p_parent?._getEnvironment();
            if (environment)
            {
    ...
    */
    ...
    nexacro.RESTAPIItem = function (path, context, svcid, method, outDataobject, argsJSONParam, datatype, async, last_modified, version)
    {
        ...
        nexacro._CommunicationItem.call(this, path, this.type, false, last_modified, version, env); //"data"type -> this.type
    ...
    _pRESTAPIItem._getCacheList = function (path)
    {
        if (this._env)
        {
            return this._env._getModuleCacheList(path, "dataobject");
        }
        return nexacro._DataObjectCacheList[path];
    };

    _pRESTAPIItem._addCacheList = function (path, data)
    {
        if (this._env)
        {
            return this._env._addModuleCacheList(path, data, "dataobject");
        }
        return nexacro._DataObjectCacheList[path] = data;
    };
    ...
    _pRESTAPIItem.on_load_dataobject = function (data, cookie, last_modified, response_header, status)
    {
        ...
        if (bcache)
        {
            ...
            if (!d_cache)
            {
                this._addCacheList(this.path, new nexacro._DataObjectCache(dataobject, last_modified, this.version, data, response_header));
            }
            else
            {
        ...
        if (this._env)
            nexacro._pushCurrentEnvironment(this._env);
    ...
    _pRESTAPIItem._loadFromDataObject = function (data, dataobject)
    {
        ...
        if (bcache)
        {
            ...
            if (!d_cache)
            {
                this._addCacheList(this.path, new nexacro._DataObjectCache(dataobject, last_modified, this.version));
            }
            else
            {
