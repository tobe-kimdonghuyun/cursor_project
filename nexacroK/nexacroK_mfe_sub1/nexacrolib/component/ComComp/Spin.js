...
if (!nexacro.Spin)
{
    ...
    _pSpin.on_attach_contents_handle = function (win, enablea11y)
    {
        if (this._p_spinedit)
        {
            this._p_spinedit.attachHandle(win, enablea11y);
        }
        if (this._p_spinupbutton)
        {
            this._p_spinupbutton.attachHandle(win, enablea11y);
        }
        if (this._p_spindownbutton)
        {
            this._p_spindownbutton.attachHandle(win, enablea11y);
        }
    ...
    _pSpin._on_value_change = function ()
    {
        ...
        {
            var ret_apply = this.applyto_bindSource("value", cur_value);
            if (!ret_apply || this._p_value == pre_value)
                return false;
        }
