...
if (!nexacro._ListViewCellControl)
{
    ...
    _pListViewCellInfo._addPropertyMap("_pListViewCellInfo", [
        ...
        ["layoutorder", "layoutorder", false, null, false],
        ["flexgrow", "flexgrow", false, null, false],
        ["flexshrink", "flexshrink", false, null, false],
        ["tablecellarea", "tablecellarea", false, null, false]
    ...
    _pListViewCellInfo._syncPosition = function (cellcontrol)
    {
        ...
        this.set_layoutorder(cellcontrol.layoutorder);
        this.set_flexgrow(cellcontrol.flexgrow);
        this.set_flexshrink(cellcontrol.flexshrink);
        this.set_tablecellarea(cellcontrol.tablecellarea);
    };
    ...
    _pListViewCellControl.on_attach_contents_handle = function (win, enablea11y)
    {
        nexacro._CellControl.prototype.on_attach_contents_handle.call(this, win, enablea11y);

        this._updateAll();
        this._refinfo._syncPosition(this);
    };

    // onCreatedChild 에서 _setLayoutContentsProps 처리로 공통화
    /*
    _pListViewCellControl._updateAll = function (status, onlycontents, nochk_updatearea)
    {
        nexacro._CellControl.prototype._updateAll.call(this, status, onlycontents, nochk_updatearea);
    ...
    */
    ...
    {
        if (this._getEnvironment()?._stringrc)
        {
            if (v && v.indexOf && v.indexOf("TEXT(") > -1)
                v = this._getEnvironment()?._getTextTemplateValue(v);
        }
        return v;
    };
...
if (!nexacro._ListViewBandExpandbarControl)
{
    nexacro._ListViewBandExpandbarControl = function (id, left, top, width, height, right, bottom, minwidth, maxwidth, minheight, maxheight, parent)
    {
        ...
        if (env?._has_listview_expand_status)
        {
...
if (!nexacro._ListViewBandControl)
{
    ...
    */
    ...
    _pListViewBandControl._is_fluid_layout = true;
    _pListViewBandControl._excluded_flex = true;
    ...
    _pListViewBandControl._onResetUserExpand = function ()
    {
        ...
        if (expandbar)
        {
            ...
            if (listview && listview.getBandExpandStatus(this._itemindex))
            {
                ...
                if (this._getEnvironment()?._p_enableaccessibility && nexacro._isArray(listview._getItem(this._itemindex)))
                {
...
if (!nexacro.ListView)
{
    ...
    */
    ...
    _pListView._updateitemtype = "band";
    ...
    // CURR:NativeScroll 재처리중
    _pListView._default_scrolleventthrottle = "aniframe";               // scroll native event 사용으로 none 처리
//  _pListView._default_scrolleventthrottle = "none";                   // scroll native event 사용으로 none 처리
    ...
    _pListView.onAttachContentsHandle = function (win, enablea11y)
    {
    ...
    */
    ...
    _pListView._onAfterCreated = function ()
    {
        this._setEventHandler("onlbuttondown", this._on_listview_onlbuttondown, this);
        this._setEventHandler("onlbuttonup", this._on_listview_onlbuttonup, this);
        ...
        if (band)
        {
            ...
            if (rowidx >= 0)
            {
                ...
                else
                {
                    ...
                    if (!nextcell)
                    {
                        if (this._getEnvironment()?._p_enableaccessibility && band._p_expandbartype == true && band._band == "body" && !this._is_expandbar_focus)
                        {
        ...
        if (band)
        {
            ...
            if (rowidx >= 0)
            {
                ...
                if (!prevcell)
                {
                    ...
                    if (band = this._getCurrentBand(rowidx, bandidx))
                    {
                        if (this._getEnvironment()?._p_enableaccessibility && band._p_expandbartype == true && band._band == "body")
                        {
                            expandbar = band.expandbar;
                        }
                        else
    ...
    _pListView.on_notify_popup_onclose = function (popupindex, startindex, startlevel)
    {
        ...
        if (panel)
        {
            ...
            if (slot)
            {
                ...
                panel._setPanelSlotStatusBandNoAction(startindex, nexacro._PanelSlotConst.STATUS_CLOSE);
    ...
    */
    ...
    _pListView.on_apply_formats = function ()
    {
        ...
        //  this._updateFormats();
            this._analyzeKeyMatrix();
            this._recreateItems(-1, true, true);
        }
    };
    ...
    // [component.layout.fluid.config:subinit]      
    _pListView._onInitSubFluidItemsLayout   = function (item, binddata, bindindex) 
    {
    };
    */

    // CURR : NativeScroll 처리중 --> CombaseEx 로 이동
    // TODO : CombaseEx 로 이동
    //*
    ...
    //*/
    // CURR: Native Scroll 처리중

    // CURR : NativeScroll 처리중
    //*
    _pListView._onCallbackApper = function ()
    {
        this._resetStatus();
    }
    _pListView._onCallbackReady = function ()
    {
        this._resetStatus();
        this._analyzeKeyMatrix();
    }
    ...
    // CURR: Native Scroll 처리중
    // Native Scroll Event 적용과 함께 기본 Scroll i/f 사용, ListView 별도처리시 분리
    //*
    ...
    _pListView._useNativeWheelScroll = function ()
    {
        return false;
    };
    ...
    _pListView.hideDetailBand = function (rowindex)
    {
        ...
        if (slot)
        {
            ...
            switch (this._p_bandexpandtype)
            {
                default         :
                case "popup"    : this._closeItem(rowindex);    break;
                case "expand"   : 
                case "accordion": this._collapseItem(rowindex); break;
            }
            ...
            // TODO : 아래 제거 _closeItem, _collapseItem에서 처리됨
            /*
    ...
    _pListView.showDetailBand = function (rowindex)
    {
        ...
        {
            var bandstatus = slot._getSlotStatusBand();
            var item = this._getItem(rowindex);
            
            switch (this._p_bandexpandtype)
            {
                default         :
                case "popup"    : this._popupItem(rowindex,  item, "band", "methodcall"); break;
                case "expand"   : 
                case "accordion": this._expandItem(rowindex, item, "band", "methodcall"); break;
            }
            ...
            // TODO : 아래 제거 _popupItem, _expandItem에서 처리됨
            /*
            var items = this._getItem(rowindex);
            if (items)
            {
    ...
    */
    ...
    _pListView._on_listview_onlbuttondown = function (obj, e)
    {
        ...
        if (popup && popup._isPopup() && e.bandid != "detail" && e.fromreferenceobject.id != "expandbar")
        {
            if (!popup._attached_comp)
                return false;

            var itemindex = popup._attached_comp._itemindex;
            this._toggleItem(itemindex);
    ...
    _pListView.on_fire_sys_onlbuttondown = function (button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, meta_key)
    {
        ...
        if (cell)
        {
            ...
            if (autoenter != "select") // autoenter가 "select"가 아닌 경우
            {
                ...
                {
                    if (cell.selected == false && this._getEnvironment()?._p_enableaccessibility)
                    {
                        cell._actionEditCell(cell, "lbuttondown");
                    }
                }
