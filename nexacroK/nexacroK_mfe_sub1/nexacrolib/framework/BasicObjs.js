...
if (!nexacro.Decimal)
{
    ...
    {       
        ...
        this._datatyperule = env?.datatyperule ?? nexacro._datatyperule;
        ...
        if (v2 == null)
        {
            ...
            {
                if (this._datatyperule != "1.0" && isNaN(+v1))
                {
                    this.hi = Number.NaN;
                    this.lo = 0;
                }
    ...
    _pDecimal._parse = function (numstr)
    {
        ...
        while (tpos < fpos)
        {
            ...
            {
                ch = numstr.charAt(pos++);
                this._mulDouble(10.0);
                this._addDouble((ch - '0') | 0);
            }
        }
    ...
    {
        if (ch == " " || (this._datatyperule != "1.0" && ch == "　"))
            return true;
        return false;
    }
    ...
    _pDecimal.add = function (other) 
    {
        const returnValue = new nexacro.Decimal(this);
        if (other instanceof nexacro.Decimal)
        {
            returnValue.addDecimal(other);
        }
        else if (typeof other === "number")
        {
            returnValue.addDouble(other);
        }
        return returnValue;
    };
    ...
    {
        this._addDouble(dval);
        this._normalizeToEPSILON();
    }
    _pDecimal._addDouble = function (dval)
    {
        var H, S, s, e;
    ...
    {
        this._addDecimal(dec);
        this._normalizeToEPSILON();
    }
    _pDecimal._addDecimal = function (dec)
    {
        var S, s, e;
    ...
    _pDecimal.sub = function (other) 
    {
        const returnValue = new nexacro.Decimal(this);
        if (other instanceof nexacro.Decimal)
        {
            returnValue.subDecimal(other);
        }
        else if (typeof other === "number")
        {
            returnValue.subDouble(other);
        }
        return returnValue;
    };
    ...
    {
        this._subDouble(dval);
        this._normalizeToEPSILON();
    }
    _pDecimal._subDouble = function (dval)
    {
        var S, s, e;
    ...
    {
        this._subDecimal(dec);
        this._normalizeToEPSILON();
    }
    _pDecimal._subDecimal = function (dec)
    {
        var H, h, T, t, S, s, e, f;
        if (this._datatyperule != "1.0")
        {
    ...
    _pDecimal.mul = function (other) 
    {
        const returnValue = new nexacro.Decimal(this);
        if (other instanceof nexacro.Decimal)
        {
            returnValue.mulDecimal(other);
        }
        else if (typeof other === "number")
        {
            returnValue.mulDouble(other);
        }
        return returnValue;
    };
    ...
    {
        this._mulDouble(dval);
        this._normalizeToEPSILON();
    }
    _pDecimal._mulDouble = function (dval)
    {
        var hx, tx, hy, ty, C, c;
        var hi;
    ...
    {
        this._mulDecimal(dec);
        this._normalizeToEPSILON();
    }
    _pDecimal._mulDecimal = function (dec)
    {
        var hx, tx, hy, ty, C, c;
        var hi;
    ...
    _pDecimal.div = function (other) 
    {
        const returnValue = new nexacro.Decimal(this);
        if (other instanceof nexacro.Decimal)
        {
            returnValue.divDecimal(other);
        }
        else if (typeof other === "number")
        {
            returnValue.divDouble(other);
        }
        return returnValue;
    };
    ...
    {
        this._divDouble(dval);
        this._normalizeToEPSILON();
    }
    _pDecimal._divDouble = function (dval)
    {
    ...
    {
        this._divDecimal(dec);
        this._normalizeToEPSILON();
    }
    _pDecimal._divDecimal = function (dec)
    {
        var q1, q2, s1, s2;
    ...
    {
        this._modDouble(dval);
        this._normalizeToEPSILON();
    }
    _pDecimal._modDouble = function (dval)
    {
    ...
    {
        this._modDecimal(dec);
        this._normalizeToEPSILON();
    }
    _pDecimal._modDecimal = function (dec)
    {
    ...
    _pDecimal.log = function ()
    {
        ...
        else 
        {
            ...
            this._mulDecimal(exp_val);
            this._addDecimal(new nexacro.Decimal(Math.log(hi)));
            this._subDouble(1.0);
        }
    ...
    _pDecimal.log10 = function ()
    {
        ...
        this._divDecimal(nexacro.Decimal.LOG10E);
        return this;
    };
    ...
    _pDecimal._toString = function (precision, format, locale)
    {
        ...
        var buf = [];
        for (i = 0; i < _cvt.buf.length; i++) 
        {
            buf[i] = parseInt(_cvt.buf.charAt(i), 10);
        }
        var epos = digitlen + 1;
        ...
        // _getCVT와 동일하게 buf 배열을 다시 string으로 변환하여 _cvt.buf에 할당
        _cvt.buf = buf.join('');

       // buf[epos] = 0; use strick mode
    ...
    _pDecimal._toStandardStr = function (cvt, locale)
    {
        ...
        function applyGrouping(value_str, targetGrouping, sepCh)
        {
            var grouping_strs = [], grouping_value;
            var grouping_idx = 0, grouping_cnt = targetGrouping.length;

            while (value_str.length > 0)
            {
                if (grouping_idx < grouping_cnt)
                {
                    grouping_value = targetGrouping[grouping_idx++];
    ...
    _pDecimal.toExponential = function (fractionDigits)
    {
        ...
        {
            if (fractionDigits < 0)
                throw new Error('toExponential() argument must be greater than or equal to 0.');
    ...
    _pDecimal.toPrecision = function (precision)
    {
        ...
        {
            if (precision < 1)
                throw new Error('toPrecision() argument must be greater than or equal to 1.');
    ...
    _pDecimal.isZero = function ()
    {
        return Math.abs(this.hi + this.lo) < this.EPSILON;
    };

    _pDecimal.equals = function (other, precision)
    {
        // precision이 주어지지 않으면 기본 정밀도(EPSILON) 사용
        const eps = precision || this.EPSILON;

        let diffHi, diffLo;

        if (typeof other === "string" || typeof other === "number")
        {
            other = new nexacro.Decimal(other);

            diffHi = this.hi - other.hi;
            diffLo = this.lo - other.lo;
        }
        ...
        if (other instanceof nexacro.Decimal)
        {
            ...
            // 절대값 차이가 EPSILON보다 작으면 true
            return Math.abs(diffHi + diffLo) <= eps;
        }

        return false;
    };

    _pDecimal._normalizeToEPSILON = function ()
    {

        // 전체 값이 임계값보다 작으면 완전한 0으로 처리
        if (Math.abs(this.hi) < this.EPSILON && Math.abs(this.lo) < this.EPSILON)
        {
            this.hi = 0.0;
            this.lo = 0.0;
            return;
...
if (!nexacro.Image)
{
    ...
    _pImageObject.set_src = function (v)
    {
        ...
        {
            this._p_src = v;
            this.handle = nexacro._getImageObject_extended(v, this.on_load, this);  //check trans Nexaro K (imagefullurl, text)//, this.on_loading, this); //, this.on_loading, this);
        }
    };
