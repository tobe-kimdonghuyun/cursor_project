...
if (!nexacro.ExcelExportObject)
{
    ...
    _pExcelExport.set_exporturl = function (v)
    {
        if (v != this._p_exporturl)
        {
            ...
            if (v)
            {
                ...
                this._exporturl = env?._getServiceLocation(v, this._p_parent._getFormBaseUrl(), null, false);
            }
        }
        return v;
    };
    ...
    _pExcelExport.addExportItem = function (type, item/*, range*/)
    {
        ...
        if (type && item)
        {
            ...
            if (use_arg_len != 2)
            {
                ...
                {
                    if (use_arg_len >= 2)  arguments[1]  && eItem.set_source(arguments[1]);
                    if (use_arg_len >= 3)  arguments[2]  && eItem.set_range(arguments[2]);
                    if (use_arg_len >= 4)  arguments[3]  && eItem.set_exporthead(arguments[3]);
                    if (use_arg_len >= 5)  arguments[4]  && eItem.set_exportselect(arguments[4]);
                    if (use_arg_len >= 6)  arguments[5]  && eItem.set_exportmerge(arguments[5]);
                    if (use_arg_len >= 7)  arguments[6]  && eItem.set_exportvalue(arguments[6]);
                    if (use_arg_len >= 8)  arguments[7]  && eItem.set_exportimage(arguments[7]);
                    if (use_arg_len >= 9)  arguments[8]  && eItem.set_exceptstyle(arguments[8]);
                    if (use_arg_len >= 10) arguments[9]  && eItem.set_exportsize(arguments[9]);
                    if (use_arg_len >= 11) arguments[10] && eItem.set_valuetypecellindex(arguments[10]);
                    if (use_arg_len >= 12) arguments[11] && eItem.set_valuetypebandid(arguments[11]);
                }
            }
    ...
    _pExcelExport._getEnvironment = function ()
    {
        if (this._p_parent)
    ...
    _pExportItem._getCellLocale = function (cell, rowidx)
    {
        ...
        if (!locale)
        {
            const env = this._getForm()._getEnvironment();
            ...
            else if (displaytype == "imagecontrol")
            {
                ...
                if (expImg == "url" || expImg == "image")
                {
                    ...
                    if (path)
                    {
                        ...
                        text = form._getEnvironment()?._getImageLocation(url, form._getFormBaseUrl());
                    }
                    else
                    {
                        text = path;
    ...
    _pExportItem._gridExportContinue = function (exportObj)
    {
        ...
        if (!ds_style2)
        {
            ds_style2 = this._ds_style2 = new nexacro.DatasetBase("STYLE2", this);
        ...
        delete this._ds_cell;
        var ds = new nexacro.DatasetBase("CELL", this);
        this._ds_cell = ds;
                    ...
                    if (display_type == "imagecontrol")
                    {
                        ...
                        if (expImg == "url" || expImg == "image")
                        {
                            ...
                            if (path)
                            {
                                var url = nexacro._getURIValue(path);
                                const form = this._getForm();
                                val = form._getEnvironment()?._getImageLocation(url, form._getFormBaseUrl());
                            }
                            else
                            {
                                val = path;
        ...
        this._ds_response = new nexacro.DatasetBase("RESPONSE", this);
            ...
            if (is_end)
            {
                ...
                if (is_finish)
                {
                    ...
                    if (exportObj._filedownload)
                    {
                        ...
                        //--------------------------------------------------------------------------------------------
                        if (nexacro._iOSWebView)
                        {
                            nexacro._downloadExport(excelURL, false, exportObj._hidden_frame_handle, exportfilename_, exportfilepath_, exportObj._p_filefilter, exportObj._p_filefilterindex, "export", exportObj._getEnvironment());
                        }
                        else
                        {
    ...
    _pExportItem._gridItemExport = function (exportObj)
    {
        ...
        if (!ds_style)
        {
            ds_style = this._ds_style = new nexacro.DatasetBase("STYLE", this);
        ...
        if (!ds_command)
        {
            ds_command = new nexacro.DatasetBase("COMMAND", this);
    ...
    _pExportItem._getEnvironment = function ()
    {
