...
if (!nexacro.FlexibleComponent)
{
    ...
    _pTextControl.on_attach_contents_handle = function (win, enablea11y)
    {
        var cellElem = this._cell_elem;
        if (cellElem)
        {
    ...
    // charcounttext를 count/separator/totalcount로 분리 하여 가지는 control
    //==============================================================================
    // nexacro.FieldBaseCharCountControl
    //==============================================================================
    nexacro.FieldBaseCharCountControl = function (id, left, top, width, height, right, bottom, minwidth, maxwidth, minheight, maxheight, parent)
    {
        nexacro.Component.call(this, id, left, top, width, height, right, bottom, minwidth, maxwidth, minheight, maxheight, parent);
        this._setControl();
        this._count_control = null;
        this._sep_control = null;
        this._total_control = null;
    };

    var _pFieldBaseCharCountControl = nexacro._createPrototype(nexacro.Component, nexacro.FieldBaseCharCountControl);
    nexacro.FieldBaseCharCountControl.prototype = _pFieldBaseCharCountControl;
    _pFieldBaseCharCountControl._type_name = "FieldBaseCharCount";

    _pFieldBaseCharCountControl.on_create_contents = function ()
    {        
        var field = this._p_parent;
        if (!field)
        {
            return;
        }
        
        var count_ctrl = this._count_control = new nexacro.FieldBaseTextControl("count", 0, 0, 0, 0, null, null, null, null, null, null, this);
        count_ctrl._setControl("StaticControl");
        count_ctrl._excluded_flex = !field._isFlexible();
        count_ctrl.createComponent(true);

        var sep_ctrl = this._sep_control = new nexacro.FieldBaseTextControl("separator", 0, 0, 0, 0, null, null, null, null, null, null, this);
        sep_ctrl._setControl("StaticControl");
        sep_ctrl._excluded_flex = !field._isFlexible();
        sep_ctrl.createComponent(true);

        var total_ctrl = this._total_control = new nexacro.FieldBaseTextControl("totalcount", 0, 0, 0, 0, null, null, null, null, null, null, this);
        total_ctrl._setControl("StaticControl");
        total_ctrl._excluded_flex = !field._isFlexible();
        total_ctrl.createComponent(true);

        if (field._isFlexible())
        {
            let control_elem = null;
            count_ctrl._update_layoutbasis();
            control_elem = count_ctrl.getElement();
            if (control_elem)
            {
                control_elem.setElementTableCellArea("C");
            }

            sep_ctrl._update_layoutbasis();
            control_elem = sep_ctrl.getElement();
            if (control_elem)
            {
                control_elem.setElementTableCellArea("C");
            }

            total_ctrl._update_layoutbasis();
            control_elem = total_ctrl.getElement();
            if (control_elem)
            {
                control_elem.setElementTableCellArea("C");
            }
    ...
    _pFieldBaseCharCountControl.on_created_contents = function (win)
    {
        if (this._count_control) this._count_control.on_created(win);
        if (this._sep_control) this._sep_control.on_created(win);
        if (this._total_control) this._total_control.on_created(win);        
    };

    _pFieldBaseCharCountControl.on_create_contents_command = function ()
    {
        var str = "";
        if (this._count_control) str += this._count_control.createCommand();
        if (this._sep_control) str += this._sep_control.createCommand();
        if (this._total_control) str += this._total_control.createCommand();
        return str;
    };

    _pFieldBaseCharCountControl.on_attach_contents_handle = function (win)
    {
        if (this._count_control) this._count_control.attachHandle(win);
        if (this._sep_control) this._sep_control.attachHandle(win);
        if (this._total_control) this._total_control.attachHandle(win);
    };

    _pFieldBaseCharCountControl.on_after_created = function()
    {
        this._recalcLayout();
    };

    _pFieldBaseCharCountControl._update_position = function ()
    {
        nexacro.Component.prototype._update_position.call(this);
        this._recalcLayout();
    };

    _pFieldBaseCharCountControl._recalcLayout = function ()
    {
        var x = 0;
        var sub_controls = [this._count_control, this._sep_control, this._total_control];
        for (var i = 0; i < sub_controls.length; i++)
        {
            if (sub_controls[i])
            {
                var control_size = sub_controls[i]._on_getFitSize();
                sub_controls[i].move(x, 0, control_size[0], control_size[1]);
                x += control_size[0];
            }
        }
    };

    _pFieldBaseCharCountControl.set_count = function (v)
    {
        if (this._count_control)
        {
            this._count_control.set_text(nexacro._toString(v));
        }
    };

    _pFieldBaseCharCountControl.set_separator = function (v)
    {
        if (this._sep_control)
        {
            this._sep_control.set_text(nexacro._toString(v));
        }
    };

    _pFieldBaseCharCountControl.set_totalcount = function (v)
    {
        if (this._total_control)
        {
            this._total_control.set_text(nexacro._toString(v));
        }
    };

    _pFieldBaseCharCountControl._on_getFitSize = function ()
    {
        var w = 0, h = 0;
        var sub_controls = [this._count_control, this._sep_control, this._total_control];
        for (var i = 0; i < sub_controls.length; i++)
        ...
        if (border)
        {
            w += border._getBorderWidth();
            h += border._getBorderHeight();
        }
    ...
    _pFieldBase._charcnt_count = "0";
    _pFieldBase._charcnt_separator = " / ";
    _pFieldBase._charcnt_total = "0";
    ...
    _pFieldBase._updateCharCount = function (b_create)
    {
        ...
        if (control_elem)
        {
            ...
            {
                var charcnt_control = this._charcnt_control = new nexacro.FieldBaseCharCountControl("charcounttext", 0, 0, 0, 0, null, null, null, null, null, null, this);
                charcnt_control._excluded_flex = true; // 하위 컨트롤 생기면서 fluid 동작 안함
    ...
    _pFieldBase.on_attach_contents_handle = function (win, enablea11y)
    {
        this._box.attachHandle(win, enablea11y);

        if (this._helptxt_control)
            this._helptxt_control.attachHandle(win, enablea11y);
        if (this._charcnt_control)
            this._charcnt_control.attachHandle(win, enablea11y);

        this._label.attachHandle(win, enablea11y);
    ...
    {
        var control_elem = this.getElement();
        if (control_elem && kind != "create") // 생성 시점에는 계산 하지 않음
        {
			...
			if (this._isFlexible())
			{
				...
				if (contbox_control)
					contbox_control.resize(client_width, client_height - footer_height - header_height);
				if (helptxt_control)
					helptxt_control.resize(client_width - charcnt_width, footer_height);
				if (charcnt_control)
			...
			{
				if (contbox_control)
					contbox_control.move(0, header_height, client_width, client_height - footer_height - header_height);
				if (helptxt_control)
					helptxt_control.move(0, client_height - footer_height, client_width - charcnt_width, footer_height);
				if (charcnt_control)
					charcnt_control.move(client_width - charcnt_width, client_height - footer_height, charcnt_width, footer_height)
			}
    ...
    _pFieldBase._recalcCharCount = function (kind)
    {
        var control_elem = this.getElement();
        if (control_elem && this._p_usecharcount && kind != "create") // 생성 시점에는 계산 하지 않음
        {
            var charcnt_control = this._charcnt_control;
    ...
    {
        var label = this._label;
        if (label && kind != "create") // 생성 시점에는 계산 하지 않음
        {
            var l = 0, t = 0, w = 0, h = 0;
            var contbox_control = this._box;
    ...
    {
        this._charcnt_count = nexacro._toString(cur);
        this._charcnt_total = nexacro._toString(max);
        this._charcnt_text = this._charcnt_count + this._charcnt_separator + this._charcnt_total;
        this.on_apply_charcnt();
    };
    ...
    _pFieldBase.on_apply_charcnt = function ()
    {
        if (this._charcnt_control)
        {
            this._charcnt_control.set_count(this._charcnt_count);
            this._charcnt_control.set_separator(this._charcnt_separator);
            this._charcnt_control.set_totalcount(this._charcnt_total);

            this._recalcCharCount(this._is_created ? undefined : "create");
        }
    };
    ...
    {
         // 생성 시점에는 계산 하지 않음
        this._recalc_contents(this._is_created ? "onsize" : "create");
    };
    ...
    _pFieldBase.on_apply_usehelpertext = function ()
    {
        ...
        this._recalc_contents(this._is_created ? undefined : "create");
    };
    ...
    _pFieldBase.on_apply_value = function ()
    {
        ...
        this._refreshLabel(this._is_created ? undefined : "create"); // 생성 시점에는 계산 하지 않음
    };
