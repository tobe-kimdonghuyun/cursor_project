...
if (!nexacro.DateRangePicker)
{
    ...
    _pDateRangePicker._footerbuttonalign = "justify";//_footerbuttonalign
    ...
    _pDateRangePicker.on_attach_contents_handle = function (win, enablea11y)
    {
        if (this._headline)
        {
            this._headline.attachHandle(win, enablea11y);
            this._headline._setAccessibilityStatHidden(true);
        }

        if (this._datetabbutton)
        {
            this._datetabbutton.attachHandle(win, enablea11y);
            this._datetabbutton._setAccessibilityStatHidden(true);
        }

        if (this._timetabbutton)
        {
            this._timetabbutton.attachHandle(win, enablea11y);
            this._timetabbutton._setAccessibilityStatHidden(true);
        }

        if (this._closebutton)
        {
            this._closebutton.attachHandle(win, enablea11y);
            this._closebutton._setAccessibilityStatHidden(true);
        }

        if (this._todaybutton)
        {
            this._todaybutton.attachHandle(win, enablea11y);
            this._todaybutton._setAccessibilityStatHidden(true);
        }

        if (this._okbutton)
        {
            this._okbutton.attachHandle(win, enablea11y);
            this._okbutton._setAccessibilityStatHidden(true);
        }

        if (this._cancelbutton)
        {
            this._cancelbutton.attachHandle(win, enablea11y);
            this._cancelbutton._setAccessibilityStatHidden(true);
        }

        if (this._datepicker_group)
        {
            this._datepicker_group.attachHandle(win, enablea11y);
        ...
        if (this._timepicker_group)
        {
            this._timepicker_group.attachHandle(win, enablea11y);
    ...
    _pDateRangePicker._convValueToDateObj = function (value)
    {
        ...
        if (this._p_displaytype == "time")
        {
            ...
            time = datautils.toTime(time_str);
        }
        else if (this._p_displaytype == "datetime")
        {
            ...
            date = datautils.toDate(date_str);
            time = datautils.toTime(time_str);
        }   
        else
        {
    ...
    _pDateRangePicker._convDisplayTypeValue = function (datestr)
    {
        ...
        if (datestr)
        {
            if (this._p_displaytype == "datetime")
            {
                ...
                date = datautils.toDateTime(datestr).toString();
            ...
            else if (this._p_displaytype == "date")
            {
                ...
                date = datautils.toDate(sdate).toString();
                retn.date = date;
            }
            else if (this._p_displaytype == "time")
            {
                ...
                date = datautils.toTime(sdate).toString();
                retn.time = date;
            }
        }
        else
        {
            ...
            if (this._p_displaytype == "datetime")
            {
                date = datautils.toDateTime(curr.toString()).toString();
                retn.date = date.substr(0, 8);
                retn.time = "000000000";
            }
            else if (this._p_displaytype == "date")
            {
                ...
                date = datautils.toDate(curr.toString()).toString();
                retn.date = date;
            }
            else if (this._p_displaytype == "time")
            {
    ...
    _pDateRangePicker._convertValueType = function (v, dateobj, bapplyrule)
    {
        ...
        {
            const env = this._getEnvironment();
            datatyperule = env ? env._p_datatyperule : "1.0";
            if (datatyperule == "1.0")
                return v;
        }
    ...
    */
    ...
    _pPickerHeadLineControl.on_attach_contents_handle = function (win, enablea11y)
    {
        if (this._p_startdatebutton)
        {
            this._p_startdatebutton.attachHandle(win, enablea11y);
            this._p_startdatebutton._setAccessibilityStatHidden(true);
        }
        if (this._p_startsubdatebutton)
        {
            this._p_startsubdatebutton.attachHandle(win, enablea11y);
            this._p_startsubdatebutton._setAccessibilityStatHidden(true);
        }
        if (this._p_enddatebutton)
        {
            this._p_enddatebutton.attachHandle(win, enablea11y);
            this._p_enddatebutton._setAccessibilityStatHidden(true);
        }
        if (this._p_endsubdatebutton)
        {
            this._p_endsubdatebutton.attachHandle(win, enablea11y);
            this._p_endsubdatebutton._setAccessibilityStatHidden(true);
        }
        if (this._splitline)
        {
            this._splitline.attachHandle(win, enablea11y);
    ...
    _pPickerGroup.on_attach_contents_handle = function (win, enablea11y)
    {
        var list = this._picker_ctrl_list;

        for (var i = 0; i < list.length; i++)
        {
            list[i].attachHandle(win, enablea11y);
    ...
    nexacro.DatePickerGroup = function (id, left, top, width, height, right, bottom, minwidth, maxwidth, minheight, maxheight, parent)
    {
        ...
        date = this._getValueFromDataUtils(curr.toString());
    ...
    _pDatePickerGroup._use_translate_scroll = false;
    ...
    _pDatePickerGroup._getValueFromDataUtils = function (v)
    {
        return this._getEnvironment()._getDataUtils().toDate(v);
    };
    ...
    _pDatePickerGroup._animateScroll = function (kind, chk_alldate, no_ani, is_popup_act)
    {
        ...
        var stime = (performance) ? performance.now() : (new Date());
        var pthis = this;
        var width = this._getClientWidth();
        var spos, epos;
    ...
    _pDatePickerGroup._animateScrollEnd = function (kind, is_popup_act)
    {
        ...
        if (kind == "next") // move →
        {
            var nextdate = this._getValueFromDataUtils(slot.next.picker._value);
        ...
        {
            var prevdate = this._getValueFromDataUtils(slot.prev.picker._value);
    ...
    _pDatePickerGroup._setStartDate = function (v, move)
    {
        ...
        if (v)
        {
            ...
            date = this._getValueFromDataUtils(v);
    ...
    _pDatePickerGroup._setEndDate = function (v, move)
    {
        ...
        //select endday
        if (this._p_type == "range" && !this._is_start_select)
        {
            this._setEndday();
        }
    };

    _pDatePickerGroup._setEndday = function ()
    {
        var s_date = this._range.start;
        var e_date = this._range.end;
        var list = this._picker_ctrl_list;
        for (var i = 0; i < list.length; i++)
        {
            ...
            var eyear = e_date.getFullYear();
            var emonth = e_date.getMonth() + 1;
            var edate = e_date.getDate();

            if (e_date && pos.year == eyear && pos.month == emonth)
            {
                var syear = s_date.getFullYear();
                var smonth = s_date.getMonth() + 1;
                var sdate = s_date.getDate();
                // 같은 년월의 e_date를 찾은 경우
                if (syear == eyear && emonth == smonth && sdate == edate)
                    list[i].body._setSelected(edate);
                else
                    list[i].body._setEndday(edate);
                break;
            }
        }
    };
    ...
    _pTimePickerGroup._getValueFromDataUtils = function (v)
    {
        return this._getEnvironment()._getDataUtils().toTime(v);
    };
    ...
    _pTimePickerGroup._setStartDate = function (v)
    {
        ...
        if (v)
            ...
            date = this._getValueFromDataUtils("000000000");
    ...
    _pDatePickerControlEx.on_create_contents = function ()
    {
        ...
        if (control_elem)
        {
            ...
            body.on_attach_contents_handle = function (win, enablea11y)
            {
                for (var i = 0; i < this._selections.length; i++)
                    this._selections[i].attachHandle(win, enablea11y);

                nexacro._DatePickerBodyControl.prototype.on_attach_contents_handle.call(this, win, enablea11y);
            };
            ...
            body._on_apply_day = function (flag)
            {
                ...
                var type = this.parent ? this.parent._type : "single";
                ...
                if (predayObj)
                {
                    if (type == "single")
                        ...
                        predayObj._changeUserStatus("startday", false);
                ...
                if (dayObj)
                {
                    if (flag)
                    {
                        if (type == "single")
                            ...
                            dayObj._changeUserStatus("startday", true);
                    }
                    else
                    {
                        if (type == "single")
                            ...
                            dayObj._changeUserStatus("startday", false);
    ...
    _pDatePickerControlEx._refreshBodyEndDay = function ()
    {
        ...
        var group = this._p_parent;
        var type = "single";
        var groupstartdate, groupenddate;
        if (group)            
        {
            type = group._p_type;
            groupstartdate = group._p_startdate;
            groupenddate = group._p_enddate;
        }    
        ...
        if (this._endvalue)
        {
            ...
            if (item)
            {
                ...
                else if (groupenddate)
                {
                    if (groupstartdate != groupenddate)
                        item._changeUserStatus("endday", this._p_userange);
                    else 
                        item._changeUserStatus("selected", this._p_userange);
                }
            } 
    ...
    _pDatePickerControlEx._setType = function (type)
    {
        this._type = type;
    };
