...
if (!nexacro.Sketch)
{
    ...
    _pSketch.on_attach_contents_handle = function (win, enablea11y)
    {
        var canvas_elem_bg = this._canvas_elem_bg;
        var canvas_elem = this._canvas_elem;
        var canvas_elem_tmp = this._canvas_elem_tmp;
    ...
    _pSketch._restoreSketch = function ()
    {
        ...
        if (img_obj)
        {
            ...
            if (img_type == "url")
            {
                ...
                url = this._getEnvironment()?._getImageLocation(url, this._getRefFormBaseUrl());
            }
            else
            {
    ...
    */
    ...
    _pSketch._moveAction = function (button, clientX, clientY)
    {
        ...
        if (button == "lbutton" || button == "touch")
        {
            ...
            clientY -= nexacro._getCurrentBodyScrollTop(this._isAllowDefaultPinchZoom());
            if (button == "touch")
            {
    ...
    {
        if (nexacro._isTouchInteraction && nexacro._SupportTouch && !(this._getEnvironment()?._p_enabletouchevent))
        {
    ...
    */
    ...
    _pSketch._loadSketch = function (url)
    {
        ...
        if (url)
        {
            if (img_type == "url")
            {
                ...
                url = this._getEnvironment()?._getImageLocation(url, this._p_parent._getRefFormBaseUrl());

                if (url && nexacro._OS == "iOS" && nexacro._isHybrid())
                {
                    var params = {url:url};
                    var json = {
                        div: "sketch",
        ...
        nexacro._getImageSize_extended(url, this._imageload, this);
    ...
    _pSketch._getClientXY = function (canvasX, canvasY)
    {
        ...
        var clientY = canvasY - border_top - nexacro._getCurrentBodyScrollTop(this._isAllowDefaultPinchZoom());
    ...
    _pImageSketchBackground.set_src = function (v)
    {
        ...
        {
            this._p_src = v;
            var v = nexacro._getURIValue(v);
            var sketch = this._p_parent;
            v = sketch._getEnvironment()?._getImageLocation(v, sketch._getRefFormBaseUrl());
            this.handle = nexacro._getImageObject_extended(v, this.on_load, this);
            if (this.handle)
            {
                return true;
            }
