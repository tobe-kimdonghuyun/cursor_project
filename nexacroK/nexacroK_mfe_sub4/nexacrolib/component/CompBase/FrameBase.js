...
if (!nexacro.Frame)
{
    ...
    _pFrame.on_apply_font = function (font)
    {
        ...
        // Frame은 font 변경되면 하위 Frame의 rem, em position 갱신
        if (this._p_frames)
        {
            // frameset
        ...
        // rem 단위 속성 재계산: MainFrame font 변경 (하위 Frame, Form, Component 순 전파)
        this._reapplyRemValues();
    };

    _pFrame._reapplyRemValues = function ()
    {
        if (this._p_frames)
        {
            var frames = this._p_frames;
            ...
            {
                if (frames[i] && this._getWindow() == frames[i]._getWindow())
                    frames[i]._reapplyRemValues();
            }
        }
        else if (this._p_frame)
            this._p_frame._reapplyRemValues();
    ...
    _pFrame.addChild = function (id, obj)
    {
        ...
        obj._p_parent = this;   
        if (obj._window_type != 2)   // modeless window는 생성하면서 설정되므로 별도로 설정하지 않는다. modalwindow 확인 필요
            obj._env = this._env;
    ...
    _pFrame._canDragResize = function ()
    {
        ...
        if (this._is_window) // TODO check! Resize Window
            return false;
    ...
    _pMainFrame.on_created_contents = function (win)
    {
        ...
        if (control_element)
        {
            ...
            control_element.setElementRtl(env?._rtl || false);
        }
    ...
    _pChildFrame.set_formurl = function (v)
    {
        ...
        if (!env) 
        {
            ...
            //trace("FrameBase._getEnvironment() is not found");
            return;
        }
    ...
    _pChildFrame._createForm = function ()
    {
        ...
        const env = this._getEnvironment();
        if (!this._formurl && formurl && env)
            this._formurl = env._getFDLLocation(formurl);
    ...
    */
    ...
    _pFrameSet.on_apply_tabbaritemheight = function ()
    {
        if (this._tabbar)
        {
            var v = this._p_tabbaritemheight;
            this._tabbar.set_itemheight(nexacro._isEmRemUnit(v) ? this._resolveEmPx(v) : v);
        }
        this._repositionTabBar();
    };
    ...
    _pFrameSet.on_apply_tabbaritemminheight = function ()
    {
        if (this._tabbar)
        {
            var v = this._p_tabbaritemminheight;
            this._tabbar.set_itemminheight(nexacro._isEmRemUnit(v) ? this._resolveEmPx(v) : v);
        }
        this._repositionTabBar();
    };
    ...
    _pFrameSet.on_apply_tabbaritemmaxheight = function ()
    {
        if (this._tabbar)
        {
            var v = this._p_tabbaritemmaxheight;
            this._tabbar.set_itemmaxheight(nexacro._isEmRemUnit(v) ? this._resolveEmPx(v) : v);
        }
        this._repositionTabBar();
    };
    ...
    _pFrameSet.on_apply_tabbaritemwidth = function ()
    {
        if (this._tabbar)
        {
            var v = this._p_tabbaritemwidth;
            this._tabbar.set_itemwidth(nexacro._isEmRemUnit(v) ? this._resolveEmPx(v) : v);
        }
        this._repositionTabBar();
    };
    ...
    _pFrameSet.on_apply_tabbaritemminwidth = function ()
    {
        if (this._tabbar)
        {
            var v = this._p_tabbaritemminwidth;
            this._tabbar.set_itemminwidth(nexacro._isEmRemUnit(v) ? this._resolveEmPx(v) : v);
        }
        this._repositionTabBar();
    };
    ...
    _pFrameSet.on_apply_tabbaritemmaxwidth = function ()
    {
        if (this._tabbar)
        {
            var v = this._p_tabbaritemmaxwidth;
            this._tabbar.set_itemmaxwidth(nexacro._isEmRemUnit(v) ? this._resolveEmPx(v) : v);
        }
        this._repositionTabBar();
    };
    ...
    _pFrameSet.on_apply_tabbaritemgap = function ()
    {
        if (this._tabbar)
        {
            var v = this._p_tabbaritemgap;
            this._tabbar.set_itemgap(nexacro._isEmRemUnit(v) ? this._resolveEmPx(v) : v);
        }
    };
    ...
    _pFrameSet.on_apply_tabbaritempinbuttonsize = function ()
    {
        if (this._tabbar)
        {
            var v = this._p_tabbaritempinbuttonsize;
            this._tabbar.set_itempinbuttonsize(nexacro._isEmRemUnit(v) ? this._resolveEmPx(v) : v);
        }
        this._repositionTabBar();
    };
    ...
    _pFrameSet.on_apply_tabbaritemclosebuttonsize = function ()
    {
        if (this._tabbar)
        {
            var v = this._p_tabbaritemclosebuttonsize;
            this._tabbar.set_itemclosebuttonsize(nexacro._isEmRemUnit(v) ? this._resolveEmPx(v) : v);
        }
        this._repositionTabBar();
    };
    ...
    _pFrameSet.on_apply_tabbarbuttongap = function ()
    {
        if (this._tabbar)
        {
            var v = this._p_tabbarbuttongap;
            this._tabbar.set_buttongap(nexacro._isEmRemUnit(v) ? this._resolveEmPx(v) : v);
        }
    };
    ...
    _pFrameSet.on_apply_tabbarbuttonseparatorsize = function ()
    {
        if (this._tabbar)
        {
            var v = this._p_tabbarbuttonseparatorsize;
            this._tabbar.set_buttonseparatorsize(nexacro._isEmRemUnit(v) ? this._resolveEmPx(v) : v);
        }
    };

    _pFrameSet.on_apply_font = function (v)
    {
        nexacro.Frame.prototype.on_apply_font.call(this, v);
        this._reapplyTabbarEmRemValues();
    };

    _pFrameSet._reapplyTabbarEmRemValues = function ()
    {
        var props = [
            "tabbarsize",
            "tabbaritemheight", "tabbaritemminheight", "tabbaritemmaxheight",
            "tabbaritemwidth", "tabbaritemminwidth", "tabbaritemmaxwidth",
            "tabbaritemgap", "tabbarbuttongap", "tabbarbuttonseparatorsize",
            "tabbaritempinbuttonsize", "tabbaritemclosebuttonsize"
        ];
        var i, pv;
        for (i = 0; i < props.length; i++)
        {
            pv = this["_p_" + props[i]];
            if (pv !== undefined && pv !== null && nexacro._isEmRemUnit(pv))
                this["on_apply_" + props[i]]();
        }
    };
    ...
    _pFrameSet._adjustTabBarPosition = function (width, height)
    {
        ...
        var tabbarsize_v = this._p_tabbarsize;
        const tabbarsize = (tabbarsize_v !== undefined && tabbarsize_v !== null && nexacro._isEmRemUnit(tabbarsize_v))
            ? this._resolveEmPx(tabbarsize_v) : tabbarsize_v;
