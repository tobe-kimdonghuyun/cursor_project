...
if (!nexacro.SpinnerControl)
{
    ...
    _pSpinnerControl._use_translate_scroll = true;
    ...
    _pSpinnerControl.on_attach_contents_handle = function (win, enablea11y)
    {
        for (var i = 0; i < this._list.length; i++)
            this._list[i].attachHandle(win, enablea11y);
    };
    ...
    _pSpinnerControl._setValues = function (values)
    {
        ...
        if (displaycnt > values.length)
        {
            ...
            {
                var n = displaycnt;
                var h = this._height / (this._disprowcnt - 2);
