...
if (!nexacro.CellInfo)
{
    ...
    nexacro.__createCellInfoSetter = function (prop, bind, type, styl, enumVals)
    {
        const propKey = "_p_" + prop;
        if (bind)
        {
            if (type === 0)
            {
                return function set_bind_int(v)
                {
                    if (v != this[propKey]._value)
                    {
                        this[propKey]._set_intval(v);
                        this._afterProcBindTypeProp();
                        if (styl)
                        {
                            this._changedStyle(prop);
                        }
                    }
                };
            }
            if (type === 2)
            {
                const enums = enumVals; // closure capture
                return function set_bind_enum(v)
                {
                    if (v != this[propKey]._value)
                    {
                        this[propKey]._set_enumval(v, enums);
                        this._afterProcBindTypeProp();
                        if (styl) 
                        {
                            this._changedStyle(prop);
            ...
            return function set_bind_default(v) { 
                if (v != this[propKey]._value)
                {
                    this[propKey]._set(v);
                    this._afterProcBindTypeProp();
        ...
        if (type === 2)
        {
            const enumSet = Object.create(null);
            for (const enumVal of enumVals)
            {
                enumSet[enumVal] = 1;
            }

            return function set_plain_enum(v)
            {
                let val = this[propKey];
                if (enumSet[v])
                {
                    val = v;
                }
    ...
    {
        const proto = (typeof ptype_val === "object")
            ? ptype_val
            : (target || null);  // 문자열 모드: target 필수 (_pCellInfo 등)
        ...
        const properties = [];
        for (const _property of _property_map)
        {
            const prop = _property[nexacro._CELLINFO_PMAP_PROPNAME];
            const bind = _property[nexacro._CELLINFO_PMAP_EXPRBIND];
            const type = _property[nexacro._CELLINFO_PMAP_VALTYPE];
            const styl = _property[nexacro._CELLINFO_PMAP_STYLE];
            const enumVals = _property[nexacro._CELLINFO_PMAP_ENUMVAL];

            proto["set_" + prop] = nexacro.__createCellInfoSetter(prop, bind, type, styl, enumVals);
            properties.push({ name: prop });
        }
        proto._properties = proto._properties.concat(properties);  // 기존과 동일: 배치 단위로 대입
    };

    nexacro._makeCellInfoPropertySetter("_pCellInfo", _pCellInfo._property_map, _pCellInfo);

    nexacro.__createCellInfoDesignGetter = function (prop)
    {
        const propKey = "_p_" + prop;
        return function design_get_prop()
        {
            const propval = this._refinfo[propKey];
            if (propval)
            {
                if (nexacro._isNumber(propval) || propval._bindtype == undefined)
                {
                    return propval;
                }
                return propval._value;
            }
            return propval;
        };
    };
    nexacro.__createCellInfoDesignSetter = function (prop)
    {
        const setterName = "set_" + prop;
        return function design_set_prop(v)
        {
            try
            {
                this._refinfo[setterName](v);
                this._updateAll();
                this._apply_normalproperty(prop, v);
            }
            catch (e)
            {
                if (e.obj)
                ...
                {
                    trace(nexacro._getExceptionMessage(e));
                }
            }
        };
    };
    ...
    {
        const proto = (typeof ptype_val === "object") ? ptype_val : (target || null);
        if (!proto)
        {
            return;
        }
        ...
        {
            const prop = _property[nexacro._CELLINFO_PMAP_PROPNAME];
            proto["design_set_" + prop] = nexacro.__createCellInfoDesignSetter(prop);
            proto["design_get_" + prop] = nexacro.__createCellInfoDesignGetter(prop);
        }
    };
...
if (!nexacro._CellControl)
{
    ...
    _pCellButton._getComputedStyle = function ()
    {
        ...
        ctrl_cs.styles = nexacro._parseJsonLiteral(str);
    ...
    _pCellBar._getComputedStyle = function ()
    {
        ...
        if (this.progressbaritem)
        {
            ...
            ctrl_cs.progressbaritem.styles = nexacro._parseJsonLiteral(str);
        }
    ...
    _pCellCalendar._getComputedStyle = function ()
    {
        ...
        if (this.calendaredit)
        {
            ...
            ctrl_cs.calendaredit.styles = nexacro._parseJsonLiteral(str);
        }
        if (this.dropbutton)
        {
            str = nexacro._getComputedStyleProperties(this.dropbutton._control_element, properties, null, true);
            ctrl_cs.dropbutton.styles = nexacro._parseJsonLiteral(str);
        }
        if (this.calendarspinupbutton)
        {
            ...
            ctrl_cs.calendarspinupbutton.styles = nexacro._parseJsonLiteral(str);
        }
        if (this.calendarspindownbutton)
        {
            ...
            ctrl_cs.calendarspindownbutton.styles = nexacro._parseJsonLiteral(str);
        }
    ...
    _pCellCombo._getComputedStyle = function ()
    {
        ...
        if (this.comboedit)
        {
            ...
            ctrl_cs.comboedit.styles = nexacro._parseJsonLiteral(str);
        }
        if (this.dropbutton)
        {
            str = nexacro._getComputedStyleProperties(this.dropbutton._control_element, properties, null, true);
    ...
    _pCellMultiCombo._getComputedStyle = function ()
    {
        ...
        if (this.multicomboedit)
        {
            ...
            ctrl_cs.multicomboedit.styles = nexacro._parseJsonLiteral(str);
        }
        if (this.dropbutton)
        {
            str = nexacro._getComputedStyleProperties(this.dropbutton._control_element, properties, null, true);
