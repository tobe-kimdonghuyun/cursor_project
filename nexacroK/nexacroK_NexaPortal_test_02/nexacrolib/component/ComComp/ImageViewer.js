...
if (!nexacro.ImageViewer)
{
    ...
    _pImageViewer.on_attach_contents_handle = function (win, enablea11y)
    {
        if (this._image)
        {
            this._image.attachHandle(win, enablea11y);
        }

        if (this._imagetext)
        {
            this._imagetext.attachHandle(win, enablea11y);
        }
           ...
           /* var image_width = this._image.width;
            ...
            if (image_url)
            {
                if (this._image._getImageType() == "url")
                {
                    ...
                    image_url = this._getEnvironment()?._getImageLocation(image_url, this._getRefFormBaseUrl());
                }
            }
    ...
    _pImageAreaControl.on_attach_contents_handle = function (win, enablea11y)
    {
        var img_elem = this._img_elem;
        if (img_elem)
        {
            ...
            if (img_elem)
            {
                if (v)
                {
                    if (this._getImageType() == "url")
                    {
                        ...
                        v = this._getEnvironment()?._getImageLocation(v, this._getRefFormBaseUrl());
                    }
                    
                    let image_size = nexacro._getImageSize_extended(v, this._on_load_image, this); //NRE getstretchedmode
                    ...
                    {
                        image_size = nexacro._getImageSize_extended(v, this._on_load_image, this);
                    }*/
    ...
    _pImageAreaControl._load_image_completed = function (url, w, h)
    {
        ...
        if (img_elem)
        {
            ...
            else
            {
                ...
                if (bChange)
                {
                    url = nexacro.UrlObject(url, this);
