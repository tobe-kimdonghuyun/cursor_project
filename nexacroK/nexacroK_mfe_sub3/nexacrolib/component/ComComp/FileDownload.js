...
if (!nexacro.FileDownload)
{
    ...
    _pFileDownload.download = function (url, targetpath)
    {
        ...
        if (url != undefined)
        {
            ...
            url = env._getImageLocation(url);

            ret = nexacro._download(url, is_popup_frame, this._hidden_frame_handle, initname, targetpath, this._p_filefilter, this._p_filefilterindex, env);
        }
        else if (downloadurl != undefined && downloadurl != "")
        {
            downloadurl = env._getServiceLocation(downloadurl);
            ret = nexacro._download(downloadurl, is_popup_frame, this._hidden_frame_handle, initname, targetpath, this._p_filefilter, this._p_filefilterindex, env);
        }
    ...
    //_pFileDownload.on_fire_onclick -> on_click_basic_action
    _pFileDownload.on_click_basic_action = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, meta_key)
    {
        this.download();
        return true;
    };
