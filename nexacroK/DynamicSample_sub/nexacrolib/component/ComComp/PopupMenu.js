...
if (!nexacro._PopupMenuItemControl)
{
	...
	var _pPopupMenuItemControl = nexacro._createPrototype(nexacro.Component, nexacro._PopupMenuItemControl);
	...
	_pPopupMenuItemControl._use_selected_status = true;
	...
	_pPopupMenuItemControl.navtarget = "";
	_pPopupMenuItemControl.navurl = "";
	_pPopupMenuItemControl.navtargetproperties = undefined;
	...
	_pPopupMenuItemControl.on_attach_contents_handle = function (win, enablea11y)
	{
		if (this._textcontrol)
		{
			this._textcontrol.attachHandle(win, enablea11y);
		}

		if (this._hotkeytextcontrol)
		{
			this._hotkeytextcontrol.attachHandle(win, enablea11y);
		}

		if (this._iconcontrol)
		{
			this._iconcontrol.attachHandle(win, enablea11y);
		}

		if (this._expiconcontrol)
		{
			this._expiconcontrol.attachHandle(win, enablea11y);
		}
	};
	...
	_pPopupMenuItemControl.on_tap_basic_action = function (elem, canvasX, canvasY, screenX, screenY, refer_comp)
	{
	...
	_pPopupMenuItemControl._on_getFitSize = function ()
	{
	...
	_pPopupMenuItemControl._apply_setfocus = function (evt_name)
	{
		// 가장 말단 element에 focus 호출.
		var control_elem = this._control_element;
		if (control_elem)
		{
			var selffocus = ((evt_name == "lbutton") ? false : this._getEnvironment()?._p_enableaccessibility === true);
	...
	_pPopupMenuItemControl._on_dragleave = function (elem, to_comp, src_comp, src_refer_comp, dragdata, userdata,
	...
	_pPopupMenuItemControl._getTextSize = function (text)
	{
	...
	_pPopupMenuItemControl._on_apply_selected = function (bflag)
	{
		this._changeUserStatus("selected", bflag);
	};

	_pPopupMenuItemControl._getStringResourceProperty = function (v)
	{
		return v;
	};
	...
	_pPopupMenuItemControl._isSelected = function ()
	{
...
if (!nexacro.PopupMenu)
{
	...
	_pPopupMenu._p_navtargetcolumn = "";
	_pPopupMenu._p_navurlcolumn = "";
	_pPopupMenu._p_navtargetpropertiescolumn = "";
	/* internal variable */
	...
	_pPopupMenu._resolved_itemheight = undefined;
	...
	_pPopupMenu.on_attach_contents_handle = function (win, enablea11y)
	{
	...
	_pPopupMenu.set_itemheight = function (v)
	{
		if (v !== undefined)
		{
			...
			{
				this._resolved_itemheight = this._resolveEmPx(v);
			}
			else
			{
				if (isNaN(v = +v) || v < 0)
				...
				this._resolved_itemheight = undefined;
			}
		}
		else
		{
	...
	_pPopupMenu.on_apply_font = function (font)
	{
	...
	_pPopupMenu._reapplyRemValues = function ()
	{
		var v = this._p_itemheight;
		if (v !== undefined && nexacro._isEmRemUnit(v))
		{
	...
	_pPopupMenu.set_navtargetcolumn = function (v)
	{
		if (v != this._p_navtargetcolumn)
		{
			this._p_navtargetcolumn = v;
			this.on_apply_navtargetcolumn();
		}
	};
	
	_pPopupMenu.on_apply_navtargetcolumn = function ()
	{
		var popupmenu = this._popupmenu;
		if (popupmenu)
		{
			popupmenu.set_navtargetcolumn(this._p_navtargetcolumn);
		}
		
	};
	
	_pPopupMenu.set_navurlcolumn = function (v)
	{
		if (v != this._p_navurlcolumn)
		{
			this._p_navurlcolumn = v;
			this.on_apply_navurlcolumn();
		}
	};
	
	_pPopupMenu.on_apply_navurlcolumn = function ()
	{
		var popupmenu = this._popupmenu;
		if (popupmenu)
		{
			popupmenu.set_navurlcolumn(this._p_navurlcolumn);
		}
		
	};
	
	_pPopupMenu.set_navtargetpropertiescolumn = function (v)
	{
		if (v != this._p_navtargetpropertiescolumn)
		{
			this._p_navtargetpropertiescolumn = v;
			this.on_apply_navtargetpropertiescolumn();
		}
	};
	
	_pPopupMenu.on_apply_navtargetpropertiescolumn = function ()
	{
		var popupmenu = this._popupmenu;
		if (popupmenu)
		{
			popupmenu.set_navtargetpropertiescolumn(this._p_navtargetpropertiescolumn);
		}
	...
	{ name: "userdatacolumn" }, { name: "navtargetcolumn" }, { name: "navurlcolumn" }, { name: "navtargetpropertiescolumn" }
	];
	...
	{
		var enableaccessibility = this._getEnvironment()?._p_enableaccessibility === true;
	...
	_pPopupMenu.on_fire_sys_onkeydown = function (keycode, alt_key, ctrl_key, shift_key, fire_comp, refer_comp, meta_key)
	{
		...
		var rootComp = this._getRootComponent(this);
		var E = nexacro.Event;
		var is_accessibility = this._getEnvironment()?._p_enableaccessibility === true;
	...
	_pPopupMenu.on_fire_onitemclick = function (obj, itemid, itemuserdata, index, level, refobj)
	{
		...
		if (rootComp.onmenuclick && rootComp.onmenuclick._has_handlers)
		{
			...
			if (rootComp.onmenuclick.defaultprevented) return;
		}

		if (refobj)
		{
			let row = refobj.datarow;
			let ds = this._innerdataset;
			if (ds)
			{
				let navtarget = ds.getColumn(row, this._p_navtargetcolumn);
				let navurl = ds.getColumn(row, this._p_navurlcolumn);
				let navtargetproperties = ds.getColumn(row, this._p_navtargetpropertiescolumn);
				rootComp.on_click_default_action(navtarget, navurl, navtargetproperties);
			}
		}
	};
	...
	_pPopupMenu.trackPopup = function (x, y, align, bcapture)
	{
		...
		{
			x *= nexacro._setzoomfactor;
			y *= nexacro._setzoomfactor;
		}
	...
	_pPopupMenu.trackPopupByComponent = function (obj, x, y, align, bcapture)
	{
		...
		if (nexacro._setzoomfactor)
		{
			win_top *= nexacro._setzoomfactor;
		}
		...
		this._popupBy(obj, x, y, width, height);
		this.setFocus();
		if (!this._is_fire_onpopup)
			this.on_fire_onpopup(this);
	};
	...
	_pPopupMenu._createPopupMenu = function ()
	{
		...
		if (control_elem)
		{
			...
			if (ds && this._p_levelcolumn && this._p_captioncolumn && this._p_idcolumn)
			{
				...
				var top = 0;
				var height = (this._resolved_itemheight !== undefined ? this._resolved_itemheight : this._p_itemheight) || 0;
	...
	_pPopupMenu._closePopup = function ()
	{
		...
		if (this._getEnvironment()?._p_enableaccessibility === true)
		{
	...
	_pPopupMenu._reCalcSize = function (/*w,h*/)
	{
		...
		if (ds && this._p_captioncolumn)
		{
			...
			var item_h = (this._resolved_itemheight !== undefined) ? this._resolved_itemheight : this._p_itemheight;
	...
	_pPopupMenu._createNavigationButton = function ()
	{
		...
		if (previcon)
		{
			var previconfullurl = nexacro._getURIValue(previcon.url);    
			previconfullurl = this._getEnvironment()?._getImageLocation(previconfullurl, this._getRefFormBaseUrl());
			img_size = nexacro._getImageSize_extended(previconfullurl, this._rearrangePopupMenuItems, this);
			if (img_size)
			{
		...
		if (nexticon)
		{
			var nexticonfullurl = nexacro._getURIValue(nexticon.url);    
			nexticonfullurl = this._getEnvironment()?._getImageLocation(nexticonfullurl, this._getRefFormBaseUrl());
			img_size = nexacro._getImageSize_extended(nexticonfullurl, this._rearrangePopupMenuItems, this);
			if (img_size)
			{
				this._navigationbuttonsize = img_size.height;
				this._rearrangePopupMenuItems();
	...
	_pPopupMenu._load_image = function (strImageUrl, strflag)
	{
		...
		if (control_elem)
		{
			...
			if (val)
			{
				...
				val = this._getEnvironment()?._getImageLocation(val, this._getRefFormBaseUrl());

				var size;
				if (strflag == "exp") //expimg
				{
					size = nexacro._getImageSize_extended(val, this._loaded_expImage, this);
					if (size)
					{
				...
				{
					size = nexacro._getImageSize_extended(val, this._loaded_chkImage, this);
					if (size)
					{
				...
				{
					size = nexacro._getImageSize_extended(val, this._loaded_iconImage, this);
					if (size)
					{
	...
	_pPopupMenu._getNavigationbuttonHeight = function (nextbutton, prevbutton)
	{
		...
		if (nexticon)
		{
			var nexticonfullurl = nexacro._getURIValue(nexticon.url);    
			nexticonfullurl = this._getEnvironment()?._getImageLocation(nexticonfullurl, this._getRefFormBaseUrl());
			img_size = nexacro._getImageSize_extended(nexticonfullurl, nexacro._emptyFn, this);
			if (img_size)
			{
		...
		if (previcon)
		{
			var previconfullurl = nexacro._getURIValue(previcon.url);    
			previconfullurl = this._getEnvironment()?._getImageLocation(previconfullurl, this._getRefFormBaseUrl());
			img_size = nexacro._getImageSize_extended(previconfullurl, nexacro._emptyFn, this);
			if (img_size)
			{
	...
	_pPopupMenu._getControlInfo = function ()
	{
		...
		for (var i = 0, len = items.length; i < len; i++)
		{
			...
			if (icon_control)
			{
				...
				if (icon)
				{
					var iconfullurl = nexacro._getURIValue(icon.value);    
					iconfullurl = this._getEnvironment()?._getImageLocation(iconfullurl, this._getRefFormBaseUrl());
					var iconsize = nexacro._getImageSize_extended(iconfullurl, this._loaded_iconImage, this);
					if (iconsize)
					{
			...
			if (expiconcontrol)
			{
				...
				if (expicon)
				{
					var expiconfullurl = nexacro._getURIValue(expicon.value);    
					expiconfullurl = this._getEnvironment()?._getImageLocation(expiconfullurl, this._getRefFormBaseUrl());
					var expsize = nexacro._getImageSize_extended(expiconfullurl, this._loaded_expImage, this);
					if (expsize)
					{
