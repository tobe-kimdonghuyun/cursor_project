...
if (!nexacro.TextArea)
{
    ...
    _pTextArea.on_create_contents = function ()
    {
        ...
        if (control)
        {
            ...
            if (!this._onlydisplay)
            {
                ...
                //input_elem.setElementScrollType(this._p_scrolltype);
    ...
    _pTextArea.on_attach_contents_handle = function (win, enablea11y)
    {
        var input_elem = this._input_element;
        if (input_elem)
        {
    ...
    _pTextArea._getDlgCode = function (keycode, altKey, ctrlKey, shiftKey)
    {
        ...
        if (keycode >= nexacro.Event.KEY_LEFT && keycode <= nexacro.Event.KEY_DOWN)
        {
            if (!this._onlydisplay)
            {
                ...
                if (!is_enable || !elem)
                {
                    want_arrow = false; // disable, no input
                }
                else 
                {
                    // legacy 접근성 키액션 아닌 경우 비접근성과 아닌 키액션이 동일해야한다.
                    var accessibility_legacykey = env && env._p_enableaccessibility && env._p_accessibilitycontentsearchkey;

                    var vscrollbar = (this._p_scrolltype != "none" && this._p_scrolltype != "horizontal") ? this._p_vscrollbar : null;
                    var hscrollbar = (this._p_scrolltype != "none" && this._p_scrolltype != "vertical") ? this._p_hscrollbar : null;

                    var vpos = vscrollbar ? vscrollbar._pos : 0;
                    var hpos = hscrollbar ? hscrollbar._pos : 0;

                    if (keycode == nexacro.Event.KEY_LEFT)
                    {
                        var hmin = hscrollbar ? hscrollbar._min : 0;
                        want_arrow = (is_readonly && (ctrlKey || hpos == hmin)) ? false : true;
                    }
                    else if (keycode == nexacro.Event.KEY_RIGHT)
                    {
                        var hmax = hscrollbar ? hscrollbar._max : 0;
                        want_arrow = (is_readonly && (ctrlKey || hpos == hmax)) ? false : true;
                    }
                    else if (keycode == nexacro.Event.KEY_UP)
                    {
                        if (accessibility_legacykey)
                        {
                            want_arrow = (is_readonly || elem.isFirstCaretLine()) ? false : true;
                        }
                        else
                        {
                            var vmin = vscrollbar ? vscrollbar._min : 0;
                            want_arrow = ((is_readonly && (ctrlKey || vpos == vmin)) || (ctrlKey && elem.isFirstCaretLine())) ? false : true;
                        }
                    }
                    else if (keycode == nexacro.Event.KEY_DOWN)
                    {
                        if (accessibility_legacykey)
                        {
                            want_arrow = (is_readonly || elem.isLastCaretLine()) ? false : true;
                        }
                        else
                        {
                            var vmax = vscrollbar ? vscrollbar._max : 0;
                            want_arrow = ((is_readonly && (ctrlKey || vpos == vmax)) || (ctrlKey && elem.isLastCaretLine())) ? false : true;
                        }
                    }
                }
            }
    ...
    _pTextArea.on_apply_scrollpropagation = function ()
    {
        var input_elem = this._input_element;
        if (input_elem)
        {
    ...
    { name: "cursor" }, { name: "readonly" }, { name: "value" }, { name: "text" }, { name: "scrollpropagation" }
    ];
    ...
    _pTextArea._useNativeWheelScroll = function()
    {
        return true;
    };
