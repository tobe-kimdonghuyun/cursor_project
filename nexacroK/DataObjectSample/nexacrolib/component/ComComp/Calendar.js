...
if (!nexacro.Calendar)
{
	...
	_pCalendar._buttonsize = undefined;
	_pCalendar._headheight = undefined;
	_pCalendar._daysize = undefined;
	_pCalendar._popupsize = undefined;
	...
	_pCalendar.on_create_contents = function ()
	{
		...
		if (control)
		{
			...
			if (maskobj)
			{
				...
				var bMobile = ((nexacro._isMobile && nexacro._isMobile()) || (nexacro._isHybrid && nexacro._isHybrid()));
				if (bMobile)
				{
	...
	_pCalendar.on_created_contents = function (win)
	{
		...
		switch (this._type)
		{
			...
			case "monthonly":
				...
				break;
			case "system":
				if (env?._p_enableaccessibility && a11ytype == 4)
				{
	...
	_pCalendar.on_attach_contents_handle = function (win, enablea11y)
	{
		if (this._p_calendaredit)
		{
			this._p_calendaredit.attachHandle(win, enablea11y);
		}
		if (this._p_dropbutton)
		{
		...
		if (this._p_calendarspindownbutton)
		{
			this._p_calendarspindownbutton.attachHandle(win, enablea11y);
		}
		if (this._p_calendarspinupbutton)
		{
			this._p_calendarspinupbutton.attachHandle(win, enablea11y);
		}
		if (this._p_datepicker)
		{
			this._p_datepicker.attachHandle(win, enablea11y);
		}
	...
	_pCalendar._getDlgCode = function (keycode, altKey, ctrlKey/*, shiftKey*/)
	{
		...
		//if (this._getEnvironment()?._p_enableaccessibility &&  keycode == 27)
		//    _want_arrows = true;
		if (ctrlKey == true)
		{
			_want_arrows = true;
	...
	_pCalendar.set_buttonsize = function (v)
	{
		if (this._p_buttonsize != v)
		{
			...
			if (nexacro._isEmRemUnit(v))
				this._buttonsize = this._resolveSizeStr(v);
			else
	...
	{
		this._headheight = nexacro._isEmRemUnit(this._p_headheight) ? this._resolveEmPx(this._p_headheight) : undefined;
		var datepicker = this._p_datepicker;
		if (datepicker)
		{
	...
	_pCalendar.set_daysize = function (v)
	{
		if (this._p_daysize != v)
		{
			this._p_daysize = v;
			if (nexacro._isEmRemUnit(v))
				this._daysize = this._resolveSizeStr(v);
			else
				this._daysize = undefined;
	...
	_pCalendar.set_popupsize = function (v)
	{
		if (this._p_popupsize != v)
		{
			this._p_popupsize = v;
			if (nexacro._isEmRemUnit(v))
				this._popupsize = this._resolveSizeStr(v);
			else
				this._popupsize = undefined;
		}
	};

	_pCalendar.on_apply_popupsize = nexacro._emptyFn;

	_pCalendar.on_apply_font = function (font)
	{
		nexacro.Component.prototype.on_apply_font.call(this, font);
		this._reapplyRemValues();
	};

	_pCalendar._reapplyRemValues = function ()
	{
		if (!this._isEnableRedraw()) return;
		var v;
		v = this._p_headheight;
		if (nexacro._isEmRemUnit(v))
			this.on_apply_headheight(v);
		v = this._p_buttonsize;
		if (nexacro._isEmRemUnit(v))
		{
			this._buttonsize = this._resolveSizeStr(v);
	...
	{
		var type_enum = ["auto", "combo", "button"];
		if (type_enum.indexOf(v) == -1)
		{
			return;
		}
	...
	{ name: "usetrailingday" },{ name: "visible" }, { name: "calendaredit", readonly: true }, { name: "calendarspindownbutton", readonly: true }, { name: "calendarspinupbutton", readonly: true }, { name: "datepicker", readonly: true }, { name: "dropbutton", readonly: true }
	];
			...
			if (obj instanceof nexacro.MaskEdit)
			{
				if (nexacro._OS == "iOS" && nexacro._Browser == "MobileSafari")
				{
					...
					if (ios_ver < 15)
					{
						...
						const enableaccessibilty = this._getEnvironment()?._p_enableaccessibility;
	...
	_pCalendar._on_edit_onkeydown = function (obj, e)
	{
		...
		switch (this._p_type)
		{
			...
			case "spin":
				...
				{
					if (!this._getEnvironment()?._p_enableaccessibility || e.ctrlkey)
					{
...
//		return false;
	...
	_pCalendar._setFocusToNextComponent = function ()
	{
		...
		{
			next_comp.setFocus();
			if (!next_comp._p_autoselect && next_comp._isEditableComponent())
	...
	_pCalendar._getButtonSize = function ()
	{
		return (this._buttonsize !== undefined) ? this._buttonsize : this._p_buttonsize;
	};

	_pCalendar._getHeadHeight = function ()
	{
		return (this._headheight !== undefined) ? this._headheight : this._p_headheight;
	};

	_pCalendar._getDaySize = function ()
	{
		return (this._daysize !== undefined) ? this._daysize : this._p_daysize;
	};

	_pCalendar._getPopupSize = function ()
	{
		return (this._popupsize !== undefined) ? this._popupsize : this._p_popupsize;
	};


	_pCalendar._getPopupSizeArr = function ()
	{
		var size = this._getPopupSize();
		if (!size)
		{
			size = "200 220";
		}
