...
if (!nexacro.MultiCombo)
{
    ...
    */
        ...
        if (this.on_fire_ondropdown(this))
        {
            ...
            if (multicombolist)
            {
                ...
                if (checkboxset)
                {
                    ...
                    var firstVisible = checkboxset._get_first_visible_row();
                    var bScrollChange = firstVisible >= rowcount;
                    checkboxset._redrawListBoxContents(bScrollChange);
                    checkboxset._onRecalcScrollSize();
    ...
    _pMultiCombo._clear_all = function (change_by_script, isNotFireEvent)
    {
        ...
        for (var i = selectedcount - 1; i >= 0; i--)
        {
            ...
            if (!this._getItemReadonly(idx))
            {
                if (checkboxset)
                {   
                    // RP 105748 : 필터 적용 시 체크박스 항목은 "보이는 뷰 위치"로 인덱싱되므로,
                    // 선택 목록의 raw 인덱스를 뷰 위치로 변환해 _getItem을 호출해야 시각적으로 해제된다.
                    var rawidx = this._convertToRawIndex(idx);
                    for (var j = 0, len = view_records.length;j < len;j++)
                    {
                        if (view_records[j]._rawidx == rawidx)
                        {
                            var item = checkboxset._getItem(j);
                            if (item)
                            {
    ...
    {   
		...
		for (var i = 0;i < rowcount;i++)
        {
            var rawidx = view_records[i]._rawidx;
            if (!this._getItemReadonly(rawidx))
			{
				if (checkboxset)
				{
    ...
    _pMultiCombo._selectAll = function ()
    {
        ...
        if (row_cnt > 0)
        {
            ...
            //  - 언체크(해제): 전체 선택 해제 (보이는 것만 해제하면 숨은 선택이 남아 혼란)
    ...
    _pMultiCombo._getViewSelectedCount = function ()
    {
        ...
        {
            if (this._select_indexOfkey(view_records[i]._rawidx) >= 0)
            {
                cnt++;
            }
        }
        ...
        if (!isNotFireEvent)
        {
            ...
            for (i = 0; i < rows.length; i++)
            {
                ...
                else
                {
                    postidx += "," + idx;
                    posttext += textseparator + txt;
                    postvalue = (postvalue === undefined ? "" : postvalue) + valueseparator + val;
                }
            }
    ...
    {
        var indexes = [];
        var ds = this._innerdataset; // filter 없는 기준으로 반환
        var column = this._p_codecolumn || this._p_datacolumn;
        ...
        for (var value of values)
        {
            ...
            {
                var rawindex = this._getRawIndex(ds, i);
                var v = this._getItemValue(rawindex);
                if (v instanceof nexacro.Decimal)
                {
                    v = v.toString();
                }

                // index가 다르고 value가 똑같은 경우 처리 조건 추가
                if (value == v && !used[rawindex])
                {
                    var newval = value;
                    var preval = v;
