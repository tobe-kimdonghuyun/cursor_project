...
if (!nexacro.Div)
{
    ...
    /*
    // min/max 범위로 값 보정
    // - 음수 min/max는 유효하지 않은 값으로 간주하고 무시
    // - min과 max 모두 적용
    _pDiv._clampToMinMax = function (value, min, max)
    {
        if (min != null && min >= 0)
        {
    ...
    */
    ...
    _pDiv._getSizeInfo = function ()
    {
        ...
        if (fit_type === "width" || fit_type === "both")
        {
                ...
                && this._p_width.indexOf("%") > -1)
            {
                ...
                if (containerComp)
                {
                    ...
                    if (refW > 0)
                    {
                        width = this._convToPixel(this._p_width, refW);
                    }
                }
            }
        ...
        if (fit_type === "height" || fit_type === "both")
        {
            ...
            {
                var containerComp2 = this._getParentComponent();
                if (containerComp2)
                {
                    var refH = containerComp2._basis_height || containerComp2._adjust_height;
                    if (refH > 0)
                    {
                        height = this._convToPixel(this._p_height, refH);
                    }
                }
            }
