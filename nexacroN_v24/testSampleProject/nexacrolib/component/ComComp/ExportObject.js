...
if (!nexacro.ExcelExportObject)
{
    ...
    _pExportItem._getCellStyle = function (cell, rowIdx, odd, sn, status, subcell)
    {
        var cellStyleinfo;
        ...
        switch (sn)
        {
            ...
            case "background":
                if (subcell)
                {
                    cellStyleinfo = subcell._query_status_background_of_subcell(rowIdx, userstatus, cell, true);
                }
                else
                {
                    ...
                    */
                    ...
                    if (cell._subcells.length)
                    {
                        ...
                        for (var i = 0; i < subL; i++)
                        {
                            ...
                            var backgroundSubCell = nexacro._nvl(this._getColortoRGB(this._getCellStyle(cell, k, odd, "background", selected, subCell[i])), "");
                            style_name = this._makeforDsStyle(ds_style2, alignCell, backgroundSubCell, colorCell, fontCell, subline, cell_type);
                    ...
                    */
                    ...
                    if (cell._subcells.length)
                    {
                        ...
                        for (var i = 0; i < subL; i++)
                        {
                            ...
                            cell_type = this._getFixedCellType(subCell[i], 0); //rowidx
                            var backgroundSubCell = nexacro._nvl(this._getColortoRGB(this._getCellStyle(cell, k, odd, "background", selected, subCell[i])), "");
                            style_name = this._makeforDsStyle(ds_style, alignCell, backgroundSubCell, colorCell, fontCell, subline, cell_type);
