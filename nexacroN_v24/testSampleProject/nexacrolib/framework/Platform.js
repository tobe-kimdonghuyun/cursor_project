...
if (!nexacro._bInitPlatform)
{
    ...
    */
    ...
    nexacro._on_no_bindOnceCallbackTimerHandler = function (pthis)
    {
        return function ()
        {
            if (pthis)
            {
                ...
                var target = pthis.target;
                if (target && target._is_alive !== false)
                {
                    pthis.callback.call(target, pthis.id);
                }
    ...
    nexacro.setTimeout = function (target, callback, interval)
    {
        if(target && target._getReferenceContext)
            return nexacro._OnceCallbackTimer.callonce(target, callback, interval);
        return nexacro._SysOnceCallbackTimer.callonce(target, callback, interval);
    };
    ...
    __pLoadManager.loadGlobalJsonResource = function (url, cache, async, service)
    {
        ...
        if (this.status < 7)
        {
            ...
            if (!load_item)
            {
                load_item = new nexacro._LoadItem(url, "json", null);
                this.globalList.push(load_item);
                this.globalCnt++;
                load_item.handle = nexacro._loadJsonResource(url, this, this.on_load_globaljson, service, async);
            }
        }
    };
    ...
    __pLoadManager.reloadJsonResource = function (url, cache, async, service)
    {
        ...
        var load_item = new nexacro._LoadItem(url, "json", this.context);


        load_item.handle = nexacro._loadJsonResource(url, this, this.on_load_json, cache, service, async);
    };
    ...
    __pLoadManager.on_load_globaljson = function (url, errstatus, module, fireerrorcode, returncode, requesturi, locationuri, extramsg)
    {
        var load_Item = this.getGlobalItem(url);
        if (!load_Item)
            return;
    ...
    __pLoadManager.on_load_datamodule = function (svcid, errstatus, message, fireerrorcode, returncode, requesturi, locationuri, extramsg, responsebody)
    {
        var load_Item = this.getDataItem(svcid);
        if (load_Item)
        {
            ...
            callback_func = callback_func ? callback_func : callback_id;
            ...
            if (errstatus < 0 && fireerrorcode)
            {
                    ...
                    load_Item._is_cancel || !load_Item.handle || (load_Item.handle && !load_Item.handle._user_aborted && load_Item.handle._user_aborted !== undefined))
                {
                    ...
                    ret = this.context._onHttpTransactionError(this.context, true, this.context, fireerrorcode, requesturi, returncode, requesturi, locationuri, extramsg, svcid, responsebody);
    ...
    */
    ...
    __pLayoutManager.createLayoutContainerInfo = function (objContext)
    {
        ...
        const layoutcontainerinfo = this._makeContainerInfo(objContext);
        this.container_info[level].push(layoutcontainerinfo);

        // 빠른 탐색을 위한 cache 처리
        objContext._layoutcontainerinfo = layoutcontainerinfo;
    };
    ...
    __pLayoutManager.destroyLayoutContainerInfo = function (objContext)
    {
        ...
        if (nLevel == 0 && this.container_info[nLevel] && this.container_info[nLevel].length == 1)
        {
            ...
            if (container_info)
            {
                ...
                objContext._layoutcontainerinfo = null;                
            }            
        }
        else if (nLevel >= 0)
        {
    ...
    __pLayoutManager._makeContainerInfo = function (objContainer)
    {
        var ret = {
            ...
            parentContext:  objContainer._is_group ? objContainer._group_panel : objContainer._p_parent._is_form ? objContainer._p_parent : objContainer._p_parent._getParentForm(),
    ...
    __pLayoutManager._removeSelfContainerInfo = function (objContainer)
    {
        ...
        for (var i = 0; i < infos.length; i++)
        {
            ...
            if (is_same)
            {
                ...
                objContainer._layoutcontainerinfo = null;
                break;
            }
        }
    ...
    __pLayoutManager._findContainerInfo = function (objContainer)
    {
        ...
        if (objContainer._layoutcontainerinfo) 
            return objContainer._layoutcontainerinfo;
    ...
    __pLayoutManager.createFlexLayoutContentsInfo = function (objContainerInfo)
    {
        ...
        for (i = 0; i < comps.length; i++)
        {
            ...
            if (!this._isContentsVisible(comp) || (!is_panel && comp._is_group) || comp._is_alive == false)
                continue;
    ...
    __pMultiLayoutManager.getValidLayoutObject = function (objContainerInfo, container_width, container_height)
    {
        ...
        for (var i = 0; i < layout_len; i++)
        {
            ...
            {
                if (curscreen_type != "desktop" && layout._p_mobileorientation && layout._p_mobileorientation != mobileorientation)
                {
                    continue;
                }
            }
    ...
    __pFluidLayoutManager.getLayoutArrangeType = function ()
    {
        ...
        //if (nexacro._is_native_fluidlayout === undefined)
        //    nexacro._is_native_fluidlayout = ret;
...
// ============================================================================
// diagnose: div01에 실제로 CSS가 어떤 값으로 설정되는지 추적
// ============================================================================
    ...
    __pWindow._postMessage = function (target_comp, id, func, userdata)
    {
        ...
        nexacro._postMessage(id, this, target_comp, nexacro._getTargetOrigin());
    };
    ...
    __pWindow._destroyInternalPart = function ()
    {
        ...
        if (this.frame)
        {
            ...
            if (!this.frame._is_main && this.frame._is_window && this.frame._window_type != 5)
    ...
    {
        if (nexacro._modallockstate && elem instanceof nexacro.ModalOverlayElement)
            return;
        ...
        else
        {
            if (comp)
            {
                ...
                else
                {
                    ...
                    var xy = nexacro._convertWindowXYToCanvasXY(comp._control_element.handle, windowX, windowY);
                    canvasX = xy[0];
                    canvasY = xy[1];
                }
        ...
        if (elem)
        {
            ...
            if (comp && comp != from_comp)
            {
                ...
                if (dragInfo && dragInfo.isDragging && dragInfo.targetwin == this)
                {
                    ...
                    if (!dragInfo.filelist && !nexacro._linux_app)
    ...
    __pWindow._isEmbeddedWindow = function ()
    {
        ...
        if (window)
        {
            ...
            try
            {
                is_embedded = window.parent.document != window.document;
            }
            catch (e)
            {
                // cross-origin 페이지에 iframe으로 embed된 경우 parent.document 접근 시 SecurityError
                is_embedded = true;
            }
            if (is_embedded)
            {
    ...
    __pWindow._setWheelZoom = function (newWheelZoom)
    {
        if ((50 <= newWheelZoom) && (newWheelZoom <= 200) && (newWheelZoom % 10 == 0))
        {
            ...
            zoomframeElement.setElementZoom(newWheelZoom, { source: 'wheel' });
            ...
            if (this._modal_frame_stack.length > 0)
            {
                ...
                for (var loopI = 0; loopI < modalCount; loopI++)
                {
                    ...
                    modalFrameElement.setElementZoom(newWheelZoom, { source: 'wheel' });
	...
	// window의 innersize(clientsize), outersize(adjustsize) 계산 반영
	__pWindow._setGapClientSize = function ()
	{
		var handle = this.handle;
		if (handle)
		{
			var adjust_width = this.width;
			var adjust_height = this.height;
			var adjust_client_width = this.clientWidth;
			var adjust_client_height = this.clientHeight;

            var zoom_ratio = nexacro._getDevicePixelRatio();
            if (zoom_ratio != 1 && (this instanceof nexacro._PopupWindow || handle.opener))
			{
				// wheelzoom 시 반영하여 계산
				if (nexacro._Browser == "Gecko")
				{
					adjust_width = parseInt(this.width / zoom_ratio);
					adjust_height = parseInt(this.height / zoom_ratio);
				}
				else
				{
					adjust_client_width = parseInt(this.clientWidth * zoom_ratio);
					adjust_client_height = parseInt(this.clientHeight * zoom_ratio);
				}
			}

			this._gap_client_width = adjust_width - adjust_client_width;
			this._gap_client_height = adjust_height - adjust_client_height;
		}
    };

    //==============================================================================
    // nexacro._PopupWindow
    //==============================================================================
    ...
    __pPopupWindow.attachHandle = function (handle)
    {
        if (!this.handle)
        {
            ...
            // popup은 열리면서 client에 zoom으로 반영해서 window 사이즈가 줄어드는 경우가 있음
            this._setGapClientSize();
        }
    };
    ...
    if (!this._application)
    {
        ...
        //==============================================================================
        _pApplication._onHttpTransactionError = function (obj, bfireevent, errorobj, errortype, url, returncode, requesturi, locationuri, extramsg, svcid, responsebody)
        {
            var ret = false;
            ...
            if (bfireevent)
            {
                ...
                if (!ret)
                {
                    ...
                    if (environment)
                    {
                        ret = environment.on_fire_onerror(obj, commerrorobj.name, commerrorobj.message, errorobj, returncode, requesturi, locationuri, svcid, responsebody);
                    }
                }
            }
        ...
        _pApplication._load = function (key, url)
        {
            ...
            //load stringresource
            this._loadStringResource(nexacro._stringrcfileid);
        ...
        _pApplication._loadTheme = function (themeid)
        {
            ...
            //for info
            var cssmapurl = nexacro._getMapFileName(nexacro._theme_uri + "/theme");
    ...
    */
    ...
    _pTouchGestureManager.ontouchstart = function (win, elem, touches, changedTouches, time)
    {
        ...
        if (is_first)
        {
            if (session && this._is_fling)
            {
                ...
                // 연속적인 fling 동작은 처리 해줘야 하기 때문에 이벤트를 중단 시키지 않도록 처리
            }
            else
            {
                this._touch_session = new nexacro.TouchSession(this, win, target_elem, session);
    ...
    _pTouchGestureManager.ontap = function (elem, data, touches/*, changedTouches*/)
    {
        ...
        if (compinfo && compinfo[0])
        {
            ...
            else if (touch)
            {
                ...
                target_elem = touch.target;

                compinfo[0]._on_lbuttondown(target_elem, "touch", false, false, false, touch._canvasx, touch._canvasy, touch.screenx, touch.screeny);
                var delaytime = nexacro._getLbuttonupDelayTime(target_elem);
                if (delaytime > 0)
                {
                    setTimeout(function ()
                    {
                        target_win._on_sys_lbuttonup(target_elem, "touch", false, false, false, touch.windowx, touch.windowy, touch.screenx, touch.screeny, undefined, undefined, undefined, elem);
                    }, delaytime);
                }
                else
                {
    ...
    _pTouchGestureManager.onfling = function (elem, data/*, touches, changedTouches*/)
    {
        ...
        if (compinfo && compinfo[0])
        {
            if (this._is_fling)
            {
                ...
                var session = this._touch_session;
                if (session && session._cur_detector && this._scroll_end && data.leadTime > 0)
                {
                    // fling 중에도 scroll이 끝까지 이동 되면 fling duration 임계 조정하여 빠른종료
                    session._cur_detector._duration_threshold = 0.2; // duration 임계 조정
                }
            }
        }
    };
    ...
    _pEnvironment._p_usedateautocalibration = true;
    
    _pEnvironment._p_calendarrestoreonblur = true;

    //internal     
    nexacro._calendarrestoreonblur = true;
    ...
    nexacro._usesoftkeyboard = true;
    nexacro._enabletouchsoftkeyboard = false;
    ...
    _pEnvironment.set_calendarrestoreonblur = function (v)
    {
        if (nexacro._is_loaded_application)
            return;
        
        v = nexacro._toBoolean(v);
        if (this._p_calendarrestoreonblur != v)
        {
            this._p_calendarrestoreonblur = v;
            nexacro._calendarrestoreonblur = v;
        }
    };
    ...
    _pEnvironment.set_usedateautocalibration = function (v)
    {
        ...
        v = nexacro._toBoolean(v);
        if (this._p_usedateautocalibration != v)
        {
            this._p_usedateautocalibration = v;
    ...
    _pEnvironment.set_httpretry = function (v)
    {
        ...
        nexacro._httpretry = this._p_httpretry;
    ...
    _pEnvironment.set_httptimeout = function (v)
    {
        ...
        nexacro._httptimeout = this._p_httptimeout;
    ...
    {
        v = nexacro._toBoolean(v);
        this._p_usestringresource = nexacro._use_stringresource = v;
    };
    ...
    _pEnvironment.set_usewaitcursor = function (v)
    {
        ...
        nexacro._usewaitcursor = this._p_usewaitcursor;
    };
    ...
    _pEnvironment.set_enabletouchsoftkeyboard = function (v)
	{
		...
		if (v != this._p_enabletouchsoftkeyboard)
		{
			...
			nexacro._enabletouchsoftkeyboard = v;
		}
	};
    ...
    _pEnvironment.set_enableaccessibility = function (v)
    {
        ...
        if (enableaccessibility == this._p_enableaccessibility)
            return;
        this._p_enableaccessibility = enableaccessibility;
        ...
        if (enableaccessibility && nexacro._enableaccessibility != this._p_enableaccessibility)
        {
    ...
    _pEnvironment.on_fire_onerror = function (obj, errortype, errormsg, errorobj, statuscode, requesturi, locationuri, errordata, responsebody)
    {
        if (this.onerror && this.onerror._has_handlers)
        {
            var evt = new nexacro.ErrorEventInfo(obj, "onerror", errortype, errormsg, errorobj, statuscode, requesturi, locationuri, errordata, responsebody);
            return this.onerror._fireEvent(this, evt);
        }
        return false;
    };
    ...
    _pEnvironment._makeProjectPathinfo = function (xadl, typedefinition_url, project_path)
    {
        ...
        if (xadl)
        {
            ...
            //  archive adl을 사용하는 경우 외부에서 지정		
            if (project_path || ((nexacro._OS == "Android" || nexacro._OS == "iOS" || nexacro._linux_app) && nexacro._isHybrid && nexacro._isHybrid()))
            {
    ...
    _pEnvironment._makeStringResourcePathinfo = function (usestringresource, stringresourceserviceurl)
    {
        ...
        if (usestringresource)
        {
            ...
            if (curlang)
                nexacro._stringrcfileid = nexacro._stringrc_service + curlang + nexacro._stringresourceurl;
        }
        else
        {
            if (nexacro._isDesignMode())
            ...
            nexacro._stringrcfileid = nexacro._stringrc_service + stringresourceserviceurl + "lang=" + curlang;
        }
    };
    ...
    {        
        ...
        if (_ext_openstyles && _ext_openstyles !== "promise")
        {
            ...
            _ext_openstyles = _ext_openstyles.toLowerCase();
        ...
        if (opener === null || opener == undefined || typeof opener == "string")
        {
            var _focus_obj = null;
        ...
        if(isPromise)
        {
            return new Promise((resolve, reject) => {
                try 
                {
                    ...
                    let handle = nexacro._createOpenWindowHandle(parent_window, id, formurl, left, top, width, height, resizable, layered, showontaskbar, false, parentframe, frameopener, arr_arg, ext_options, close_callback);
                    if (!handle)
    ...
    {
        this._p_url = v;
        nexacro._setLocalStorageforService(this._p_prefixid, this._p_url, this._p_cachelevel);
    };
...
/*
    nexacro.toPromise = function (context, methodName) 
    {
        return function(...args) {
            return new Promise((resolve, reject) => {
                try {
                    const result = context[methodName].apply(context, args);
                    resolve(result);
                } catch (err) {
                    reject(err);
                }
