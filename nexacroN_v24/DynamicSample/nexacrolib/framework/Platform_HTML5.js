...
if (nexacro._Browser != "Runtime" && !nexacro._init_platform_HTML5)
{
	...
	nexacro._createSysEvent_ForwardFuncs = function (_cur_win)
	{
		...
		_cur_win._syshandler_onkeydown_forward = function (evt)
		{
			...
			if (_cur_win._linked_window.frame._is_popup_frame)
			{
				var keycode = nexacro._getSysEventKeyCode(evt);
				if (keycode == 116 || (evt.ctrlKey || evt.metaKey) && keycode == 82)
				{
					nexacro._stopSysEvent(evt);
				}
			}
		...
		_cur_win._syshandler_onkeyup_forward = function (evt)
		{
			...
			var linkedwindow = _cur_win._linked_window;
			if (linkedwindow && linkedwindow.frame && linkedwindow.frame._is_popup_frame)
			{
				var keycode = nexacro._getSysEventKeyCode(evt);
				if (keycode == 116 || ((evt.ctrlKey || evt.metaKey) && keycode == 82))
				{
	...
	_pHTMLSysEvent.lockMouseMove = function (/*node,*/ isInput)
	{
		...
		nexacro._observeSysEvent(this._win_win, "mousemove", "onmousemove", this._syshandler_lock_onmousemove_forward, isInput);
		nexacro._observeSysEvent(this._win_win, "mouseup", "onmouseup", this._syshandler_lock_onmouseup_forward);
	};
	...
	nexacro._initHTMLSysEvent = function (_cur_win, _cur_doc)
	{
		...
		{
			if (nexacro._BrowserVersion >= 44 && _win_doc && _win_doc.scrollingElement)
			{
				nexacro.__setDOMStyle_overscrollBehavior(_win_doc.scrollingElement.style, "contain");
			}

			if (nexacro._BrowserVersion >= 63)
			{
	...
	nexacro._syshandler_onmousedown = function (_sysEvent, node, evt)
	{
		...
		if (evt.button == nexacro_HTMLSysEvent.MOUSE_LBUTTON)
		{
			...
			var is_input_elem = elem.isInputElement() && elem.enable;
			var is_chromium148 = (nexacro._Browser == "Chrome" || (nexacro._Browser == "Edge" && nexacro._BrowserType == "WebKit")) && nexacro._BrowserVersion >= 148;
			if (is_chromium148)
				_sysEvent.lockMouseMove(is_input_elem);
			else
				_sysEvent.lockMouseMove();
...
*/
	...
	nexacro._syshandler_onkeyup = function (_sysEvent, node, evt)
	{
		...
		if (win && elem)
		{
			...
			var ret = win._on_sys_keyup(elem, keycode, evt.altKey, evt.ctrlKey, evt.shiftKey, evt.metaKey);
			// IME 입력 체크위해 위치 이동 (RP 104763)
	...
	nexacro._createOpenWindowHandle = function (parent_win, name, formurl, left, top, width, height, resizable, layered, taskbar, is_main, parentframe, frameopener, arr_arg, ext_openstyles, close_callback)
	{
		//var _win_handle = null;
		var parent_handle = null;
		if (parent_win) parent_handle = parent_win.dest_handle;
		return nexacro.__createOpenWindowHandle(parent_handle, name, formurl, left, top, width, height, resizable, layered, taskbar, parent_win, parentframe, frameopener, arr_arg, ext_openstyles, close_callback);
	};
	...
	nexacro._setLocalStorageforOpen = function (id, parentwin, parentframe, frameopener, arrarg, close_callback)
	{
		//cssurl
		if (nexacro._cssurls)
		...
		if (nexacro._popupframeoption[id])
		{
			...
			//else
			{
				...
				_popup_opt._close_callback = close_callback;
			}
		}
	...
	nexacro.__createOpenWindowHandle = function (_handleParent, name, formurl, left, top, width, height, resizable, /*[nouse]*/layered, /*[nouse]*/taskbar, parentwin, parentframe, frameopener, arr_arg, ext_openstyles, close_callback)
	{
		if (left == null)
		...
		nexacro._setLocalStorageforOpen(name, _parent_win, parentframe, frameopener, arr_arg, close_callback);
	...
	nexacro._isEmbeddedWindow = function (win)
	{
		...
		if (_win_handle)
		{
			...
			{				
				is_embedded = true; // cross-origin 페이지에 iframe으로 embed된 경우 parent.document 접근 시 SecurityError
			}
			if (is_embedded)
			{
				win._last_focused_elem = null;
