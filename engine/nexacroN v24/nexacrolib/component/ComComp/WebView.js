...
if (!nexacro.WebView)
{
	...
	if (!nexacro.WebMessageEventInfo)
	{
		nexacro.WebMessageEventInfo = function (obj, data, origin, source, id)
		{
		...
		var _pWebMessageEventInfo = nexacro._createPrototype(nexacro.Event, nexacro.WebMessageEventInfo);
		nexacro.WebMessageEventInfo.prototype = _pWebMessageEventInfo;
		_pWebMessageEventInfo._type_name = "WebMessageEventInfo";

		delete _pWebMessageEventInfo;
	}
	...
	_pWebView.on_fire_onmessage = function (obj, data, origin, source)
	{
		if (this.onmessage && this.onmessage._has_handlers)
		{
			var evt = new nexacro.WebMessageEventInfo(obj, data, origin, source);
			evt.eventid = "onmessage";
			return this.onmessage._fireEvent(this, evt);
		}
		return true;
	};

	_pWebView._on_webview_message = function (evt)
	{
		if (!evt) return;
		this.on_fire_onmessage(this, evt.data, evt.origin, evt.source);
	};
	...
	_pWebView.postMessage = function (message, targetOrigin)
	{
		var ifrm_elem = this._ifrm_elem;
		if (ifrm_elem && ifrm_elem.postMessage)
		{
			return ifrm_elem.postMessage(message, targetOrigin);
		}
		return false;
	};
