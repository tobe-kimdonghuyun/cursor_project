...
if (!nexacro._bInitPlatform)
{
    ...
    */
    ...
    {        
        ...
        var isPromise = returntype === "promise" ? true : false;
        ...
        if (ext_openstyles && ext_openstyles !== "promise")
        {
            var btoolbar = false,
                bmenubar = false,
                blocation = false;
        ...
        if (width == undefined || width < 0)
            width = 0;
        else if(width == "promise")
            isPromise = true;
        
        if (height == undefined || height < 0)
            height = 0;
        else if(height == "promise")
            isPromise = true;
        ...
        if(isPromise)
        {
            return new Promise((resolve, reject) => {
                try 
                {
                    let handle = nexacro._createOpenWindowHandle(parent_window, id, formurl, left, top, width, height, resizable, layered, showontaskbar, false, parentframe, frameopener, arr_arg, ext_options);
                    var t = new nexacro._CallbackTimer(parent_frame, function () {
                        if (nexacro._isClosePopupFrame(handle))
                        {
                            t.stop();
                            resolve();
                        }
