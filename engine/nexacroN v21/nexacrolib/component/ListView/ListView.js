    ...
    */
    ...
    _pListView._setItemSelect = function (rowidx, status, show)
    {
    	...
    	else
        {
            ...
            // RP 102451 해결을 위한 조건처리
            if (this._getForm()._last_focused !== this)
            {
            ...
            for (var i = rowcnt - 1; i >= 0; i--)
            {
                ...
                if (item)
                {
                    ...
                    if (item[1] && this._isExistSubSlot(item[1]._rowidx))
                    {
                        ...
                        last_cell = this._findNextCell(item[1], cellidx, undefined, -1, is_editable, is_accessibility);
                        last_cell = (!last_cell) ? this._findNextCell(item[1], cellidx, undefined, -1, is_editable, is_accessibility) : last_cell;
                    }
                    ...
                    if (!last_cell && item[0])
                    {
                        ...
                        last_cell = this._findNextCell(item[0], cellidx, undefined, -1, is_editable, is_accessibility);
                        last_cell = (!last_cell) ? this._findNextCell(item[0], cellidx, undefined, -1, is_editable, is_accessibility) : last_cell;
                    }
    ...
    _pListView._findNextCell = function (item, from_cellidx, from_bandidx, dir, is_editable, is_accessibility)
    {
        ...
        {            
            ...
            if (cells)
            {
                ...
                {
                    // next cell focus
                    from_cellidx = (from_cellidx < 0) ? -1 : ((from_cellidx >= cells.length) ? cells.length : from_cellidx);
                    while (cells[from_cellidx + dir])
                    {
                        from_cellidx += dir;
