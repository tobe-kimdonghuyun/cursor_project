...
if (nexacro.Component)
{
						...
						if (lose_focus && lose_focus._is_alive && !lose_focus._is_killfocusing)
						{
							...
							if (lose_focus.enableevent)
							{
								if (new_focus._is_popup_control)
								{
									...
									var lelem = _win._last_mouseover_elem;
									if (lelem)
									{
										var lcomp = _win.findComponent(lelem);
										if (lcomp)
										{	
											lcomp._clearMouseoverStatus();
										}	
									}
								}	
	...
	{
		//clck에서 popup을 띄우면 mouseover status 해제
		this._releaseStatusByPopup();
		return;
	};
	...
	_pComponent.on_lbuttondown_default_action = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, refer_comp, meta_key)
	{
		...
		if (this.visible && this._isEnable() && refer_comp)
		{
			...
			{
				// lastfocus는 component 기준으로 등록
				refer_comp = this._getRootComponent(refer_comp);
	...
	_pComponent.on_rbuttondown_default_action = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, refer_comp, meta_key)
	{
		...
		if (this.visible && this._isEnable() && refer_comp)
		{
			// lbuttondown과 동일하게 popup에 대한 처리는 lastfocus만 갱신
	...
	_pComponent.on_lbuttonup_basic_action = function (elem, button, alt_key, ctrl_key, shift_key, canvasX, canvasY, screenX, screenY, refer_comp, meta_key)
	{
		if (this._use_pushed_status)
		{
			if (this._isPushed())
			{
				...
				{
					this._changeUserStatus("pushed", false);
					if (button != "touch" && !this._isFireVirtualMouseEvent()) // 가상커서로 발생된 lbuttonup 은 처리 안함
						this._changeStatus("mouseover", true);
				}
			}
		}
	...
	_pComponent.on_fire_sys_onfling = function (elem, touch_manager, xstartvalue, ystartvalue, xdeltavalue, ydeltavalue, touchlen, from_comp, from_refer_comp)
	{
		...
		var dragscrolltype;
		var scrolltype = this._scrolltype || this.scrolltype;
		if (nexacro._enabletouchevent)
		{
			dragscrolltype = this.dragscrolltype;
		...
		if (scroll_comp)
		{
			if (scroll_comp != this)
			{
				// flingstart component와 flingend component 다르면 현재 component scroll visible false
				this._applyVisibleScrollEnd();
				return false;
			}
		}
		else
		{
			...
			{
				// Check Horz
				if (xdeltavalue != 0 && is_scrollable && hscrollbar && hscrollbar.enable)
				{
					can_hscroll = this._getCanHscroll(hscrollbar, dragscrolltype, xdeltavalue);
				}

				// Check Vert
				if (ydeltavalue != 0 && is_scrollable && vscrollbar && vscrollbar.enable)
				{
				...
				if (selectscrollmode != undefined)
				{
					var select_mode;

					if (selectscrollmode == "default")
						select_mode = (nexacro._isTouchInteraction || nexacro._SupportTouch) ? "scroll" : "select";
					else
						select_mode = selectscrollmode;
		...
		if (scroll_mode > 0)
		{
			...
			// scroll 끝까지 이동하는지 체크
			var is_hscroll_max = (hscrollbar && hscrollbar._max > 0) ? (new_hpos <= hscrollbar._min || new_hpos >= hscrollbar._max) : false;
			var is_vscroll_max = (vscrollbar && vscrollbar._max > 0) ? (new_vpos <= vscrollbar._min || new_vpos >= vscrollbar._max) : false;

			if (this._is_bubble_fling_v === undefined && this._is_bubble_fling_h === undefined)
			{		
				// 스크롤 끝까지 이동하면 flag 변경
				touch_manager._scroll_end = (is_hscroll_max || is_vscroll_max);
			...
			else if (xdeltavalue != 0)
			{
				...
				// 스크롤 끝까지 이동하면 flag 변경
				touch_manager._scroll_end = is_hscroll_max;
			}
			else if (ydeltavalue != 0)
			{
				...
				// 스크롤 끝까지 이동하면 flag 변경
				touch_manager._scroll_end = is_vscroll_max;
			}
			return true;
		}
	...
	_pComponent.on_fire_sys_onslide = function (elem, touch_manager, touchinfos, xaccvalue, yaccvalue, xdeltavalue, ydeltavalue, from_comp, from_refer_comp, scroll_start, scroll_end)
	{
		if (nexacro._cur_track_info)
		{
		...
		else
		{
			...
			var selectscrollmode = this._selectscrollmode;
			...
			if (selectscrollmode == "select")
			{
				scroll_mode = 0;
				touch_manager._touch_session._fling_blocked = true; // selectmode 인경우 fling 동작 막음
			}
		...
		this._scrollTo(hscroll_pos, vscroll_pos, true, scroll_end ? true : false, undefined, "slide");
		...
		if (scroll_start)
		{
			...
			if (xdeltavalue != 0 && ydeltavalue != 0)
			{
				// slide bubble 체크되는 경우 bubble되도록 return false
				if (old_vpos == new_vpos && old_hpos == new_hpos)
				{
		...
		if (is_vscroll_max || is_hscroll_max)
		{
			// scroll 끝까지 이동하면 fling 발생하지 않도록 처리
	...
	{
		return this.on_fire_sys_onslide(elem, touch_manager, touchinfos, xaccvalue, yaccvalue, xdeltavalue, ydeltavalue, from_comp, from_refer_comp, false, true);
	};
	...
	// autoindicator의 경우 scroll 가리도록 처리하는 함수 모듈화
	_pComponent._applyVisibleScrollEnd = function ()
	{
		...
		if (vscrollbar && vscrollbar.visible && vscrollbartype == "autoindicator")
			vscrollbar.set_visible(false);

		if (hscrollbar && hscrollbar.visible && hscrollbartype == "autoindicator")
