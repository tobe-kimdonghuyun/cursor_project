...
if (nexacro._Browser != "Runtime" && !nexacro.Element)
{
    ...
    */
    ...
    //_pElement.setElementFocus = nexacro._emptyFn;
    ...
    _pTextBoxElement.setElementTagFocus = function (index)
    {
        ...
        if (handle)
        {
            ...
            nexacro.__setLastFocusedElement(this.owner_elem);
        }
    };
    ...
    */
    ...
    _pInputElement.attachHandle = function (win)
    {
        if (this.name && !this.handle)
        {
            ...
            var control = this.parent_elem.linkedcontrol;
            if (control instanceof nexacro._CalendarEditControl)
            {
                if (control._caret_pos == -1)
    ...
    _pInputElement.setElementBlur = function ()
    {
        ...
        if (input_handle)
        {
			...
			if (_doc)
			{
				...
				{
					// RP 104452 caret을 삭제하려는 원인은 16년도에 작업되어서 caret을 지우려는 node 가 동일하면 수행(focusin event 시점에 caret을 날려서 caret 안나오는 문제)
    ...
    _pInputElement._go_next_focus = function ()
    {
        ...
        if (input_handle && comp)
        {
            ...
            if (newfocus_comp && newfocus_comp[0])
            {
                ...
                if (!newfocus_comp[0].autoselect && newfocus_comp[0]._isEditableComponent())
    ...
    */
    ...
    var _bind_container_scroll_handler = function (elem)
    {
        return function (evt)
        {
            ...
            else if (elem.parent && elem.parent._step_count > 0 && (nexacro._Browser == "IE" && nexacro._BrowserVersion >= 9))
            {
                // IE9+에서 step form 포커스 이동 시 브라우저가 DOM scrollLeft를 자동 변경하는 것을 복원
                target.scrollLeft = elem._scroll_left;
            }
...
//	        if (this.left || this.top)
    ...
    * position 이동  : 터치시 화면반응속도 중간, 스크롤속도 중간, 화면출렁거림 거의없음 (노드량이 많으면 느려질 수 있음)
    ...
    nexacro.GridRowControlElement = function (parent_elem, left_gap)
    {
        ...
        this._client_body_element._no_focus_scroll = (nexacro._enableaccessibility && nexacro._accessibilitytype > 3) ? false : true;
