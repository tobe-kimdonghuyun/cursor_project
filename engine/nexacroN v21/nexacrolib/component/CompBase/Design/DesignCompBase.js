    ...
    _pComponent._rePositioning = function (left, top, width, height, right, bottom)
    {
        ...
        */
        ...
        if (this.left != null || (this.left == null && this.right == null)) 
        {
		    ...
		    if (compid) { leftVal = compid; leftVal += ":"; leftVal += distance; }
            else        { leftVal = distance; }
        }
        else
            leftVal = undefined;
        ...
        {            
		    ...
		    if (typeof this.top == "string" && this.top.indexOf("%") >= 0)
		    {
                ...
                distance = parseInt(distance);
		    if (compid) { topVal = compid; topVal += ":"; topVal += distance; }
            else        { topVal = distance; }
        }
        else
            topVal = undefined;
        if (this.width != null)
        {            
            ...
            {
                // table layout 에서는 여기서 this.parent._adjust_width 대신 component가 속한 cell의 width로 처리되어야 한다.                
                ...
                distance = width;
			if (compid) { widthVal = compid; widthVal += ":"; widthVal += distance; }
            else        { widthVal = distance; }
        }
        else
            widthVal = undefined;
        if (this.height != null)
        {            
            ...
            if (typeof this.height == "string" && this.height.indexOf("%") >= 0)
            {
                ...
                distance = height;
			if (compid) { heightVal = compid; heightVal += ":"; heightVal += distance; }
            else        { heightVal = distance; }
        }
        else
            heightVal = undefined;
        ...
        if (this.right != null)
        {
	        ...
	        if (typeof this.right == "string" && this.right.indexOf("%") >= 0)
			{
                ...
                distance = parseInt(distance);
			if (compid) { rightVal = compid;	rightVal += ":"; rightVal += distance; }
            else        { rightVal = distance; }
        }
        else
            rightVal = undefined;
        if (this.bottom != null)
        {            
            ...
            if (typeof this.bottom == "string" && this.bottom.indexOf("%") >= 0)
			{
                ...
                distance = parseInt(distance);
			if (compid) { bottomVal = compid; bottomVal += ":"; bottomVal += distance; }
            else        { bottomVal = distance; }
        }
        else
            bottomVal = undefined;    
