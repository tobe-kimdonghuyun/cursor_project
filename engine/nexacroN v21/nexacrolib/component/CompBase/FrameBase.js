...
if (!nexacro.Frame)
{
    ...
    _pFrame._waitCursor = function (wait_flag, context)
    {
        ...
        if (owner_frame)
        {
            if (owner_frame._is_alive)
                owner_frame._waitCursor(wait_flag, context);
            else
            {
                // 부모 팝업이 close()로 소멸된 경우 waitcursor 체인이 끊기므로 mainframe으로 위임
                var application = nexacro.getApplication();
                var mainframe = application ? application.mainframe : null;
                if (mainframe && mainframe._is_alive && mainframe !== this)
