...
if (!nexacro.Form)
{
    ...
    {
        for (var f = this; (f != null); f = (f.parent || f.opener))
        {
            if (name in f)
                return f[name];
        }
            ...
            for (var i = 0; i < comp_len; i++)
            {
                ...
                if (!(filter_type & 1))
                {
                    if (bAccessibility)
                    {
						...
						{
							if (!comp.accessibilityenable || (comp._isVisible && !comp._isVisible() || !comp.visible))
								continue;
						}
                    }
    ...
    _pFormBase._processHotkey = function (keycode, altKey, ctrlKey, shiftKey, obj)
    {
        ...
        if (this._is_frame && this._is_window && (this._window_type == 1 || this._window_type == 2 || this._window_type == 4))
            return;
    ...
    _pForm.on_fire_user_onkeydown = function (keycode, alt_key, ctrl_key, shift_key, fire_comp, refer_comp, meta_key)
    {
        ...
        if (!this.onkeydown || (this.onkeydown && !this.onkeydown.defaultprevented))
        {
            ...
            if (this._isComponentKeydownAction() && (keycode == nexacro.Event.KEY_LEFT || keycode == nexacro.Event.KEY_RIGHT))
            {
                ...
                if (hscrollbar && hscrollbar.visible && (ctrl_key == true || (nexacro._enableaccessibility && !nexacro._accessibilitycontentsearchkey)))
                {
            ...
            else if (this._isComponentKeydownAction() && (keycode == nexacro.Event.KEY_UP || keycode == nexacro.Event.KEY_DOWN))
            {
                ...
                if (vscrollbar && vscrollbar.visible && (ctrl_key == true || (nexacro._enableaccessibility && !nexacro._accessibilitycontentsearchkey)))
                {
                    if (!dlgc.want_arrows) // arrowkey를 직접 처리하는 컴포넌트는 제외
                    {
            ...
            if (this._is_scrollable && this._control_element)
            {
                ...
                for (var i = 0, n = comps.length; i < n; i++)
                {
                    ...
                    if (comp && comp.visible)
                    {
                        ...
                        // 실제 컨텐츠 크기(contents max)는 스크롤 필요 여부 판정용이므로, right/bottom 앵커로 늘어나는
                        // 컴포넌트가 minwidth/minheight로만 잡히는 경우 늘어난 offset이 아니라 최소크기(left+minwidth /
                        // top+minheight) 기준으로 반영한다. 늘어난 offset을 쓰면 스크롤바 폭만큼 초과가 생겨 phantom 스크롤이
                        // 발생한다. (RP 105767)
                        if (comp._right < 0 || !comp.right && (comp.width > 0) || (nexacro._isString(comp.width) && comp.width.indexOf("%") < 0))
                            _c_w = Math.max(_c_w, offsetright);
                        else if (comp.minwidth)
                            _c_w = Math.max(_c_w, comp.getOffsetLeft() + nexacro._toInt(comp.minwidth));
                        if (comp._bottom < 0 || !comp.bottom && (comp.height > 0) || (nexacro._isString(comp.height) && comp.height.indexOf("%") < 0))
                            _c_h = Math.max(_c_h, offsetbottom);
                        else if (comp.minheight)
                            _c_h = Math.max(_c_h, comp.getOffsetTop() + nexacro._toInt(comp.minheight));
                    }
                }
        ...
        _pInnerForm.addChild = function (id, obj)
        {
            ...
            if (obj._is_component)
            {
                ...
                {                    
                    ...
                    if (this.parent)
                    {
                        this.parent._setAccessibilityStatHidden(false);
                    }
                } 
            }
            else if (obj instanceof nexacro.BindItem)
