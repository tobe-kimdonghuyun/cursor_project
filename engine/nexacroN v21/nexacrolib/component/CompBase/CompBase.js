...
if (!nexacro.Component)
{
    ...
    */
    ...
    // popup이 열릴때 mousemove로 mouseover된 component는 mouseover 해제
    _pComponent._releaseStatusByPopup = function ()
    {
        var _win = this._getWindow();
        if (_win)
        {
            var capture_comp = _win._getCaptureComp(true, false, this);
            if (capture_comp && capture_comp != this)
            {
                this._changeStatus("mouseover", false);

                // subcontrol이거나 innerform이면 경우 부모까지 반영
                if (this.parent && (this._is_subcontrol || this instanceof nexacro._InnerForm))
                {
                    this.parent._releaseStatusByPopup();
                }
            }
        }
    };
    ...
    */
	...
	{
		v = v !== undefined ? this.usesoftkeyboard : nexacro._usesoftkeyboard;
    ...
    _pComponent._apply_setTagfocus = nexacro._emptyFn;
    ...
    _pPopupControl.set_visible = function (v)
	{
        ...
        if (this.visible != v)
        {
			...
			if (nexacro._enableaccessibility)
			{
                ...
                if (!v && nexacro._accessibilitytype > 3)
                {
                    var parent = this.parent;
                    if (parent && parent instanceof nexacro.Combo)
                    {
                        parent._setAccessibilityStatExpanded(v);
                    }
                }
			}
