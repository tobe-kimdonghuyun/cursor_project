...
if (!nexacro.ExcelImportObject)
{
    ...
    _pExcelImport._makeImportFormat = function ()
    {
        ...
        var sheets;
        var rangeOptions = this._range.match(/\[.*?\]/g);
        if (rangeOptions)
        {
            var sheets = rangeOptions.map(function(rangeOption) {
                var content = rangeOption.slice(1, -1);
                var parts = content.split(';');
                var commandObj = { _content: content, _is_command: true };
        ...
        {
            sheets = [];
            var content = this._range.replace(/;$/, "");
            sheets.push({_content:content, _is_command:false, body:content});
        }
        ...
        if (sheets && sheets.length > 0)
        {
            for (var i = 0; i < sheets.length; i++)
            {
                ...
                if (sheets[i]._is_command)
                {
                    ...
                    for (var pname in sheets[i]) {
                        if (pname == "_content" || pname== "_is_command") continue;
                        str += pname + "=\"" + sheets[i][pname] + "\" ";
                    }
                ...
                else
                {
                    ...
                    if (sheets[i]._content.indexOf("!") >= 0)
                    {
                        var range_arr = sheets[i]._content.split("!");
                        if (range_arr[1].indexOf(":") >= 0)
                        {
                            ...
                            str += 'output=\"' + this._responseRVal + '\" head=\"' + sheet + '!' + start_col + start_row + ':' + end_col + start_row + '\" body=\"' + sheet + '!' + start_col + (+start_row + 1) + ':' + end_col + end_row + '\" />';
                        }
                        else
                        {
                            str += 'output=\"' + this._responseRVal + '\" body=\"' + sheets[i]._content + '\" />';
                        }
                    }
                    else
                    {
