...
if (!nexacro.CheckBoxSet)
{
    ...
    _pCheckBoxSet.setSelectRange = function (start, end, bSelect)
    {
        ...
        if (start > end)
        {
            for (i = start; end <= i; i--)
            {
                itemReadonly = readonlyCol && ds ? nexacro._toBoolean(ds.getColumn(i, readonlyCol)) : false;
                if (items[i] && !itemReadonly)
                    rows.push(i);
            }
        }
        else
    ...
    _pCheckBoxSet.setSelectItems = function (items, bSelect)
    {
        ...
        for (i = 0; i < items.length; i++)
        {
            ...
            itemReadonly = readonlyCol && ds ? nexacro._toBoolean(ds.getColumn(items[i], readonlyCol)) : false;
            if (!itemReadonly)
                rows.push(items[i]);
        }
    ...
    _pCheckBoxSet._removeindex = function (v, change_by_script, isNotFireEvent)
    {
        if (!isNotFireEvent)
        {
            ...
            if (dataset && column)
            {
                ...
                postvalue = this.value == undefined ? this.value : nexacro._toString(this.value).split(valueseparator);
