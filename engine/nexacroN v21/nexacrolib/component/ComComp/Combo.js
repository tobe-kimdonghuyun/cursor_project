...
if (!nexacro.Combo)
{
    ...
    _pCombo.on_created_contents = function (win)
    {
        ...
        if (this.comboedit)
        {
            this.comboedit.on_apply_usesoftkeyboard();
        }
    ...
    //===============================================================
    _pCombo._isEditableComponent = function ()
    {
        return this._isDropdownType() ? false : this._is_editable_control;
    };
    ...
    _pCombo.on_apply_value = function (value)
    {
        ...
        //if (control_elem)
        {
            var idx = -1;
            var txt = "";
            var ds = this._innerdataset || this._selectDataset();
            if (ds)
            {
    ...
    _pCombo.on_apply_usesoftkeyboard = function ()
    {
        if (this.comboedit)
        {
            this.comboedit.set_usesoftkeyboard(this._getUseSoftKeyboardValue(this._initsoftkeyboard));
        }
    };
        ...
        else if ((keycode == nexacro.Event.KEY_UP || (keycode == nexacro.Event.KEY_LEFT && this._isDropdownType())) && this._isComponentKeydownAction())
        {
			...
			else
			{
				...
				if (this.index != nextidx)
				{
                    ...
                    // dropdown mode에서 popup 없이 방향키 이동시 동적으로 label 변경하여 출력 (RP 105223)
                    if (control_elem && this._isDropdownType())
                    {
                        control_elem.setElementAccessibilityLabel(cur_text);
                    }
    ...
    _pCombo.on_killfocus_basic_action = function (new_focus, new_refer_focus)
	{
        ...
        // dropdown mode에서 popup 없이 방향키 이동시 동적으로 변경할 label 복원 (RP 105223)
        var control_elem = this.getElement();
        if (control_elem && this._isDropdownType())
        {
            control_elem.setElementAccessibilityLabel(this._getAccessibilityLabel());
        }
    ...
    _pComboPopupControl._getPopupPosition = function ()
	{
		...
		if (combo && combolist && rootframe)
		{
            ...
            */
            ...
            var combo_screen_pos = nexacro._getElementScreenPosition(combo.getElement());
            if (combo_screen_pos && screen_avail_height > 0)
            {
                var dpr = nexacro._getDevicePixelRatio(rootframe.getElement());
                var screen_based_below = Math.round((screen_avail_height - combo_screen_pos.y - combo_size[1] * dpr) / dpr);
