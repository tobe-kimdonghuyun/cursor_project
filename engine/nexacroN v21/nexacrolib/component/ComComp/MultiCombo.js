...
if (!nexacro.MultiCombo)
{
    ...
    _pMultiCombo._apply_setfocus = function (evt_name)
    {
        if (this._tagmode)
        {
            ...
            if (tagboxedit)
            {
                ...
                if (nexacro._isTouchInteraction && nexacro._SupportTouch && this.type == "dropdown")
                {
    ...
    _pMultiCombo.on_apply_autoselect = function (autoselect)
    {
        ...
        {
            var multicombotext = this.multicombotext;
            if (multicombotext && this.type != "dropdown")
            {
                multicombotext.set_autoselect(autoselect);
            }
        }
    };
    ...
    */
    ...
    _pMultiCombo._on_tagbox_mobile_onclick = function (obj, e)
    {
        ...
        if (e.button == "touch")
            this._on_tagbox_mobile_click(e);
    ...
    _pMultiCombo._on_tagbox_mobile_click = function(e)
    {
        this._on_dropdown();
    };
    ...
    _pMultiCombo.on_killfocus_basic_action = function (new_focus, new_refer_focus)
    {
        ...
        else if (nexacro._isNull(this.value))
        {
            var target = this._tagmode ? this.multicombotagbox.tagboxedit : this.multicombotext;
            if (target)
                target._changeUserStatus("nulltext", true);
        }
    ...
    _pMultiCombo._createMultiComboTagBoxControl = function ()
    {
        ...
        {
            multicombotagbox._vscrollbartype = "none";
            multicombotagbox.set_scrolltype("horizontal");
        }
        else
        {
            multicombotagbox._hscrollbartype = "none";
        ...
        if (this.on_fire_ondropdown(this))
        {
            ...
            if (multicombolist)
            {
                ...
                if (checkboxset)
                {
                    ...
                    if (!this._isFiredOnInput && (!(nexacro._isTouchInteraction && nexacro._SupportTouch) || this.type != "dropdown"))
                    {
                        if (this._tagmode)
                        {
                            var tagboxedit = this.multicombotagbox.tagboxedit;
    ...
    _pMultiComboCheckBoxSetControl._select_withmouseevent = function (idx, obj)
    {
        ...
        if (this._shiftKey)
        {
            ...
            var is_filter = multicombo._filtereddataset && (this.type == "filter" || this.type == "filterlike" || this.type == "caseifilter" || this.type == "caseifilterlike");
            if (is_filter && this._innerdataset._rawRecords != this._innerdataset._viewRecords)
            {
            ...
            if (checkboxset)
            {
                ...
                if (multicomboedit)
                {
                    if (multicombo.type != "dropdown" && (multicombo.edittype == "text" || multicombo.edittype == "count"))
                    {
                        ...
                        if (!text_height)
                        {
                            text_height = checkboxset_itemheight;
                        }
                ...
                var multicombo_screen_pos = nexacro._getElementScreenPosition(multicombo.getElement());
                if (multicombo_screen_pos && screen_avail_height > 0)
                {
                    var dpr = nexacro._getDevicePixelRatio(rootframe.getElement());
                    var screen_based_below = Math.round((screen_avail_height - multicombo_screen_pos.y - multicombo_size[1] * dpr) / dpr);
                    if (screen_based_below < below_space_height)
                        below_space_height = Math.max(0, screen_based_below);
                }
        ...
        if (this._is_created_contents)
        {
            ...
            if (multicomboedit)
            {
                if (this.parent.type != "dropdown" && (this.parent.edittype == "text" || this.parent.edittype == "count"))
                {
                    ...
                    if (!text_height)
                    {
                        text_height = checkboxset._getItemHeight();
                    }
