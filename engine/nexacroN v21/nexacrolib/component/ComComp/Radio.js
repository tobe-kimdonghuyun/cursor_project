...
if (!nexacro.Radio)
{
	...
	_pRadio._on_focus = function (self_flag, evt_name, lose_focus, refer_lose_focus, new_focus, refer_new_focus)
	{
		...
		if (self_flag == false)
		{
			...
			if (focusdir == 0 || focusdir == 1)   //tab, shifttab
            {
                ...
                if (index == -1)
                {
                    // index == -1 인 경우, 첫번째(Tab) 또는 마지막(Shift + Tab) 라디오 아이템에 포커스
                    index = focusdir == 0 ? 0 : this._items.length - 1;
                }
    ...
    _pRadio.on_keydown_default_action = function (keycode, alt_key, ctrl_key, shift_key, refer_comp)
    {
        ...
        if (keycode == nexacro.Event.KEY_SPACE) // KEY_SPACE
        {
            ...
            if (!this.readonly)
            {
                var acc_index = this._accessibility_index;
                if (acc_index > -1)
                {
        ...
        else if (this._isComponentKeydownAction() && (keycode >= E.KEY_LEFT && keycode <= E.KEY_DOWN))
        {
            ...
            var idx = (this.index == -1) ? this._accessibility_index + op : this.index + op;
