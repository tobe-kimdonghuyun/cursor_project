            ...
            for (var i = 0, l = attrs.length; i < l; i++)
            {
                ...
                if (!nexacro._isNull(_text))
                {
                    if (nexacro._isString(_text))
                    {
                        ...
                        {
                            case "bind:": this._addBind(bandsq, bandid, basename, _name, _text.substr(5)); this._arrBind(item); break;
                            case "expr:": this._addExpr(bandsq, bandid, basename, _name, _text.substr(5)); this._arrExpr(item); break;
                        }
                    }
    ...
    _pPanel._relocPanelSlotSize = function (hr, vr, hi, vi, hs, vs, cw, ch)
    {
        ...
        */
        ...
        if (this._isColFirst())
        {
            ...
            {
            	var cn = this._getPanelEachSizeSlots(i) || 0;	// 한줄에 배치될수 있는 slotcount
            	var nn = this._getPanelEachSizeCount(i) || 0;	// 한줄에 실제 배치된 slotcount
                var ew = this._getPanelEachSizeWidth(i) || 0;	// 한줄에 배치된 total slot width 
                var sw = cw - ew;								// slot 배치하고 남은 공백
                ...
                for (var i = 0, s = this._panel_slots.length; i < s; i++)
                {
                    ...
                    if (slot && slot._isVisible())
                    {
                        ...
                        if (o)
                        {
                            ...
                            if (k && i)
                            {
                            	tm = this._accPanelEachSizeHeight(ix, d);												// _panel_eachsize 에 계산된 diff height 만큼 보정처리.
...
if (!nexacro.ComplexComponent)
{
    ...
    */
    ...
    _pComplexComponent._createPopup = function (slot, isband)
    {
        ...
        if (popup && slot)
        {
            ...
            // init popup info
            //popup._initInfo(this, slot._getSlotTarget(), popup._getMainFrame(), popupindex, startindex, startlevel);
    ...
    */
    ...
    _pComplexComponent._findNextSelectIndex = function (keycode, alt_key, ctrl_key, shift_key)
    {
        ...
        var select = this.getSelectIndex();                      if (select === undefined || select === null)                select = -1;
    ...
    */
    ...
    _pComplexComponent.readonly = false;                     // readonly 지원시 추가
