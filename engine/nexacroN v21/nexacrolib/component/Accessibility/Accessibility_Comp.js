...
if (nexacro.Div)
{
    ...
    // Div : Override
    //==============================================================================
    _pDiv.on_apply_prop_accessibilityenable = function ()
    {
        nexacro.Component.prototype.on_apply_prop_accessibilityenable.call(this);

        // 접근성 상태에서만 생성 시점 이후 form 내부 accessibilityenable 변화가 생길 경우
        if (!this.form || !this._is_created || !nexacro._enableaccessibility)
			return;
    ...
    _pDiv._on_focus = function (self_flag, evt_name, lose_focus, refer_lose_focus, new_focus, refer_new_focus)
    {
        ...
        {
            if (this.form && self_flag == false)
            {
                var desclevel = this._getAccessibilityDescLevel();
                if (desclevel != "self")
                {
                    ret = this.form._on_focus(self_flag, evt_name, lose_focus, refer_lose_focus, new_focus, refer_new_focus);
                }
            }
		}
		return ret;
    ...
    // 접근성 정보 변경되면 control들에 대한 접근성 정보 업데이트
	_pDiv._updateAccessibilityControl = function ()
	{
		if (!nexacro._enableaccessibility)
			return;
		...
		if (control_elem)
		{
			var inner_form = this.form;
			var cell_elem = this._cell_elem;

            // innerform에 accessibilityenable이 true인 component가 존재하면 내부 초점 이동(없으면 전체 skip)
            // Div가 visible 상태일때만 체크한다. 가려지면 스크린리더의 초점도 이동안함
            var _inner_focus = (inner_form && inner_form.components && inner_form.components.length > 0);
            if (_inner_focus && this._isVisible && this._isVisible() && this.visible)
            {
                _inner_focus = (inner_form.getFirstAccessibilityComponent(true) != null);
            }

            var accessibilityenable = this._isAccessibilityEnable();
			if (accessibilityenable)
			{
				// displaytext
				if (cell_elem)
				{
					cell_elem.setElementAccessibilityStatHidden(_inner_focus);
				}

				// component 없는 경우 Div focus
				control_elem._refreshControlTabIndex(null, _inner_focus ? null : control_elem.tabindex);
			}
			else // accessibilityenable false
			{
				// displaytext
				if (cell_elem)
				{
                ...
                // component 없는 경우 Div focus
				control_elem.setElementAccessibilityStatHidden(!_inner_focus);
	...
	_pDiv._getHeadingOrderNext = function (current, direction, filter_type)
	{
		if (this.form)
		{
			return this.form._getHeadingOrderNext(current, direction, filter_type);
		}
	...
	_pDiv._getHeadingOrderFirst = function (filter_type)
	{
		if (this.form)
		{
			return this.form._getHeadingOrderNext(null, 1, filter_type);
		}
	...
	_pDiv._searchNextHeadingFocus = function (current, filter_type)
	{
		...
		if (this.form)
		{
			comp = this.form._searchNextHeadingFocus(current, filter_type);
		}
	...
	_pDiv._searchPrevHeadingFocus = function (current, filter_type)
	{
		...
		if (this.form)
		{
			comp = this.form._searchPrevHeadingFocus(current, filter_type);
		}
    ...
    _pDiv._getAccessibilityRole = function ()
    {
        ...
        if (nexacro._enableaccessibility && this.semantictag)
        {
            return "null";
        }
    ...
    _pDiv._setAccessibilityRole = function (role)
    {
        ...
        if (this._isAccessibilityRoleHeading())
        {
            this._setAccessibilityLabel(this.accessibilitylabel);
        }
    };
...
if (nexacro.Tab)
{
    ...
    _pTab._getFirstTabbuttonIndex = function ()
    {
        ...
        for (var i = 0; i < tabbuttonitems_len; i++)
        {
            ...
            if (nexacro._enableaccessibility)
            {
                if (tabbuttonitem.visible && tabbuttonitem.left > -1)
                {
                    v = i;
                    break;
                }
    ...
    _pTab._adjustTabButtonPosition = function (beginTabindex, tabbuttonitem, tab_size, tab_max_size, bovered)
    {
        ...
        if (nexacro._enableaccessibility && bovered && !bmultiline)
        {
            ...
            if (next_pos > tab_max_size)
            {
                ...
                this._show_button_cnt = (this._show_button_cnt == 0 || tabbuttonitem._tabindex - beginTabindex < this._show_button_cnt) ? tabbuttonitem._tabindex - beginTabindex : this._show_button_cnt;
                next_pos = tab_size;
            }
    ...
    //==============================================================================
    _pTab._on_accessibility_create_spinbutton = function ()
    {
        var prevbutton = this.prevbutton;
        var nextbutton = this.nextbutton;

        // label을 설정하지 않으면 class 속성을 출력함 (id를 출력)
        if (nexacro._enableaccessibility && prevbutton && nextbutton)
        {
            prevbutton._setAccessibilityLabel(prevbutton.id);
            nextbutton._setAccessibilityLabel(nextbutton.id);
        }
    };    	
...
if (nexacro._TabButtonItemControl)
{
    ...
    _pTabButtonItemControl._on_getAccessibilityAdditionalLabel = function ()
    {
         ...
         * RP 103332
         * tab에 role=tablist를 넣으면 하위 컴포넌트에 전부 탭을 읽어주어
         * role = tablist는 삭제
         * 구조상 텝버튼을 감싸는 container가 하나있고 그위에 role=tablist가 들어가면되는데
         * 우리는 그렇게 안되어있기때문에 iOS버전 특성상 addtionallabel에 role을 넣어줌
        **********************************************************************************/
        if (nexacro._accessibilitytype == 4 && nexacro._BrowserVersion >= 18)
        {
            var accrole = nexacro._roleList[this._getAccessibilityRole()];
            if (accrole)
                add_label += " " + accrole;
        }
...
if (nexacro._RadioItemControl)
{
    ...
    _pRadioItemControl._setAccessibilityStatFocus = function (evt_name)
    {
        ...
        if (radio)
        {
            radio._accessibility_index = this._index;
        }
    };
...
if (nexacro.Edit)
{
	...
	_pEdit._on_attach_accessibility_contents_handle = function (win)
    {
        if (nexacro._enableaccessibility)
        {
            this._setAccessibilityFlagPassword(this.password);
	...
	_pEdit._on_getAccessibilityAdditionalLabel = function ()
    {
		...
		if (nexacro._Browser == "Runtime")
        {
			...
			if (this._onlydisplay)
			{
                if (!this.value)
                {
                    str_additional_label = this.displaynulltext ? this.displaynulltext : "";
                }
			}
			else
			{
				if (!this.password)
				{
					str_additional_label = this.value ? this.text : this.displaynulltext ? this.displaynulltext : "";
				}
			}
        }
...
if (nexacro.Combo)
{
   ...
   _pCombo._apply_setfocus = function (evt_name)
    ...
    _pCombo._on_created_accessibility_contents = function (win)
    {
        ...
        var control_elem = this.getElement();
        var edit_control_elem = comboedit ? comboedit.getElement() : null;

        this._setAccessibilityStatExpanded(false);

        if (control_elem)
        {
            // GridCellCombo(_cellobj)는 CellControl에서 subcomp에 aria-hidden 처리되어 방향키 탐색 대상이 아니므로 inert 불필요
			if (this._isDropdownType() && !this._cellobj)
			{
            ...
            else
            {
                if (edit_control_elem)
                {
                    edit_control_elem.setElementPreventedEvent(false);
                }
    ...
    _pCombo._setAccessibilityStatLive = function (v)
	{
		if (this.comboedit)
		{
            this.comboedit._setAccessibilityStatLive(v);
        }
            ...
            if (comboedit && combolist)
            {
                ...
                if (ds)
                {
                    ...
                    if (item)
                    {
                        ...
                        if (input_elem)
                        {
                            input_elem.setElementAccessibilityControls(combolist._unique_id);
    ...
    {
        var add_label = "";
        if (nexacro._accessibilitytype == 4 && this._isDropdownType())
        {
            add_label = this.text ? this.text : this.displaynulltext ? this.displaynulltext : "";
        }
        return add_label;
    };
...
if (nexacro._ComboEditControl)
{
    ...
    _pComboEditControl._updateAccessibilityInfo = function ()
    {
        if (!nexacro._enableaccessibility)
            return;
        ...
        if (input_elem && (input_elem instanceof nexacro.InputElement))
        {
            ...
            if (combo && !combo.readonly)
            {
                ...
                if (!combo._isDropdownType())
                {
                    ...
                    input_elem.setElementAccessibilityRole(combo.accessibilityrole);
                }
                //-------------------------------------------
            }
        }
    ...
    _pComboEditControl._on_getAccessibilityAdditionalLabel = function ()
    {
        ...
        if (nexacro._Browser == "Runtime")
        {
            ...
            {
                this._setAccessibilityRole("combobox");
                add_label = this.value ? this.value : this.displaynulltext ? this.displaynulltext : "";
            }
        }
        return add_label;
    };
...
if (nexacro.Calendar)
{
		...
		else
        {
            if (nexacro._Browser == "Runtime")
            {
                ...
                else
                {
                    ...
                    if (maskobj && this.calendaredit)
                    {
                        var binvalid;
                        var value = this.calendaredit.value ? this.calendaredit.value : this.value;
                    ...
                    str_additional_label = formattedText ? formattedText : (this.text ? this.text : this.displaynulltext);
                }
			}
        }
                ...
                if (control_elem)
                {
                    ...
                    else
                    {
                        if (calendaredit)
                        {
                            ...
                            {
                                calendaredit._setAccessibilityStatHidden(true);
                            }
                        }
                    }
                }
    ...
    _pCalendar._setAccessibilityLabel = function (label)
    {
        ...
        if (this.calendaredit)
        {
            this.calendaredit._setAccessibilityLabel(label);
        }
    };
    ...
    _pCalendar._setAccessibilityDescription = function (desc)
    {
        ...
        if (this.calendaredit)
        {
            this.calendaredit._setAccessibilityDescription(desc);
        }
    };
    ...
    _pCalendar._setAccessibilityAction = function (action)
    {
        ...
        if (this.calendaredit)
        {
            this.calendaredit._setAccessibilityAction(action);
        }
    };
...
if (nexacro._CalendarEditControl)
{
    ...
    _pCalendarEditControl._on_getAccessibilityAdditionalLabel = function ()
    {
        ...
        if (nexacro._accessibilitytype > 3)
        {
            return this.value ? this.text : this.displaynulltext ? this.displaynulltext : "";
        }
        return "";
    };
...
if (nexacro.TextArea)
{
	...
	_pTextArea._on_attach_accessibility_contents_handle = _pTextArea._on_created_accessibility_contents;

	_pTextArea.on_apply_prop_accessibilityrole = function ()
	{

        if (nexacro._Browser != "Runtime" && !this.enable)
        {
    ...
    {   
        ...
        if (nexacro._Browser == "Runtime")
        {
            // Desktop NRE만 value + role을 출력하기 위해 사용
            add_label = this.value ? this.text : this.displaynulltext ? this.displaynulltext : "";
        }
...
if (nexacro.ListBox)
{
    ...
    _pListBox.on_killfocus_basic_action = function ()
    {
        ...
        if (nexacro._enableaccessibility)
        {
            var rowobj = this._getItem(this._accessibility_index);
            if (rowobj)
            {
    ...
    //==============================================================================
    _pListBox._getAccessibilityRole = function ()
    {
        var role = "";
        if (this.enable)
        {
            role = (this.accessibilityrole) ? this.accessibilityrole : "none";
        }
        return role;
    };

    _pListBox._on_created_accessibility_contents = function ()
    {
        if (nexacro._enableaccessibility)
        {
            this.on_apply_prop_accessibilitylabel();
    ...
    // 동일동작으로 바인드함 (달라지면 분리)
    _pListBox._on_attach_accessibility_contents_handle = _pListBox._on_created_accessibility_contents;

	_pListBox._on_keydown_basic_accessibility_action = function (keycode, alt_key, ctrl_key, shift_key, refer_comp)
	{
		if (nexacro._enableaccessibility && this._isComponentKeydownAction())
		{
			// 상하 방향키 이전/다음 컴포넌트로 넘길지 처리
			var accIdx = this._accessibility_index;
			if (keycode === nexacro.Event.KEY_UP)
            {
                if (accIdx === -3) //tab key 진입 후 마지막 이동
				{
					accIdx = this._getInnerdatasetInfo("_rowcount");
				}
            ...
            {
                if (accIdx === -3) //tab key 진입 후 첫번째 이동
                {
                    accIdx = -1;
                }
...
if (nexacro._ListBoxItemControl)
{
    ...
    _pListBoxItemControl._on_getAccessibilityAdditionalLabel = function ()
    {
        return "";
    };
    ...
    _pListBoxItemControl._getAccessibilityRole = function ()
    {
        ...
        if (listbox)
        {
            if (listbox.enable)
            {
                ...
                {
                    if (listbox._is_first_focus)
                    {
                        role = listbox._getAccessibilityRole();
                    }
                    else
                    {
    ...
    _pWebBrowser._apply_setfocus = function (evt_name, self_flag, refer_lose_focus, refer_new_focus)
    {
        ...
        if (ifrm_elem)
        {
            if (nexacro._enableaccessibility && nexacro._accessibilitytype == 2)
			{
				if (!evt_name)
				{
					if (refer_lose_focus == this._refform._getPrevComponent(this, true))
						evt_name = "downkey";
					else
						evt_name = "upkey";

					this._old_evt_name = evt_name;
				}
...
if (nexacro.Grid)
{
    ...
    _pGrid._setFocus = function (bResetScroll, dir, block_inner_focus)
    {
        if (nexacro._enableaccessibility)
        {
			...
			{
				this._setAccessibilityRole("noread"); // role 출력 방지
    ...
    {
        // 비어 있는 그리드의 음성 출력
        if (nexacro._accessibilitytype < 4 && this.rowcount < 1)
        {
            return true;
        }
        return (this._accept_focus && this.accessibilityenable);
    };
    ...
    _pGrid._on_acessibility_killfocus = function ()
    {
    ...
    _pGrid._setGridAccessibilityFocus = function (comp)
    {
        if (nexacro._enableaccessibility)
        {
            if (comp.accessibilityenable && !this._accept_focus)
            {
                this._accept_focus = true;
            }
        }
    ...
    _pGrid._on_attach_accessibility_contents_handle = function ()
    {
        if (nexacro._enableaccessibility)
        {
            var editable_cell = null;

            // 처음 editable cell의 정적 editcontrol 생성
            var editable_pos = this._getFirstEditableCell();
            if (editable_pos)
            {
	...
	{
		if (this._hasTree && this.treeuseexpandkey == false)
		{
			// tree의 단축키 지원
        ...
        if (refer_comp)
        {
            ...
            if (refer_cell && _is_focusin_process)
            {
                if (refer_cell instanceof nexacro._GridCellControl)
                {
                    ...
                    if (refer_cell._rowidx != this.currentrow || refer_cell._cellidx != this.currentcell)
					{
						this._currentBand = refer_cell._band.name; //band 설정
                        this.currentrow = refer_cell._rowidx;
                        this.currentsubrow = refer_cell._refinfo ? refer_cell._refinfo._row : this.currentrow;
                        this.currentcol = refer_cell._refinfo ? refer_cell._refinfo._col : this.currentcol;
    ...
    _pGrid._moveToAccessibilityCell = function (type, tabstop, extcomp, colmove)
    {
        ...
        if (this._is_band_focus && !tabstop)
        {
            if (this._currentBand == "head" && this._focused_cell >= 0)
            {
                ...
                this.currentcell = this._focused_cell;
            }
            else
            {
                retn = true;
		...
		if (next_idx.row > -1 && next_idx.cell > -1)
		{
			...
			if (create_cell && create_cell._hasEditor())
			{
				if (create_cell._static_create === false)
                {
					...
					if (editor)
                    {
                        this._editor = this._currentCellEditor = editor;
...
if (nexacro._GridCellControl)
{
    ...
    _pGridCell._on_created_accessibility_contents = function (win)
    {
        ...
        if (grid)
        {
            ...
            if (control_elem)
            {
                // 해당 코드 원인 파악해서 연관성 없으면 통합해서 처리 (attach와 created)
                if (nexacro._accessibilitytype == 5)
                {
	...
	_pGridCell._created_accessibility_process = function ()
	{
		...
		if (control_elem)
		{
			...
			var subcontrol_elem = this._subComp ? this._subComp.getElement() : null;
			if (subcontrol_elem)
			{
    ...
    _pGridCell.set_accessibilityrole = function (accessibilityrole)
    {
        ...
        else
        {
            ...
            if (nexacro._accessibilitytype < 4 && nexacro._Browser == "Chrome")
            {
                accessibilityrole = this._getAccessibilityRoleType(accessibilityrole);
            }
        }
	...
	_pGridCell._getAccessibilityRole = function ()
	{
        ...
        if (nexacro._accessibilitytype < 4 && nexacro._Browser == "Chrome")
        {
            ...
            else
            {
                ...
                role = this._getAccessibilityRoleType();
            }
        }
        else
        {
            if (role === "gridcell")
            {
                var _role = this._getAccessibilityRoleType();                
    ...
    _pGridCell.__getAccessibilityMakeAddLabelMiddleClass = function ()
    {
        ...
        if (curCellinfo._type == "body" || curCellinfo._type == "summary")
        {
            ...
            for (i = 0, n = cells.length; i < n; i++)
            {
                ...
                {
                    var displaytext = cell._displaytext ? " " + cell._displaytext : "";
                    label = cell._getAccessibilityLabel(true) + displaytext;
    ...
    // cell의 성격에 따라 구분하여 지정한 경우 처리하고, 아니면 gridcell로 통일하는 함수용
    // 기존 이름에 맞지 않는 용도로 이름 변경
    _pGridCell._getAccessibilityRoleType = function (role)
    {
        var cellinfo = this._refinfo;
        if (!role)
        ...
        if (cellinfo)
        {
            if (role === "columnheader" && cellinfo._type == "head")
                return role;

            // rowheader는 OS마다 동작이 해 요청시 검토
            //if (role === "rowheader" && cellinfo._type == "body" && cellinfo._area == "left")
            //	return role;
        }

        // 내부 control의 role은 별개로 처리하고 cell들은 전부 layout role 처리
        return "gridcell";
    };
...
if (nexacro._CellControl)
{
	...
	_pCellControl._on_create_accessibility_contents = function ()
    {
        if (nexacro._enableaccessibility)
        {
            ...
            {
                var cellinfo = this._refinfo;
                var edittype = cellinfo ? cellinfo._getEdittype(this._getDataRow()) : null;
    ...
    _pCellControl._getAccessibilityMakeAddLabel = function ()
    {
        ...
        {
            // Grid의 접근성이 활성화 되어야 설정 (accessibilityenable == true)
            ...
            if (nexacro._accessibilitytype > 3 || !this._isAccessibilityEnable())
            {
                if (this._cellidx === 0 && this._rowidx < 1)
                {
                    ...
                    if (!view._headBand)
                    {
                        if (view._summBand && view.summarytype === "top")
                        {
                            bandid = "summ";
                        }
    ...
    _pCellControl._getEditorInputElement = function ()
    {
        ...
        {
            if (editor.calendaredit)
                editor = editor.calendaredit;
            else if (editor.comboedit)
                editor = editor.comboedit;
...
{    
    ...
    {
        if (nexacro._enableaccessibility && this._displaymode)
            return false;
        else
...
if (nexacro._CellCalendarControl)
{
    ...
    if (nexacro._Browser == "IE" || (nexacro._Browser == "Edge" && nexacro._BrowserType == "Edge"))
    {
        _pCellCalendar._setAccessibilityInputLabel = function (label)
        {
            var calendaredit = this.calendaredit;
            if (calendaredit)
            {
    ...
    _pCellCalendar._getAccessibilityReadLabel = function ()
    {
        var cellobj = this._cellobj;
        if (cellobj)
        {
            return cellobj._getAccessibilityReadLabel();
        }
        return "";
    };
...
if (nexacro._CellComboControl)
{
    ...
    if (nexacro._Browser == "IE" || (nexacro._Browser == "Edge" && nexacro._BrowserType == "Edge"))
    {
        _pCellCombo._setAccessibilityInputLabel = function (label)
        {
            var comboedit = this.comboedit;
            if (comboedit)
            {
    ...
    {
        var cellobj = this._cellobj;
        var control_elem = this.comboedit ? this.comboedit.getElement() : null;
        if (cellobj && control_elem)
        {
            var cell_control_elem = cellobj.getElement();
            if (cell_control_elem)
...
if (nexacro._CellCheckboxControlBase)
{
    ...
    {
        // lbuttondown시 focus때문에 DOM이 이동하지 않도록 함. (비접근성)
        if (!nexacro._enableaccessibility)
            return;

        // 접근성 일때는 focus 줘야 음성 출력 및 키액션 가능
        nexacro.Component.prototype._apply_setfocus.call(this, evt_name, callback);
    }; 
...
if (nexacro._CellRadioItemControl)
{
    ...
    _pCellRadioItem._apply_setfocus = function (evt_name, callback)
    {
        // lbuttondown시 focus때문에 DOM이 이동하지 않도록 함. (비접근성)
        if (!nexacro._enableaccessibility)
            return;
...
if (nexacro._CellImageControl)
{
	...
	_pCellImage._getAccessibilityRole = function ()
	{
		...
		else
		{
			...
			if (cellobj)
			{
				if (cellobj.accessibilityrole == "gridcell")
				{
					return this.accessibilityrole;
				}

				return cellobj.accessibilityrole;
			}
...
if (nexacro.ScrollBarControl)
{
    ...
    _pScrollBar.accessibilityenable = false;
...
if (nexacro.ListView)
{
    ...
    // Touch Event Process (Android NRE Accessibility)
    _pListView.on_fire_sys_onaccessibilitygesture = function (direction, fire_comp, refer_comp)
    {
        var rowcnt = this._getBindCount();
        if (rowcnt > 0)
        {
            var cur_sel = this.getSelect();
            if (cur_sel && cur_sel.row > -1)
            {
                var focus_comp = null;

                // 이벤트에서 전달되는 direction과 다르게 처리 하기 위함 (next: 1, prev: -1, current: 0)
                var dir = direction === 0 ? -1 : direction;

                var cellidx = cur_sel.cell;
                var rowidx = cur_sel.row;
                var bandidx = cur_sel.band;
                var item = this._getItem(rowidx);

                // expand control to bodyband lastcell focus
                if (this._is_expandbar_focus && bandidx > -1)
                {
                    if (dir < 0)
                    {
                        // First cell in prev band to last cell in cur band focus
                        cellidx = 0;
                        bandidx++;
                    }
                    else if (dir > 0)
                    {
                        // Last cell in cur band to first cell in next band focus
                        cellidx = item[bandidx]._getLastChild() ? item[bandidx]._getLastChild()._cellidx + 1 : 0;
                    }
                }

                while (item)
                {
                    // row 간 이동시 첫번째 또는 마지막 cell focus 가능한지 체크
                    if (cur_sel.row !== rowidx)
                    {
                        focus_comp = this._findNextCell(item, cellidx, bandidx, 0, false, true);
                    }

                    focus_comp = (!focus_comp) ? this._findNextCell(item, cellidx, bandidx, dir, false, true) : focus_comp;

                    if (focus_comp)
                    {
                        // focus 처리해야 selection 정보에 위치 저장 됨
                        if (focus_comp instanceof nexacro._ListViewBandExpandbarControl)
                        {
                            this._is_expandbar_focus = true;
                        }
                        focus_comp._on_focus(true);
                        focus_comp._setAccessibilityNotifyEvent(direction);
    ...
    // Accessibility Notify Process (Android NRE Accessibility)
    _pListView._setAccessibilityNotifyEvent = function (direction)
    {
        var focus_comp = null;
        if (direction > 0)
        {
            // 첫번째 Cell
            focus_comp = this._getFirstCell(false, true);
        }
        else
        {
            //마지막 Cell
            focus_comp = this._getLastCell(false, true);
        }
    ...
    // Touch Hover Process (Android NRE Accessibility)
    _pListView._setAccessibilityInfoByHover = function (control)
    {
        if (!control)
            return false;

        if (control instanceof nexacro._ListViewBandExpandbarControl)
        {
            this._is_expandbar_focus = true;
            control._on_focus(true);
            return control._setAccessibilityNotifyEvent();
        }
        else
        {
            var obj = this._findObj(control);
            var cell = obj ? obj.cellobj : null;

            if (cell && cell instanceof nexacro._ListViewCellControl && cell.accessibilityenable == true)
            {
                cell._on_focus(true);
                return cell._setAccessibilityNotifyEvent();
            }
        }
...
if (nexacro._ListViewCellControl)
{
    ...
    //==============================================================================
    // ListViewCellControl : Accessibility Function
    //==============================================================================
...
if (nexacro.Static)
{
    ...
    {
        if (nexacro._accessibilitytype < 4 || this.usedecorate)
        {
            // link 걸리거나 heading이 걸려 있으면 내부로 초점 이동하기 때문에 displaytext를 설정안함
            if (this._isAccessibilityRoleHeading() || this.usedecorate)
            {
                return "";
            }
        }
    ...
    {
        if (this.usedecorate && this._linkCount > 0)
        {
            var cell_elem = this._cell_elem;
            if (cell_elem)
            {
...
{
    var _pStepImageButtonCtrl = nexacro.StepImageButtonControl.prototype;

    //==============================================================================
    // StepImageButtonControl : Override
    //==============================================================================
    _pStepImageButtonCtrl._on_created_accessibility_contents = function ()
    {
        if (nexacro._enableaccessibility)
        {
            if (nexacro._Browser == "Runtime" && nexacro._accessibilitytype == 5) // TalkBack
            {
                this._setAccessibilityStatHidden(true);
            }
        }
...
if (nexacro.ProgressBar)
{
    ...
    //===============================================================
    _pProgressBar._on_created_accessibility_contents = function (win)
    {
        if (nexacro._enableaccessibility)
        {
            this._setAccessibilityInfoValueMin(this.min);
