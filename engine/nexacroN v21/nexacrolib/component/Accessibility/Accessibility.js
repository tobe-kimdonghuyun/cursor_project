...
if (nexacro.Component)
{
	...
	{
		var description = this._getLinkedDescription(this.accessibilitydescription);
		return (description) ? description : this.on_get_accessibility_description();
	};
    ...
    _pComponent._setAccessibilityRole = function (role)
    {
        ...
        if (control_elem)
        {
            ...
            {
                var level = +(role.charAt(role.length - 1))
    ...
    {   
        ...
        if (!nexacro._enableaccessibility || !this._is_listtype || !this.itemaccessibilityenable)
            return false;
        return true;
    };

    _pComponent._updateAccessibilityRole = function ()
    {
        var control_elem = this.getElement();
        if (control_elem)
            control_elem._updateAccessibilityRole();
    };
    ...
    _pComponent._isAccessibilityCotainerComponent = function ()
    {
        ...
        if (nexacro._accessibilitytype == 4 || nexacro._accessibilitytype == 5)
        {
            if (this.accessibilityenable == true && (this.accessibilitydesclevel == "all" || this.accessibilitydesclevel == "self"))
                return true;
        }
        return false;
    };
                ...
                if (modal_info)
                {
                    ...
                    if (old_focus_path && old_focus_path.length > 0)
                    {
                        ...
                        for (i = 0; i < old_focus_path_len; i++)
                        {
                            ...
                            //공통에서 통합하고 싶었으나, 현재 동작에서 오류가 많아 추후 처리 해야할것으로 보임 RP 104949
                            //Grid에서 Cell에 대한 lastfocus가 동작하지 않아 오동작을 발생
