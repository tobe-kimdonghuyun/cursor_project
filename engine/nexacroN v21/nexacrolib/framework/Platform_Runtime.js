...
if (nexacro._Browser == "Runtime")
{
    if (!nexacro._init_platform_runtime)
    {
		...
		{
			if (_window._touch_count && _window._touch_count > 0)
			{
				_window._touch_count--;
				_window._cancelEvent();
				return;
			}
	    ...
	    nexacro._syshandler_onmousemove_forward = function (_window, elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY, metaKey)
		{
			...
			_window._last_mouseover_elem = elem;
	    ...
	    nexacro._syshandler_ontouchstart_forward = function (_window, elem, touchid, time/*, windowX, windowY, screenX, screenY, is_last_changedtouch, changed*/)
	    {
			...
			if (!_window._touch_slop)
			{
				_window._touch_slop = 10;
			}
			_window._touch_end = false;
            var last_focused = _window._last_focused_elem;
            var isComposing = false;
	    ...
	    nexacro._syshandler_ontouchmove_forward = function (_window, elem, touchid, time/*, windowX, windowY, screenX, screenY, is_last_changedtouch*/)
	    {
			...
			{
				current_touches = changedTouches;
			}

			if (_window._touch_last)
			{
				var delta_x = Math.abs(_window._touch_last[0].screenX - current_touches[0].screenX);
				var delta_y = Math.abs(_window._touch_last[0].screenY - current_touches[0].screenY);

				if (delta_x < _window._touch_slop && delta_y < _window._touch_slop)
				{
					return;
				}
			}
			...
			if (current_touches)
			{
				_window._touch_list.add(current_touches[0].id.toString(), current_touches);
				_window._touch_last = current_touches;
			}
	    ...
	    nexacro._syshandler_ontouchend_forward = function (_window, elem, touchid, time/*, windowX, windowY, screenX, screenY, is_last_changedtouch*/)
	    {
			...
			if (!_window._touch_count)
				_window._touch_count = 0;
			_window._touch_count++;

			var ret = nexacro._syshandler_ongesture_touchend_forward(_window, elem, touches, changedTouches, time);

			var is_tab = false;
			if (_window._gesture_manager && _window._gesture_manager._is_tap === true)
			    is_tab = true;

			if (elem && elem._use_decoration && is_tab === true)
	    	{
	    		// click link
	    		var firecur_comp = _window._findComponentForEvent(elem);
	...
	nexacro._getTargetOrigin = nexacro._emptyFn;
