...
if (!nexacro._bInitPlatform)
{
    ...
    */
    ...
    __pWindow._on_default_sys_keydown = function (elem, keycode, altKey, ctrlKey, shiftKey, metaKey)
    {
        ...
        // IME 체크 위해 Platfotm_HTML5.js 이동
        ...
        if (this._is_active_window !== true)
        {
            ...
            if (frame)
            {
                ...
                if (this._last_focused_elem)
                {
                    ...
                    var linked_comp = this.findComponent(this._last_focused_elem);
                    if (linked_comp)
                    {
                        linked_comp._apply_setTagfocus();
                    }
                }    
            }
    ...
    */
    ...
    _pTouchGestureManager.ontap = function (elem, data, touches/*, changedTouches*/)
    {
        ...
        if (compinfo && compinfo[0])
        {
            ...
            else if (touch)
            {
                ...
                // 103553 fling 중 터치하면 fling 시작 시점의 start element와 fling 중지 시점의 start element가 달라 발생하는 문제 수정
                // fling 중 touchend에 focused가 남는 현상 수정 작업은 click 발생하여 원복
        ...
        if (nexacro._OS == "iOS")
        {
            switch (type)
            {
                ...
                case 2: // touchend
                    ...
                    if (focused_comp && focused_comp._has_inputElement)
                    {
                        return true;
                    }
    ...
    _pEnvironment.accessibilitycontentsearchkey = false; // component searching by nexacro
    ...
    _pEnvironment.usesoftkeyboard = true;
    nexacro._usesoftkeyboard = true;
    _pEnvironment.enabletouchsoftkeyboard = false;
    nexacro._enabletouchsoftkeyboard = false;

    _pEnvironment.calendarrestoreonblur = true;
    ...
    _pEnvironment.set_calendarrestoreonblur = function (v)
    {
        ...
        if (nexacro._calendarrestoreonblur != v || this.calendarrestoreonblur != v)
        {
            this.calendarrestoreonblur = v;
	...
	{
		v = nexacro._toBoolean(v);
		if (v != this.enabletouchsoftkeyboard)
		{
			this.enabletouchsoftkeyboard = v;
