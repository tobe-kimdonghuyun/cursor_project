...
if (!nexacro._Init_systembase)
{
    ...
    _pTouchSession.onaction = function (name, status, data)
    {
        ...
        {
            //trace("tap: " + elem.name);
            // fling 강제 종료시에는 data가 null 이기 때문에 tab 처리 안함
            if (this._prev_session_action_type != "fling" || this._prev_session_action_data != null)
            {
    ...
    _pTouchActionSlide._on_action_fling = function (/*status*/)
    {
        ...
        this._duration_threshold = 1; //init
