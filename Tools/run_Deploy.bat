...
rem --- 7. Build extra options ---
set "EXTRA_OPTS="
if defined OPT_MERGE    set "EXTRA_OPTS=!EXTRA_OPTS! -MERGE"
if defined OPT_COMPRESS set "EXTRA_OPTS=!EXTRA_OPTS! -COMPRESS"
if defined OPT_SHRINK   set "EXTRA_OPTS=!EXTRA_OPTS! -SHRINK"

rem --- 7-1. Detect nexacro version from nexacrolib.json -> set RULE_OPT ---
set "NEXLIB_JSON=%~dp0..\nexacrolib\nexacrolib\nexacrolib.json"
set "NEXACRO_VER="
for /f "usebackq delims=" %%V in (`powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0get_version.ps1" -JsonFile "!NEXLIB_JSON!" 2^>nul`) do set "NEXACRO_VER=%%V"

set "RULE_OPT=-GENERATERULE"
if not "!NEXACRO_VER!"=="" (
    set "_VER_PREFIX=!NEXACRO_VER:~0,2!"
    if "!_VER_PREFIX!"=="21" (
        set "RULE_OPT=-CSSRULE"
        echo [INFO] nexacro version: !NEXACRO_VER! -> using -CSSRULE
    ) else (
        echo [INFO] nexacro version: !NEXACRO_VER! -> using -GENERATERULE
    )
) else (
    echo [WARN] Could not read version from nexacrolib.json. Defaulting to -GENERATERULE
)

rem --- 7-2. Convert JS files in NexacroLibPath to UTF-8 BOM ---
echo [INFO] Converting JS files to UTF-8 BOM: !NexacroLibPath!
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0convert_utf8bom.ps1" -TargetDir "!NexacroLibPath!"
if errorlevel 1 (
    echo [WARN] UTF-8 BOM conversion failed. Deploy may fail with encoding errors.
)

echo.
echo [INFO] start.bat : !START_BAT!
echo [INFO] ProjectPath: !ProjectPath!
echo [INFO] OutputPath : !OutputPath!
echo [INFO] DeployPath : !DeployPath!
echo [INFO] RuleOption : !RULE_OPT!
echo.

rem --- 8. Run Java directly (bypass pause in start.bat) ---
for %%F in ("!START_BAT!") do set "_BIN_DIR=%%~dpF"
set "_BIN_DIR=!_BIN_DIR:~0,-1!"
for %%F in ("!_BIN_DIR!\..") do set "_JAR_ROOT=%%~fF"
echo [CMD] "!JAVA_HOME!\bin\java.exe" "-Dlog4j.configurationFile=!_JAR_ROOT!\log4j2.xml" -classpath "!_JAR_ROOT!\libs\*" com.nexacro.build.cli.Main -P "!ProjectPath!" -B "!NexacroLibPath!" -O "!OutputPath!" !RULE_OPT! "!GenerateRule!" -D "!DeployPath!"!EXTRA_OPTS!
"!JAVA_HOME!\bin\java.exe" "-Dlog4j.configurationFile=!_JAR_ROOT!\log4j2.xml" -classpath "!_JAR_ROOT!\libs\*" com.nexacro.build.cli.Main -P "!ProjectPath!" -B "!NexacroLibPath!" -O "!OutputPath!" !RULE_OPT! "!GenerateRule!" -D "!DeployPath!"!EXTRA_OPTS!
if errorlevel 1 (
    echo [ERROR] Java deploy failed. Aborting post-deploy steps.
    exit /b 1
)
...
for /d %%D in ("%COMP_SRC%\*") do (
    echo   [%%~nxD]
    set "HAS_SUB=0"
    for /d %%S in ("%%D\*") do (
        set "HAS_SUB=1"
        xcopy "%%S" "%COMP_DEST%\%%~nxD\%%~nxS\" /E /I /Y /Q
        if errorlevel 1 echo [WARN] Failed to copy: %%~nxD\%%~nxS
    )
    if "!HAS_SUB!"=="0" (
        xcopy "%%D" "%COMP_DEST%\%%~nxD\" /E /I /Y /Q
        if errorlevel 1 echo [WARN] Failed to copy files: %%~nxD
    )
)
...
if "!OPT_IGNORE!"=="1" (
    ...
    set "PSCOMPRESS_SRC=!ENGINE_DIR!"
    set "PSCOMPRESS_DST=!ZIP_OUT!"
    powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0compress_deploy.ps1"
    if errorlevel 1 (
